...
param1 = {'max_depth': 7, 'eta': 0.1, 'objective': 'multi:softprob', 'num_class': 5,
          'eval_metric': 'mlogloss', 'subsample': 0.75, 'colsample_bytree': 0.85}

param2 = {'max_depth': 6, 'eta': 0.1, 'objective': 'multi:softprob', 'num_class': 5,
          'eval_metric': 'mlogloss', 'subsample': 0.85, 'colsample_bytree': 0.75}

param3 = {'max_depth': 8, 'eta': 0.1, 'objective': 'multi:softprob', 'num_class': 5,
          'eval_metric': 'mlogloss', 'subsample': 0.65, 'colsample_bytree': 0.75}

param4 = {'max_depth': 9, 'eta': 0.1, 'objective': 'multi:softprob', 'num_class': 5,
          'eval_metric': 'mlogloss', 'subsample': 0.55, 'colsample_bytree': 0.65}

param5 = {'max_depth': 10, 'eta': 0.1, 'objective': 'multi:softprob', 'num_class': 5,
          'eval_metric': 'mlogloss', 'subsample': 0.55, 'colsample_bytree': 0.55}
...
prediction_4 = xgb.train(param3, dtrain, num_round)
prediction_5 = xgb.train(param3, dtrain, num_round)
...
prediction = prediction_1.predict(dtest)
prediction = pd.DataFrame(prediction)

submission = pd.DataFrame()
submission["id"] = test_ID.values
submission["Adoption"] = prediction[2]
submission["Died"] = prediction[4]
submission["Euthanasia"] = prediction[1]
submission["Return_to_owner"] = prediction[0]
submission["Transfer"] = prediction[3]
