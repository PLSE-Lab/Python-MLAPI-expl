...
# (note the capital 'D' in date!).This is also of type object as suspected
...
# double-check that the dtype is correct!).Tried to manually parse dates using a specified format but
#maybe we can specify multiple formats

print(earthquakes.head(5))
earthquakes['earthquake_date_parsed']=pd.to_datetime(earthquakes['Date'],infer_datetime_format=True)
print(earthquakes.dtypes)
print(earthquake_date_parsed.head(5))
...
day_of_month_earthquakes=earthquakes['earthquake_date_parsed'].dt.day
...
# plot the day of the month(x axis is the no of days in a month and y axis)
...
# earthquake dataset and make sure they make sense.The Data is correctly formatted as needed

day_of_month_earthquakes=day_of_month_earthquakes.dropna()

print(day_of_month_earthquakes.value_counts)
sns.distplot(day_of_month_landslides, kde=False, bins=31)
...
'''
Removed the Values which are unknown as we don't need them.Now here we are only given the year and nothing else

Can also do another thing.Store the Prefix for the Date as a separate column and the date as a separate column
which we can then pass to the datetime function
'''

def parse_string(date_str):
    
    split_value=date_str.split()
    
    print(split_value,split_value[0],split_value[1])
    
    return pd.Series([split_value[0],split_value[1]])

#Can Also use the Replace method to convert the Unknown to np.Nan.CUrrently ignoring the rows with Unknown
dummy=volcanos.loc[volcanos['Last Known Eruption'] != 'Unknown']

#Create new columns in our dataframe with the returning value as a series
volcanos[['No_Prefix_date','Date_prefix']]=dummy['Last Known Eruption'].apply(parse_string)

print(volcanos['No_Prefix_date'],volcanos['Date_prefix'])


# In[72]:


get_ipython().run_line_magic('pinfo', 'dummy.apply')
