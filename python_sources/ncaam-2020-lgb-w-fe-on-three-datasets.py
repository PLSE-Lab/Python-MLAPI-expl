...
DATA_DIR = '/kaggle/input/google-cloud-ncaa-march-madness-2020-division-1-mens-tournament/'
...
MRSCResults = pd.read_csv(DATA_DIR + '/MDataFiles_Stage1/MRegularSeasonCompactResults.csv')

A_w = MRSCResults[MRSCResults.WLoc == 'A']    .groupby(['Season','WTeamID'])['WTeamID'].count().to_frame()    .rename(columns={"WTeamID": "win_A"})
N_w = MRSCResults[MRSCResults.WLoc == 'N']    .groupby(['Season','WTeamID'])['WTeamID'].count().to_frame()    .rename(columns={"WTeamID": "win_N"})
H_w = MRSCResults[MRSCResults.WLoc == 'H']    .groupby(['Season','WTeamID'])['WTeamID'].count().to_frame()    .rename(columns={"WTeamID": "win_H"})
win = A_w.join(N_w, how='outer').join(H_w, how='outer').fillna(0)

H_l = MRSCResults[MRSCResults.WLoc == 'A']    .groupby(['Season','LTeamID'])['LTeamID'].count().to_frame()    .rename(columns={"LTeamID": "lost_H"})
N_l = MRSCResults[MRSCResults.WLoc == 'N']    .groupby(['Season','LTeamID'])['LTeamID'].count().to_frame()    .rename(columns={"LTeamID": "lost_N"})
A_l = MRSCResults[MRSCResults.WLoc == 'H']    .groupby(['Season','LTeamID'])['LTeamID'].count().to_frame()    .rename(columns={"LTeamID": "lost_A"})
...
A_scr = wl_scr[wl_scr.Loc == 'A'].groupby(['Season','TeamID'])        ['Score','relScore'].mean()        .rename(columns={"Score": "Score_A", "relScore": "relScore_A"})
N_scr = wl_scr[wl_scr.Loc == 'N'].groupby(['Season','TeamID'])        ['Score','relScore'].mean()        .rename(columns={"Score": "Score_N", "relScore": "relScore_N"})
H_scr = wl_scr[wl_scr.Loc == 'H'].groupby(['Season','TeamID'])        ['Score','relScore'].mean()        .rename(columns={"Score": "Score_H", "relScore": "relScore_H"})
All_scr = wl_scr.groupby(['Season','TeamID'])['Score','relScore']    .mean().rename(columns={"Score": "Score_All", "relScore": "relScore_All"})
scr = A_scr.join(N_scr, how='outer').join(H_scr, how='outer')    .join(All_scr, how='outer').fillna(0).reset_index()
...
MRSDetailedResults = pd.read_csv(DATA_DIR + '/MDataFiles_Stage1/MRegularSeasonDetailedResults.csv')
...
MMOrdinals = pd.read_csv(DATA_DIR + '/MDataFiles_Stage1/MMasseyOrdinals.csv')

MOR_127_128 = MMOrdinals[(MMOrdinals.SystemName == 'MOR') &                 ((MMOrdinals.RankingDayNum == 127)                  | (MMOrdinals.RankingDayNum == 128))]                [['Season','TeamID','OrdinalRank']]
MOR_50_51 = MMOrdinals[(MMOrdinals.SystemName == 'MOR') &                 ((MMOrdinals.RankingDayNum == 50)                  | (MMOrdinals.RankingDayNum == 51))]                [['Season','TeamID','OrdinalRank']]
MOR_15_16 = MMOrdinals[(MMOrdinals.SystemName == 'MOR') &                 ((MMOrdinals.RankingDayNum == 15)                  | (MMOrdinals.RankingDayNum == 16))]                [['Season','TeamID','OrdinalRank']]
...
MOR = MOR_127_128.merge(MOR_50_51, how='left', on=['Season','TeamID'])        .merge(MOR_15_16, how='left', on=['Season','TeamID'])
...
MOR_stats = MOR_tmp.groupby(['Season','TeamID'])['OrdinalRank']            .agg(['max','min','std','mean']).reset_index()
MOR_stats.columns = ['Season','TeamID','RankMax','RankMin','RankStd','RankMean']

MOR = MOR.merge(MOR_max, how='left', on='Season')        .merge(MOR_stats, how='left', on=['Season','TeamID'])
...
wl_1.columns = [str(col) + '_1' if col not in ['Season','TeamID']                 else str(col) for col in wl_1.columns ]

wl_2 = wl.loc[:,['Season','TeamID','win_pct_A','win_pct_N',
                 'win_pct_H','win_pct_All']]
wl_2.columns = [str(col) + '_2' if col not in ['Season','TeamID']                 else str(col) for col in wl_2.columns ]

scr_1 = scr.copy()
scr_1.columns = [str(col) + '_1' if col not in ['Season','TeamID']                  else str(col) for col in scr_1.columns ]

scr_2 = scr.copy()
scr_2.columns = [str(col) + '_2' if col not in ['Season','TeamID']                  else str(col) for col in scr_2.columns ]

dt_1 = dt.copy()
dt_1.columns = [str(col) + '_1' if col not in ['Season','TeamID']                 else str(col) for col in dt_1.columns ]

dt_2 = dt.copy()
dt_2.columns = [str(col) + '_2' if col not in ['Season','TeamID']                 else str(col) for col in dt_2.columns ]

MOR_1 = MOR.copy()
MOR_1.columns = [str(col) + '_1' if col not in ['Season','TeamID']                  else str(col) for col in MOR_1.columns ]

MOR_2 = MOR.copy()
MOR_2.columns = [str(col) + '_2' if col not in ['Season','TeamID']                  else str(col) for col in MOR_2.columns ]
...
TCResults = pd.read_csv(DATA_DIR + '/MDataFiles_Stage1/MNCAATourneyCompactResults.csv')
...
if STAGE_1:
    MSampleSubmission = pd.read_csv(DATA_DIR + 'MSampleSubmissionStage1_2020.csv')
else:
...
def model_training(X, y, cv, groups, params, metric, early_stopping=10,     plt_iter=True, X_test=[], cat_features=[]):
    ...
    feature_importance = feature_importance[["feature", "importance"]]                         .groupby("feature").mean().sort_values(
...
pred = pd.concat([test.ID, pd.Series(test_pred)], axis=1).groupby('ID')[0]        .mean().reset_index().rename(columns={0:'Pred'})
...
# For some reasons, the score here is slightly different from LB
...
if STAGE_1:
    ...
    rslt['team1'] = TCResults_s.apply(lambda x: x.WTeamID                                       if x.WTeamID < x.LTeamID else x.LTeamID
                                      , axis=1)
    rslt['team2'] = TCResults_s.apply(lambda x: x.WTeamID                                       if x.WTeamID > x.LTeamID else x.LTeamID
                                      , axis=1)
    rslt['wl'] = TCResults_s.apply(lambda x: 1 if x.WTeamID < x.LTeamID else 0
                                   , axis=1)
    rslt['ID'] = rslt.apply(lambda x: str(x.season) + '_' + str(x.team1)                             + '_' + str(x.team2), axis=1)
