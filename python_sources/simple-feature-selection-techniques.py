...
# 
# In this notebook I'll show you some simple techniques for selecting features to keep/remove from a dataset. I've found [this](https://github.com/WillKoehrsen/feature-selector) awesome library made by Will Koehrsen, it is perfect for the job.
# You can use this simple class for finding features to remove, it has five main methods:
# 
...
# 
# 
# For this notebook I used [@artgor](https://www.kaggle.com/artgor)'s dataset.
# - Dataset [here](https://www.kaggle.com/artgor/dsbowl19-features)
# - Example kernel [here](https://www.kaggle.com/artgor/oop-approach-to-fe-and-models)
...
class FeatureSelector:
    ...
    def identify_zero_importance(self, task, eval_metric=None, n_estimators=1000,
        ...
        for _ in range(n_iterations):
            ...
            if task == 'classification':
                model = lgb.LGBMClassifier(n_estimators=n_estimators, learning_rate=0.05, verbose=-1)

            elif task == 'regression':
                model = lgb.LGBMRegressor(n_estimators=n_estimators, learning_rate=0.05, verbose=-1)
...
train_new = pd.read_csv("../input/dsbowl19-features/generated_train_new.csv")
train_new.head(3)
...
label = train_new['accuracy_group']
train_new = train_new.drop(columns = ['accuracy_group', 'game_session', 'installation_id', 'timestamp'])

# FeatureSelector will one-hot encode these features.
train_new['title'] = train_new['title'].astype('category')
train_new['world'] = train_new['world'].astype('category')


# # Feature Selector
# The `FeatureSelector` only requires a dataset with samples in the rows and features in the columns.

# In[ ]:


fs = FeatureSelector(data=train_new, labels=label)


# ## Missing Values
# With this method you can easily find any columns with a missing fraction greater than a specified threshold. 
...
# More details about the missing stats
fs.missing_stats.head(10)


# ## Single Unique Value
# With this method you can identify any features that have a constant value.
...
# Iteresting, let's see. Maybe a bug in Andrew's code?!
train_new['Clip'].value_counts()
...
# This method finds pairs of highly correlated features.
...
fs.identify_zero_importance(task = 'classification', eval_metric = 'multi_logloss', n_iterations = 5, early_stopping = True, n_estimators=1000)


# One hot encoding the features are saved in the one_hot_features attribute of the FeatureSelector. The original features are saved in the base_features.
...
# Example: Threshold = 0.99 will tell us the number of features needed to account for 99% of the total importance.
...
# You can remove features detected by a specific method:
# train_no_missing = fs.remove(methods = ['missing'])

# You can remove multiple method's detectoins too:
# train_no_missing_zero = fs.remove(methods = ['missing', 'zero_importance'])

# Or you can remove all
train_removed = fs.remove(methods = 'all')

# Remove one-hot encoded features (and restore the original)
...
# Don't forget to remove the features from the test set too!
