...
def trainModel():
    ...
    model = Sequential()
    model.add(Conv2D(26, kernel_size = (3, 3), activation='relu', input_shape=(IMG_SIZE, IMG_SIZE, 1)))
    ...
    model.fit(trainImages, trainLabels, batch_size = 50, epochs = 1, verbose = 1, validation_data=(validationImages, validationLabels))
...
def captureVideoAndTestFrames(model, index):
    ...
    prediction, index = predict(model, frame_test)
    print (prediction)
...
model = trainModel()
#saveModel(model, 'model10.json', 'model10.h5')
#model = loadModel('model10.json', 'model10.h5')
#loadModelAndTestImages()
testModel(model)
"""
while True:
    captureVideoAndTestFrames(model, IMAGE_INDEX)
    IMAGE_INDEX += 1
    time.sleep(2)
"""