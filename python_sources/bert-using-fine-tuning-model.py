...
# Still based on Matei's great kernel [Taming the BERT - a baseline](https://www.kaggle.com/mateiionita/taming-the-bert-a-baseline)
# 
# Fine tuned model from my kernel [bert-finetuning-classifier](https://www.kaggle.com/wochidadonggua/bert-finetuning-classifier)
...
get_ipython().system(' ls ../input/bert-finetuning-classifier/model_repo/outputs')
...
def run_bert(data):
	...
	os.system("python3 extract_features.py 	  --input_file=input.txt 	  --output_file=output.jsonl 	  --vocab_file=uncased_L-12_H-768_A-12/vocab.txt 	  --bert_config_file=uncased_L-12_H-768_A-12/bert_config.json 	  --init_checkpoint=../input/bert-finetuning-classifier/model_repo/outputs/model.ckpt-306 	  --layers=-1 	  --max_seq_length=256 	  --batch_size=8")
...
# 		eee = sorted([(emb_A,A_offset), (emb_B,B_offset), (emb_P,P_offset)], key=lambda x: x[1])
# 		emb.iloc[i] = [_[0] for _ in eee] + [label]
...
for r in range(rounds):
    ...
    folds = KFold(n_splits=n_fold, shuffle=True, random_state=3+r)
    scores = []
...
#     print(scores)
    print("Round {} oof score: {}".format(r, log_loss(Y_train, oof)))
    final_oof_score += log_loss(Y_train, oof) / float(rounds)
    print("Test score:", log_loss(Y_development,prediction))
    final_test_score += log_loss(Y_development,prediction) / float(rounds)
    final_prediction += prediction / float(rounds)
    
print("Mean oof score of {} rounds: {}".format(rounds, final_oof_score))
print("Mean Test score of {} rounds: {}".format(rounds, final_test_score))
...
submission["A"] = final_prediction[:,0]
submission["B"] = final_prediction[:,1]
submission["NEITHER"] = final_prediction[:,2]
submission.to_csv("submission_bert.csv")


# From the results, we can see that the local CV has improved significantly and the predictions for test sets are worse. But in the  [discussion](https://www.kaggle.com/c/gendered-pronoun-resolution/discussion/87145), other kagglers got a good results through fine tuning. So if something goes wrong with my steps, please point it out and I'll fix it in time. 
# 
