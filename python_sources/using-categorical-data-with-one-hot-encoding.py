...
# #imputer
# imputer=Imputer()
# candidate_train_predictors = imputer.fit_transform(train_data)
# candidate_test_predictors = imputer.transform(test_data)
...
#imputer
imputer=Imputer()
candidate_train_predictors = imputer.fit_transform(final_train)
candidate_test_predictors = imputer.transform(final_test)

# model=RandomForestRegressor()
# model.fit(candidate_train_predictors, target)
# predicted_prices=model.predict(candidate_test_predictors)
my_model = XGBRegressor(n_estimators=1000, learning_rate=0.05)
my_model.fit(candidate_train_predictors, target, early_stopping_rounds=5, 
             eval_set=[(candidate_train_predictors, target)], verbose=False)
predicted_prices = my_model.predict(candidate_test_predictors)
...
mae_one_hot_encoded = get_mae(candidate_train_predictors, target)
