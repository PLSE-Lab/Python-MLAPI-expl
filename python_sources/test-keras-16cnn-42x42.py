__author__ = 'ZFTurbo: https://kaggle.com/kongpasom'
...
def get_im_cv2(path):
    img = cv2.imread(path)
    resized = cv2.resize(img, (42, 42), cv2.INTER_LINEAR)
    return resized
...
def create_model():
    model = Sequential()
    model.add(ZeroPadding2D((1, 1), input_shape=(3, 42, 42), dim_ordering='th'))
