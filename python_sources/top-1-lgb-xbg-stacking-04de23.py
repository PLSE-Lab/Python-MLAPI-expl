...
# ## A quick look at the train and test sets 
# 
...
# ensem['target']=y
ensem=pd.DataFrame()
ensem['elastic_model_full_data']=elastic_model_full_data.predict(X)
ensem['lasso_model_full_data']=lasso_model_full_data.predict(X)
ensem['ridge_model_full_data']=ridge_model_full_data.predict(X)
ensem['svr_model_full_data']=svr_model_full_data.predict(X)
ensem['gbr_model_full_data']=gbr_model_full_data.predict(X)
ensem['xgb_model_full_data']=xgb_model_full_data.predict(X)
ensem['lgb_model_full_data']=lgb_model_full_data.predict(X)
ensem['stack_model_full_data']=stack_gen_model.predict(np.array(X))
...
print('RMSLE score on stack data:')
print(rmsle(y, stacker.predict(ensem)))
...
submission.iloc[:,1] = (np.expm1(stacker.predict(stacker_data(X_sub))))
