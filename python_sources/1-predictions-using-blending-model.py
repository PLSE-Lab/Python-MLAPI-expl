...
# Before getting started Please give it an upvote if you like it.
...
def blended_predictions(X,weight):
    return ((weight[0] * ridge_model_full_data.predict(X)) +             (weight[1] * svr_model_full_data.predict(X)) +             (weight[2] * gbr_model_full_data.predict(X)) +             (weight[3] * xgb_model_full_data.predict(X)) +             (weight[4] * lgb_model_full_data.predict(X)) +             (weight[5] * stack_reg_model.predict(np.array(X))))
