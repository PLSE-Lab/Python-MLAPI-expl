...
# # This kernel is mostly a copy of Dieter's Neural Network found at https://www.kaggle.com/christofhenkel/market-data-nn-baseline

# ## Some background: I have developed a working RNN (LSTM) that scores around the same on Log Loss when training, but then when I obtain the validation metric score I was receiving only 0.39 (???).

# # I hypothesized that my model really was doing well, but in fact that the other public kernels whose validation metrics I was basing mine off of, were lying in some sense. The only thing I could think of was that I had split my train and validation differently.

# # Instead of just doing the regular old train_test_split that many others have been doing to check the validation metric, I had my train as before 2016 and validation as after 2016, to simulate that the test data is in the future. I changed it in a high-scoring public kernel to see how the validation result changes... 
...
# # Create train and validation indices like how I have mine:

# In[ ]:


train_indices = market_train.index[market_train['time'].dt.year < 2016].values
val_indices = market_train.index[market_train['time'].dt.year >= 2016].values
...
# # Recall that the original script scores like 0.65, but here it says your validation is going to be below 0.40 ... what is going on?
