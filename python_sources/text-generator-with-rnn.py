...
# Adding file path
file_path = '../input/myData.txt'

# Reading the file data, and then decoding to py2 compat.
text_data = open(file_path, 'rb').read().decode(encoding='utf-8')

# Text Length
print ('No. of Characters in the Text: {}'.format(len(text_data)))


# In[273]:


# First 300 characters in given text
print(text_data[:300])
...
size_vocabulary = len(vocab_unique)
...
model = tf.keras.Sequential([
    tf.keras.layers.Embedding(size_vocabulary, dim_embed, 
                              batch_input_shape=[BATCH_SIZE, None]),
    rnn(rnn_units,
        return_sequences=True, 
        recurrent_initializer='glorot_uniform',
        stateful=True),
    tf.keras.layers.Dense(size_vocabulary)
  ])
...
example_batch_loss  = Model_Loss(target_example_batch, example_batch_predictions)
