...
# This is the third time that I've rewritten the kernel of [Data Preparation & Exploration](https://www.kaggle.com/bertcarremans/data-preparation-exploration) made by [Bert Carrenmans](https://www.kaggle.com/bertcarremans).

# ## Introduction
# 
# This notebook aims at getting a good insight in the data for the PorteSeguro comptetition. Besides that, it gives some tips and tricks to prepare your data for modeling. The notebook consists of the following main sections. 
# 
...
# - Feature names include the postfix bin to indicate binary features and cat to indicate catgorical features. 
# - Features without theses designations are eiter continuous or ordinal. 
...
# We have 59 variables and 595212 rows. Let's see if we have the same number of variables in the test data. 
# 
...
# - the range (min to max) differs between the variables. We could apply scaling (eg. StandardSclaer), but it depends on the classifier we will want to use. 
# 
# car variables
# 
...
# 
# Overall, we can see that the range of the interval variables is rather small. Perhaps, some transformation (eg. log) is already applied in order to anonymize the data?
...
# 
# As we mentioned above, the proportion of records with target=1 is far less than target=0. This can lead to a model that has great accuracy but does have any added value in practice. Two possible strategies to dea with this problem are:
...
# Get the indices per target vlaue
...
# - ps_car_11 (ordinal) has only 5 records with mising values. Replace by the mode. 
...
# 
# EDIT: nickycan made an excellent remark on the fact that my first solution could lead to data leakage. He also pointed me to another kernel made by oliber which deals with that. I therefore replaced this part with the kernel of oliver. All credits go to him. It is so great what you can learn by participating in the Kaggle competitions :)
...
# ### Interval variables
# 
# Checking the correlations between interval variables. A heatmap is a good way to visualize the correlation between variables. The code below is based on an example by Michael Waskon.
...
# For the ordinal variables we do not see many correlations. Wecould, on the other hand, look at how the distributions are when grouping by the target value. 
...
print('{} variables hav too low variance.'.format(len(v)))
