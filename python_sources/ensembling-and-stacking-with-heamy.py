...
# 
# **Caching**: We can also select, whether caching should take place (recommend to do it at home, as kaggle kernel maybe not working).
...
# Models with sklearn-interface can be included.
...
# Models can also be defined and used directly via a function
...
# ##  Second Stage - Train LogisticRegression on stacked data
# 
# A LogisticRegression is often used as Classifier for the second stage, so let's try it out.
...
# How did stacking work in this case? 
# Let's compare the single classifiers.
# 
# For Verification - evaluation of the single models
...
# So Stacking works nice here.

# ## Predict & submit
# The prediction gives the probabilities for the different  classes. In the next step the respective class with the highest probability is selected. To  get the the final label the addition of 1 is needed.
...
# ## Alternative: Second Stage with xgboost
...
# So let's use a xgb-model as 2nd-stage model
...
# Notice the lower cross-validation score in comparison to the logistic regression stacker.
# 
