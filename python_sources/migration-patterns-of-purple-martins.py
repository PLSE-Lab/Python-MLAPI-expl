...
americas.plot(ax=ax,color='green')
birds.plot(ax=ax, color="C1")
fig.tight_layout()
...
totalArea = totalArea = (south_america.to_crs(epsg=3035)["geometry"].area / 1e6).sum()
# Check your answer
q_7.check()
...
fig, ax = plt.subplots(figsize=(8, 8))
south_america.plot(ax=ax)
end_gdf.plot(ax=ax, color="C1")
protected_areas[protected_areas["MARINE"] != "2"].plot(ax=ax, color="C2")
fig.tight_layout()
