...
testSet = pd.read_csv("../input/test.csv", dtype={"Age": np.float64}, )
...
print(scores.mean()) #=> 79.9%

## ------------- NOW FOR TEST DATA:---------------------
alg.fit(titanic[predictors], titanic["Survived"]) #train the model using training set

# clean TEST set
testSet["AgeUnknown"] = testSet["Age"].isnull()
testSet.AgeUnknown = testSet.AgeUnknown.astype(int)
testSet["Age"] = testSet["Age"].fillna(testSet["Age"].median())
testSet["Fare"] = testSet["Fare"].fillna(testSet["Fare"].median())
testSet["MrJames"] = (testSet["Name"].str.contains('Mr.', na=False)) & (testSet["Name"].str.contains('James', na=False))
testSet["MrAsian"] = (testSet["Name"].str.contains('Mr.', na=False)) & (
            (testSet["Name"].str.contains('Choong', na=False)) |
            (testSet["Name"].str.contains('Chang', na=False)) |
            (testSet["Name"].str.contains('Fang', na=False)) |
            (testSet["Name"].str.contains('Lee', na=False))
        )
testSet.MrJames = testSet.MrJames.astype(int)
testSet.MrAsian = testSet.MrAsian.astype(int)
...
testSet["Embarked"] = testSet["Embarked"].fillna("S") #S is most used port, so use it for the two null records
