...
df_train[df_train.age > 1000].age.hist(bins=10)
...
sns.boxplot(x='age', y='country_destination', data=df_train, ax =ax, showfliers=False)
ax.set_xlim([10, 70])
plt.title('Age Range by Country')
...
df_train['date_first_booking'].value_counts().plot(color = 'black')
plt.xlabel('year')
plt.ylabel('count')
plt.title('Distribution of User Activity')
df_train['date_account_created'].value_counts().plot(color = 'red')
df_train['date_first_active'].value_counts().plot(color = 'blue')
plt.legend(loc = 2)
