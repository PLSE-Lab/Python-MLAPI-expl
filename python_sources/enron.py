...
# Now, we would be building a separate dataframe with distinct columns for message, date, time etc
...
# Use .head() to show the first two row entries.
...
df['Content-Transfer-Encoding']=df['message'].apply(content_encoding)
...
# Using and implementing the above code, we have successfully generated the table shown below 
...
#  To display the five most used file extensions

# In[ ]:


df['File Name'].str.split('.').str[1].str.lower().value_counts().drop_duplicates().head(5)


# To display the most used internal(not including enron.com that is) e-mail service

# In[ ]:


df1 = pd.DataFrame(df['Receiver_Email'].str.split('@').str[1])
df1 = df1[df1.Receiver_Email != 'enron.com']
df1['Receiver_Email'].value_counts().head(1)
