...
project_name = '03-cifar10-feedforward-c6fb5'
...
for img, label in dataset:
    if dataset.classes[label] in class_dict:
        class_dict[dataset.classes[label]] += 1
    else:
        class_dict[dataset.classes[label]] = 1
class_dict
...
history += fit(epochs_u[0], lr[0], model, train_loader, val_loader)


# In[ ]:


history += fit(epochs_u[1], lr[1], model, train_loader, val_loader)


# In[ ]:


history += fit(epochs_u[2], lr[2],  model, train_loader, val_loader)


# In[ ]:


history += fit(epochs_u[3], lr[3], model, train_loader, val_loader)
...
arch = "4 layers (2048, 1024, 512, 10)"
...
test_acc = 0.54541015625
test_loss = 1.285340666770935
