...
<html>
  ...
  <body> <!--  DataCanary_s fix -->
    ...
    <script>
      ...
      zoom: 4,
      center: {lat: 40.005326, lng: -95.154777}
...
//         return String(str).replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
      ...
      type: 'poly'
       ...
       for (var i = 0; i < crashes.length; i++) {
        ...
        <script async defer
            src="https://maps.googleapis.com/maps/api/js?key=AIzaSyA0wJsknjKk5pkO2aOqsIGkSNcELPjc830&signed_in=false&callback=initMap"></script>
...
d=d[(d.longitude != 0 ) | (d.latitude != 0 )]
...
title=[]
for i in t.eventid.tolist():
  title.append("eventid:%s " % i)


desc=[]
for i in t.attacktype1_txt.tolist():
  desc.append("Attack:%s " % i)


twp=[]
for i in t.success.tolist():
  twp.append("   success:%s" % i)

timeStamp=t.iyear.tolist()
lat=t.latitude.tolist()
lng=t.longitude.tolist()
...
for i in range(0,len(lat)):
    ...
    s+="['%s', %s, %s, %s,'https://storage.googleapis.com/montco-stats/images/hand.png'],\n" % (displayTitle,lat[i],lng[i],i)
