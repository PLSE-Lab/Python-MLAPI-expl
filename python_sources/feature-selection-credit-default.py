...
# # Feature Selection | Credit Default  
#   
# **David Rivas, Ph.D.**    
#   
# # Abstract  
...
# 
# We also take a look at an example of applying PCA although we do not use this method for feature reduction.   
...
# Let's now remove the features that have zero importance (271 zero importance features out of at total 844 features - knowing that 288 of the latter's top features are necessary to keep 0.9 cumulative importance).
...
# Therefore, as pointed out in the abstract above, and in conclusion we can see that the results and accuracy with the Test "Full" Dataset (536 features) and the Test "Small" Dataset (342 features - with only 95% importance) are very similar ones.
...
# Even though we have accounted for most of the variance, that does not mean the pca decomposition makes the problem of identifying loans repaid vs not repaid any easier. Keep in mind that the differences along PC_1 are more important than that differences along PC_2. PCA does not consider the value of the label when projecting the features to a lower dimension. Feel free to try a classifier on top of this data, but when I have done so, I noticed that it was not very accurate. 
