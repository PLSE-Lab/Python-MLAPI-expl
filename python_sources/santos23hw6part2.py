...
#     sumOfSquaresb = 0
#     for i in range(1, len(instance1)):
#         sumOfSquaresab += instance1[i]*instance2[i]
#         sumOfSquaresa += instance1[i]*instance1[i]
#         sumOfSquaresb += instance2[i]*instance2[i]
#     return (1-(sumOfSquaresab/(np.sqrt(sumOfSquaresa)*np.sqrt(sumOfSquaresb))))
  
#Generalized Jaccard Distance
def distance(instance1, instance2):
    if instance1 == None or instance2 == None:
        return float("inf")
    s1= sum(min(instance1[i],instance2[i]) for i in range(1, len(instance1)))
    s2= sum(max(instance1[i],instance2[i]) for i in range(1, len(instance1)))
    sumOfSquares = 1-s1/s2
    return sumOfSquares
...
# c=[('centroid0', 3.0, 1.0, 0.1),
#  ('centroid1', 2.5, 4.5, 1.5000),
#  ('centroid2', 3.5, 1.50, 0.2000000)]
# dataset = loadCSV("IRIS_data.csv")
# showDataset2D(dataset)
# clustering = kmeans(dataset, 3, False,c)
# printTable(clustering["centroids"])

dataset = loadCSV("IRIS_data.csv")
showDataset2D(dataset)
clustering = repeatedKMeans(dataset, 3, 100)
printTable(clustering["centroids"])

# CF3=np.zeros(len(dataset))

# for k in range(3):
#     for i in range(len(dataset)):
#         for j in range(len(clustering['clusters'][k])):
#             if(dataset[i]==clustering['clusters'][k][j]):
#                 CF1[i]=k+1

# for i in range(len(dataset)):
#     for j in range(len(clustering['clusters'][2])):
#         if(dataset[i]==clustering['clusters'][2][j]):
#             CF3[i]=3

# import pandas as pd
# DF=pd.DataFrame(data=CF1, columns=['Euclidean'])
# DF['Cosine']=pd.DataFrame(data=CF2)
# DF['Jaccard']=pd.DataFrame(data=CF3)
# DF[[0,0]]

# CF00=np.zeros(len(dataset))
...
# DF['Major Vote']=pd.DataFrame(data=CF00)

# from sklearn.metrics import accuracy_score

# a_euclidean=accuracy_score(DF["Major Vote"],DF["Euclidean"])
# a_cosine=accuracy_score(DF["Major Vote"],DF["Cosine"])
# a_jaccard=accuracy_score(DF["Major Vote"],DF["Jaccard"])