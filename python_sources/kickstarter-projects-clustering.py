...
drop_columns= ['ID','name','launched','deadline','pledged','backers','usd pledged','usd_pledged_real','pledge_per_backer','goal_reached']
...
for c in df_kick.columns:
    ...
    if col_type == 'object' :
        ...
        df_kick[c] = [diction[item] for item in df_kick[c]] 
        # converting data type to 'category'
        df_kick[c] = df_kick[c].astype('category')
...
#plotting the end result on a chart with, having money raised on X-axis and number of backers on the y-axis
# %matplotlib inline
