...
label1.astype(dtype='object')


estimator=SVC(kernel='rbf', C=1,gamma='auto')
selector = GeneticSelectionCV(estimator,cv=5,verbose=1,scoring="accuracy",max_features=1200,n_population=100,crossover_proba=0.4,mutation_proba=0.3,n_generations=600,crossover_independent_proba=0.1,mutation_independent_proba=0.05,tournament_size=3,caching=True,n_jobs=-1)
selector = selector.fit(x_train,label1)
...
np.save('benchmark_label2_GA.npy',x_train_label1)


# In[ ]:


np.save('benchmark_label2_features_GA.npy',selector.support_ )
