...
print(reviews.groupby('price').points.agg(max))
check_q2(best_wine)
...
# Your code here
wine_price_extremes = pd.DataFrame({'min': reviews.groupby('variety').price.agg(min), 'max': reviews.groupby('variety').price.agg(max)})
