...
class MultilabelImageClassificationBase(nn.Module):
    ...
    def epoch_end(self, epoch, result):
        print("Epoch [{}], last_lr: {:.4f}, train_acc: {:.4f}, train_loss: {:.4f}, val_loss: {:.4f}, val_score: {:.4f}".format(
            epoch, result['lrs'][-1], result['train_acc'], result['train_loss'], result['val_loss'], result['val_score']))
...
model.load_state_dict(torch.load('./proteinmodel.pth'))
