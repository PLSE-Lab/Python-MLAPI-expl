...
# ![1](https://images.theconversation.com/files/117015/original/image-20160331-28462-qliwnl.jpg?ixlib=rb-1.1.0&q=45&auto=format&w=926&fit=clip)
# 
...
# ### Impact of bmi over old and young patients?
...
# ### How do ICU-types and the days in hospital before the patients' ICU admissions look like?
...
# ### What's the relationship between apache bodyststem categorization and hospital death?
# let's choose one bodyststem category from the dropdown list.
...
# Such as BMI (KG/m^2)= Weight / (Height * Height)
...
# We will also fill in some 'xxx_apache' columns as those were the worst value within 24 hours of the ICU admission
...
# We will also add one feature: **Hco3/PCo2** is a metric that can be used in some ICU  
# And fill out another feature with estiamtion: **Mean Arterial Pressure**
...
# Sum up the three component of GCS score as **'gcs_sum'**
...
# Since **'Invasive/Non-invasive'** columns are redundant and contain less information, we can drop them
...
# The dataset contains a dictionary where we get to know various kinds of features, we create a set of **"null_sum"** variables to describe the **completeness of each patient's data**
...
# We also want to capture how the patient's records change during the first hour and first day.  
# **Difference varialbes** were created to describe the difference beween maximum and minimum value.
...
# **Revised Trauma Score** is another score that is used to describe the severity of patient condition.  
...
# # Imputation
# We still have a lot of missing value, so we will use lightgbm regressor to impute two important features: **apache_4a_icu_death_prob and apache_4a_hospital_death_prob**
...
# # Lightgbm Model
# Finally, we get to build our model!
...
# ** Here is a base model without parameter tuning..**
...
get_ipython().run_cell_magic('time', '', "lgbm_clf = lgbm.LGBMClassifier(boosting_type='dart',n_estimators=1000,random_state = 23,categorical_feature=category,metric='auc')\nlgbm_clf.fit(X_train, y_train)\ny_pred = lgbm_clf.predict(X_test)\ny_score = lgbm_clf.predict_proba(X_test)[:,1]")
...
# I have skipped the hyperparameter tuning process as it could be found in many kernels. Let's look at the result!
...
# I hope you enjoy this notebook. This is my first Kaggle competition. I'm grateful for all those discussions and kernels posted. Hope this may inspire you a bit as well! Happy Learning!
