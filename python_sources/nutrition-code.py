...
def load(refresh=False,source=''):
    global tables, table_filenames, isloaded, ROOT_DIR
    if source=='':
        source = ROOT_DIR
    if (not isloaded) or (isloaded and refresh):
        for tblName in table_filenames:
            tables[tblName] = pd.read_csv(source + table_filenames[tblName])
