...
df_pro_desc = pd.read_csv('../input/product_descriptions.csv')[:64000] #update here
...
rfr = RandomForestRegressor(n_estimators = 820,max_features=10, max_depth=20, n_jobs = -1, random_state = 1301, verbose = 1)
...
pd.DataFrame({"id": id_test, "relevance": clf.predict(X_test)}).to_csv('submission.csv',index=False)
