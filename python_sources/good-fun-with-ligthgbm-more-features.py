...
#        df[f_], _ = pd.factorize(df[f_])
    ...
    df_ret = df[[f for f in df.columns if f not in cat_features]].groupby(groupby_ids).mean().join(df_pivots, how='left')
...
oof_preds, sub_preds, feature_importance_dfs = OOFPreds(X=data[features], y=y, test_X=test[features], params=params_LGBM, n_splits=5, random_state=42)
...
sns.barplot(x="importance", y="feature", data=best_features.sort_values(by="importance", ascending=False).reset_index())
