...
# Create X
features = ['YearRemodAdd','OverallQual','OverallCond','WoodDeckSF','OpenPorchSF','Fireplaces','MSSubClass','BsmtFinSF1','BsmtFinSF2','LotArea', 'YearBuilt', '1stFlrSF', '2ndFlrSF','LowQualFinSF', 'FullBath','GrLivArea', 'BedroomAbvGr','KitchenAbvGr', 'TotRmsAbvGrd']
cols_with_missing = [col for col in features 
                                 if home_data[col].isnull().any()]
for col in cols_with_missing:
    home_data[col].fillna(0, inplace=True)
...
rf_model_on_full_data = GradientBoostingRegressor(max_features=9,n_estimators=70,max_depth=5,min_samples_split=2,learning_rate=0.1,loss='ls')
