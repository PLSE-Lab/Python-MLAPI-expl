...
def get_indicators_and_clean_comments(df):
    ...
    df["ant_slash_n"] = df["BODY"].apply(lambda x: count_regexp_occ(r"\n", x))

    # Get length in words and characters

    df["raw_word_len"] = df["BODY"].apply(lambda x: len(x.split()))

    df["raw_char_len"] = df["BODY"].apply(lambda x: len(x))

    # Check number of upper case, if you're angry you may write in upper case

    df["nb_upper"] = df["BODY"].apply(lambda x: count_regexp_occ(r"[A-Z]", x))

    # Number of F words - f..k contains folk, fork,

    df["nb_fk"] = df["BODY"].apply(lambda x: count_regexp_occ(r"[Ff]\S{2}[Kk]", x))

    # Number of S word

    df["nb_sk"] = df["BODY"].apply(lambda x: count_regexp_occ(r"[Ss]\S{2}[Kk]", x))

    # Number of D words

    df["nb_dk"] = df["BODY"].apply(lambda x: count_regexp_occ(r"[dD]ick", x))

    # Number of occurence of You, insulting someone usually needs someone called : you

    df["nb_you"] = df["BODY"].apply(lambda x: count_regexp_occ(r"\W[Yy]ou\W", x))

    # Just to check you really refered to my mother ;-)

    df["nb_mother"] = df["BODY"].apply(lambda x: count_regexp_occ(r"\Wmother\W", x))

    # Just checking for toxic 19th century vocabulary

    df["nb_ng"] = df["BODY"].apply(lambda x: count_regexp_occ(r"\Wnigger\W", x))

    # Some Sentences start with a <:> so it may help

    df["start_with_columns"] = df["BODY"].apply(lambda x: count_regexp_occ(r"^\:+", x))

    # Check for time stamp

    df["has_timestamp"] = df["BODY"].apply(lambda x: count_regexp_occ(r"\d{2}|:\d{2}", x))

    # Check for dates 18:44, 8 December 2010

    df["has_date_long"] = df["BODY"].apply(lambda x: count_regexp_occ(r"\D\d{2}:\d{2}, \d{1,2} \w+ \d{4}", x))

    # Check for date short 8 December 2010

    df["has_date_short"] = df["BODY"].apply(lambda x: count_regexp_occ(r"\D\d{1,2} \w+ \d{4}", x))

    # Check for http links

    df["has_http"] = df["BODY"].apply(lambda x: count_regexp_occ(r"http[s]{0,1}://\S+", x))

    # check for mail

    df["has_mail"] = df["BODY"].apply(
    ...
    df["has_emphasize_equal"] = df["BODY"].apply(lambda x: count_regexp_occ(r"\={2}.+\={2}", x))

    df["has_emphasize_quotes"] = df["BODY"].apply(lambda x: count_regexp_occ(r"\"{4}\S+\"{4}", x))



    # Now clean comments

    df["clean_comment"] = df["BODY"].apply(lambda x: prepare_for_char_n_gram(x))
...
if __name__ == '__main__':
    ...
    with timer("Creating numerical features"):
        ...
        num_features = [f_ for f_ in train.columns
                        ...
                        if f_ not in ["BODY", "clean_comment", "remaining_chars", 'X', 'has_ip_address'] + class_names]
    ...
    clf.fit(train_features, train['REMOVED'])

    # Compute prediction and use sigmoid

    preds = sigmoid(clf.predict(test_features))

    score = roc_auc_score(test["REMOVED"], preds)
