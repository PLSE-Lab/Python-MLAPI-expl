...
pd.read_excel('/kaggle/input/telecom-churn-data-directory/DataDictionary-TelecomChurnCaseStudy(1).xlsx')
...
get_null_percentage(telecom_data)
...
# ##### Convert DateTime to numeric for model to consume:
...
# #### Remove columns with suffic _9 as those are once which decided to be a chrun customer. 
...
# #### Remove rows with more than 80% null values:
...
#     * Consider the quartiles of 0, 10, 50 for lower limit
...
# ##### Note: By observing the data we can say that it's better to do outlier treatment with 99% and 0.5% QUARTILES
...
# #### Features list with variance explanation:
...
# ### Sampling: 
#     * Use different sampling techniques to solve class imbalance problem.
