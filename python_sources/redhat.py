...
#minor edits - trying to play with xgboost
...
############################# Added #################################
def modelfit(alg, dtrain, predictors, target, useTrainCV=True, cv_folds=10, early_stopping_rounds=50):
    
    if useTrainCV:
        xgb_param = alg.get_xgb_params()
        xgtrain = xgb.DMatrix(dtrain[predictors].values, label=dtrain[target].values)
        cvresult = xgb.cv(xgb_param, xgtrain, num_boost_round=alg.get_params()['n_estimators'], nfold=cv_folds,
            metrics='auc', early_stopping_rounds=early_stopping_rounds, show_progress=False, verbose_eval=True)
...
xgb1 = XGBClassifier(
 ...
 seed=270)
#modelfit(xgb1, train, test, features, 'outcome')
