...
def run_single(train, test, features, target, random_state=1):
    ...
    gbm2 = xgb.train(params2, dtrain, num_boost_round, evals=watchlist,
                    early_stopping_rounds=early_stopping_rounds, verbose_eval=True)                    

    print("Validating...")
    check = gbm.predict(xgb.DMatrix(X_valid), ntree_limit=gbm.best_iteration+1)
    check2 = gbm.predict(xgb.DMatrix(X_valid), ntree_limit=gbm.best_iteration+1)
    check=check+check2
    ...
    test_prediction=test_prediction+test_prediction2
...
def run_kfold(nfolds, train, test, features, target, random_state=6102):
    eta = 0.205
    max_depth = 3
    subsample = 0.67
    ...
    for train_seq_index, test_seq_index in kf:
        ...
        gbm2 = xgb.train(params2, dtrain, num_boost_round, evals=watchlist, early_stopping_rounds=early_stopping_rounds, verbose_eval=1000)
