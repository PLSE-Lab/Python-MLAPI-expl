...
# #  Inception Model for Fashion MNIST
# 
...
# 
# This part is pretty standard but there are a few things to note. The pixel values are scaled to the range $[0, 1]$. The validation set is selected with a fixed seed so as to be reproducible. I use a validation set that is balanced across classes.

# In[ ]:


# load data
rawdata = np.loadtxt('../input/fashion-mnist_train.csv', dtype=int, delimiter=',', skiprows=1)
...
# ### Label Smoothing
# This is a regularization method from section *7. Model Regularization via Label Smoothing* in ["Rethinking the Inception Architecture for Computer Vision"](https://arxiv.org/abs/1512.00567) (Szegedy et al, 2015). 
...
# ### Train
# 
# I save the best model for final evaluation and use 100 epochs so cosine annealing can thoroughly explore the solution space.
...
# load test set data
test_data = np.loadtxt('../input/fashion-mnist_test.csv', dtype=int, delimiter=',', skiprows=1)
