...
for i in np.unique(sub_table['layer_id']):
    ax.scatter( sub_table[sub_table['layer_id']==i]['x'],                sub_table[sub_table['layer_id']==i]['y'],                sub_table[sub_table['layer_id']==i]['z'], 'o', edgecolor='k', label='Layer id: ' + str(i))
...
for i, layer_id_ in enumerate(np.unique(sub_table['layer_id'])):
    ...
    ax.scatter( sub_table[sub_table['layer_id']==layer_id_]['x'],                sub_table[sub_table['layer_id']==layer_id_]['y'],                sub_table[sub_table['layer_id']==layer_id_]['z'], 'o', edgecolor='k', label='Layer id: ' + str(layer_id_))
    ax.set_xlim([-200, 200])
    ax.set_ylim([-200, 200])
...
for i, layer_id_ in enumerate(layer_ids):
    ax.scatter(sub_table[sub_table['layer_id']==layer_id_]['x'],               sub_table[sub_table['layer_id']==layer_id_]['layer_id'],                sub_table[sub_table['layer_id']==layer_id_]['z'], 'o', edgecolor='k', label='Layer id: ' + str(i))
ax.set_xlabel('x')
ax.set_ylabel('y')
ax.set_zlabel('z')
ax.view_init(elev=45)
