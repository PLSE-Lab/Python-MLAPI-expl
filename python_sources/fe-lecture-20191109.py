...
def crps(y_true, y_pred):
    loss = tf.reduce_mean((tf.cumsum(y_pred, axis = 1) - y_true)**2)
    return loss
...
def get_model(batch_size = 32, epochs = 10):
     ...
     ## inputs
    input_dense_game = tf.keras.layers.Input(shape=(train_dense_game.shape[1],), name = "numerical_general_inputs")
    input_dense_players = tf.keras.layers.Input(shape=(train_dense_players.shape[1],train_dense_players.shape[2]), name = "numerical_players_inputs")
    input_cat_game = tf.keras.layers.Input(shape=(train_cat_game.shape[1], ), name = "categorical_general_inputs")
    input_cat_players = tf.keras.layers.Input(shape=(train_cat_players.shape[1], train_cat_players.shape[2]), name = "categorical_players_input")
    
     ## embedding
    embedding = tf.keras.layers.Embedding(num_classes, 4)
    emb_cat_game = embedding(input_cat_game)
    emb_cat_game = tf.keras.layers.Flatten()(emb_cat_game)
    emb_cat_players = embedding(input_cat_players)
    emb_cat_players = tf.keras.layers.Reshape((int(emb_cat_players.shape[1]), int(emb_cat_players.shape[2]) * int(emb_cat_players.shape[3])))(emb_cat_players)
    
    game = tf.keras.layers.Concatenate(name = "general_features")([input_dense_game, emb_cat_game])
    game = tf.keras.layers.Dense(32, activation="relu")(game)
    game = tf.keras.layers.Dropout(0.5)(game)
    
    players = tf.keras.layers.Concatenate(name = "players_features")([input_dense_players, emb_cat_players])
    n_unit = 16
    players_aves = []
    for k in range(3):
        players = tf.keras.layers.Dense(16, activation=None)(players)
        players_aves.append(tf.keras.layers.GlobalAveragePooling1D()(players))
        players = tf.keras.layers.Activation("relu")(players)
    players = tf.keras.layers.Concatenate(name = "deep_players_features")(players_aves)
    players = tf.keras.layers.Dropout(0.5)(players)
    
    
    out_soft = tf.keras.layers.Dense(199, activation="softmax", name = "out_soft")(game)
    out_reg = tf.keras.layers.Dense(1, activation=None, name = "out_reg")(players)
    
    model = tf.keras.models.Model(inputs = [input_dense_game, input_dense_players, input_cat_game, input_cat_players],
                               outputs = [out_soft, out_reg])
    
    ## compile
    model.compile(loss=[crps, tf.keras.losses.mae],
                  loss_weights=[1.0, 0.01],
                  optimizer=tf.keras.optimizers.Adam(learning_rate=0.002, decay = 1e-4))
...
def get_model(batch_size = 32, epochs = 10):
    ...
    x1 = tf.keras.layers.Dense(128, activation='relu')(input_dense_game)
    
    out_soft = tf.keras.layers.Dense(199, activation="softmax", name = "out_soft")(x1)
    out_reg = tf.keras.layers.Dense(1, activation=None, name = "out_reg")(input_cat_game)
