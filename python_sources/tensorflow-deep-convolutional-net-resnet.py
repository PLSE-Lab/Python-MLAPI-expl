...
def build_model(X, label_cnt, weight_initializer=None, fc_weight_initializer=None):
    ...
    with tf.name_scope('conv1'):
        ...
        tf.histogram_summary('conv1_maxpooling', conv_pool)
    ...
    with tf.name_scope('avg_pool'):
        ...
        tf.histogram_summary('conv1_avgpooling', conv_avg_pool)
...
with tf.name_scope('output'):
    with tf.name_scope('cross_entropy'):
        classification = tf.nn.softmax_cross_entropy_with_logits(output, Y)
        with tf.name_scope('total'):
            cost = tf.reduce_mean(classification)
        tf.scalar_summary('output/cross_entropy', cost)
...
test_result_file = open('result.csv', 'w')
