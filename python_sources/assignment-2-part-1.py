...
tf.reset_default_graph() # will clean the graph we built before
Num_Classes = 10
...
conv1_w = tf.get_variable("conv1_w", [3,3,3,32], initializer= tf.random_normal_initializer(stddev=1e-2))
conv1_b = tf.get_variable("conv1_b", [32], initializer= tf.random_normal_initializer(stddev=1e-2))
...
conv2_w = tf.get_variable("conv2_w", [3,3,32,32], initializer= tf.random_normal_initializer(stddev=1e-2))
conv2_b = tf.get_variable("conv2_b", [32], initializer= tf.random_normal_initializer(stddev=1e-2))
...
conv3_w = tf.get_variable("conv3_w", [3,3,32,32], initializer= tf.random_normal_initializer(stddev=1e-2))
conv3_b = tf.get_variable("conv3_b", [32], initializer= tf.random_normal_initializer(stddev=1e-2))
conv3 = tf.nn.conv2d(pool2, conv3_w, strides = [1,1,1,1], padding= 'SAME')+conv3_b
relu3= tf.nn.relu(conv3)
print(relu3)

"""flatten layer"""
flatten = tf.reshape(relu3, [-1, 8*8*32])
