...
image_list_name = os.listdir(dir_Flickr_jpg)
print("The number of jpg flies in Flicker30k: {}".format(len(image_list_name)))
print(image_list_name[0:5])
...
for i in range(11000):
    loc = random.randrange(0,31000)
    if loc not in loc_list:
        loc_list.append(loc)
        jpgs.append(image_list_name[loc])
...
for i in df_txt.index.values:
    if df_txt['filename'][i] in jpgs:
        df_txt['train'][i] = 1

df_txt = df_txt[df_txt['train'] == 1]
df_txt = df_txt[["filename","index","caption"]]
...
df_txt.to_csv('edit_caption.csv')


# In[ ]:


#df_txt = pd.read_csv('edit_caption.csv')
...
modelvgg = VGG16(include_top=True,weights="../input/vgg16/vgg16_weights_tf_dim_ordering_tf_kernels.h5")
modelvgg.summary()
...
keras.utils.plot_model(modelvgg, to_file='modelVGG16.png')
...
keras.utils.plot_model(modelvgg, to_file='modelVGG16Modify.png')
...
keras.utils.plot_model(model, to_file='decoder.png')
...
model.save_weights("model_weights_10e.h5")
...
model.save_weights("model_weights_15e.h5")
...
model.save_weights("model_weights_15e2.h5")
