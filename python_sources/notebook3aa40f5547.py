...
x_test = []
for f, tags in tqdm(df_test.values[20000:30000], miniters=1000):
    img = cv2.imread('../input/test-tif-v3/{}.tif'.format(f), -1)
...
nadam = k.optimizers.Nadam(lr=0.0001, beta_1=0.9, beta_2=0.999, epsilon=1e-08, schedule_decay=0.0004)
...
for i in range(0, length, batchsize):
    x_test = []
    for f, tags in tqdm(df_test.values[i:(i + batchsize - mul * (batchsize - length % batchsize))], miniters=1000):
        img = cv2.imread('../input/test-tif-v3/{}.tif'.format(f), -1)[:,:,:3]
        x_test.append(cv2.resize(img, (32, 32)))
    x_test = np.array(x_test, np.float16) / 255.
    p_test = model.predict(x_test, batch_size = 128, verbose=0)
    get_ipython().run_line_magic('xdel', 'x_test')
    if test_predict_df is None:
        test_predict_df = pd.DataFrame(p_test)
    else:
        test_predict_df =test_predict_df.append(pd.DataFrame(p_test))    
...
for col in test_predict_df.columns:
    new_column = 'decoded_{}'.format(col)
...
preds = []
for i in tqdm(range(test_predict_df.shape[0]), miniters=1000):
    a = test_predict_df.ix[[i]]
    a = a.apply(lambda x: x > 0.5, axis=1)
    a = a.loc[a[i] == True]
    ' '.join(list(a.index))
