...
model.fit(X, y, epochs = 100, batch_size=1024, verbose = 5)
...
test_out.to_csv('meta_nn_test.csv')


train_pred = model.predict_proba(train[data_columns].values)
#Normalize the predictions
pred_sum = train_pred.sum(axis=1)
train_pred = train_pred / pred_sum[:, None]
train_out = pd.DataFrame(train_pred, columns = ['high', 'medium', 'low'], index=train.index)
test_out.to_csv('meta_nn_train.csv')
