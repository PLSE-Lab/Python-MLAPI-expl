...
lbl = LabelEncoder()
lbl.fit(list(structures['atom'].values))
structures['atom'] = lbl.transform(structures['atom'])
structures.set_index('molecule_name',inplace = True)
train_id = edge_index_train['molecule_name'].unique()
test_id = edge_index_test['molecule_name'].unique()

n_f_train = structures.loc[list(train_id),:].drop('atom_index',axis = 1)
n_f_test = structures.loc[list(test_id),:].drop('atom_index',axis = 1)
n_f_train.to_csv("n_f_train.csv",index = True)
...
X_test['molecule_name'] = edge_index_test['molecule_name']

X.set_index('molecule_name').to_csv("edge_features_train.csv",index = True)
X_test.set_index('molecule_name').to_csv("edge_features_test.csv", index = True)     

edge_index_train.set_index('molecule_name').to_csv("edge_index_train.csv", index = True)
