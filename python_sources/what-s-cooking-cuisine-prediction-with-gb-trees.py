...
for i in range(0, len(predictions)):
    cuisinePredictions.append(cuisines[np.int32(predictions[i])]) # cuisines[list.index(list(predictions[i]), max(predictions[i]))]
...
submission.to_csv('cuisine-prediction-submission-4.csv', index=False)
