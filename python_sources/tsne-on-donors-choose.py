...
from collections import Counter
my_counter = Counter()
#print(project_data["clean_subcategories"].value_counts)
...
sns.FacetGrid(project_data, hue="project_is_approved",height =10).map(sns.distplot, "teacher_number_of_previously_posted_projects").add_legend()


# conclusion:
#     No specific conclusion can be derived because both the plots almost overlap.

# In[ ]:


sns.boxplot(x='project_is_approved',y='teacher_number_of_previously_posted_projects', data=project_data ,palette=['g','r'])
plt.show()
...
#print(approved_word_count_summary[:20])
...
# making a binary list which will contain 1 if there is a number in the summary and 0 if there isn't 
project_data[['project_resource_summary','project_is_approved' ]]
list_of_project_resource_summary = project_data['project_resource_summary']
binary_list_of_project_resource_summary_has_numbers_or_not=[]
...
for summary in list_of_project_resource_summary:
    result= hasNumbers(summary)
    binary_list_of_project_resource_summary_has_numbers_or_not.append(result)
...
# adding the list to the dataframe project_data
project_data["numbers_in_summary_or_not"]= binary_list_of_project_resource_summary_has_numbers_or_not


# In[ ]:


project_data[["numbers_in_summary_or_not","project_is_approved"]]
list_numbers_in_summary_or_not =list(project_data['numbers_in_summary_or_not'])
list_project_is_approved =list(project_data['project_is_approved'])
count =0
for i in list_numbers_in_summary_or_not:
    if(list_numbers_in_summary_or_not[i]==list_project_is_approved[i]):
        count=count+1
#print(count)
#print(len(list_numbers_in_summary_or_not))
#print("percentage of equality in the lists is",(count/len(list_numbers_in_summary_or_not))*100,"%")


# a series to use for counting- formatted as cat1_cat2
ts = (project_data['numbers_in_summary_or_not'].astype(str) + '_' + project_data['project_is_approved'].astype(str))
ts.value_counts().plot(kind = 'bar')


# Conclusion:
#     1. The blue plot concludes that there are many such summaries where no numbers are present and yet the project is approved.
#     2. There is no specific relationship between if there a number is present in the summary or not with its approval.
# 
...
# <h3><font color='red'>1.3.2 Project teacher prefix</font></h3>
...
#print(project_data["clean_categories"].value_counts())
print("Shape of matrix after one hot encodig ",categories_one_hot.shape)
#print(categories_one_hot)
#print(categories_one_hot[:10].todense())
...
# Please do the similar feature encoding with project_grade_category 
...
#approved_word_count_summary# We are considering only the words which appeared in at least 10 documents(rows or projects).
...
title_bow_5000=title_bow[:5000]#project_title_bow
state_one_hot_5000=state_one_hot[:5000]
...
project_grade_category_one_hot_5000=project_grade_category_one_hot[:5000]
...
X = hstack((title_bow_5000,
            state_one_hot_5000,
            categories_one_hot_5000,
            sub_categories_one_hot_5000,
            teacher_prefix_one_hot_5000,
            project_grade_category_one_hot_5000,
            price_standardized_5000,
            teacher_number_of_previously_posted_projects_standardized_5000))
X.shape
...
y= np.asarray(project_data["project_is_approved"])
np.vstack(y)
...
y= np.asarray(project_data["project_is_approved"][:5000])
Y = np.vstack(y)

tsne = TSNE(n_components=2, perplexity=30, learning_rate=200 )


X_embedding = tsne.fit_transform(X.todense())
X_embedding
...
colors = {0:'red', 1:'blue'}
plt.title('t-SNE for categorical, numerical features + project_title(BOW) ')
...
title_tfidf_5000=title_tfidf[:5000]#project_title_tfidf
state_one_hot_5000=state_one_hot[:5000]
categories_one_hot_5000=categories_one_hot[:5000]
sub_categories_one_hot_5000=sub_categories_one_hot[:5000]
teacher_prefix_one_hot_5000=teacher_prefix_one_hot[:5000]
...
for_tsne_df = pd.DataFrame(data=for_tsne, columns=['Dimension_x','Dimension_y','Score'])
colors = {0:'red', 1:'blue'}
plt.title('t-SNE for categorical, numerical features + title_tfidf ')
plt.xlabel('Dimension 1')
plt.ylabel('Dimension 2')
plt.scatter(for_tsne_df['Dimension_x'], for_tsne_df['Dimension_y'], c=for_tsne_df['Score'].apply(lambda x: colors[x]))
plt.show()
...
avg_w2v_vectors_5000=avg_w2v_vectors[:5000]#project_avg_w2v_vectors
state_one_hot_5000=state_one_hot[:5000]
categories_one_hot_5000=categories_one_hot[:5000]
sub_categories_one_hot_5000=sub_categories_one_hot[:5000]
teacher_prefix_one_hot_5000=teacher_prefix_one_hot[:5000]
...
for_tsne_df = pd.DataFrame(data=for_tsne, columns=['Dimension_x','Dimension_y','Score'])
colors = {0:'red', 1:'blue'}
plt.title('t-SNE for categorical, numerical features + avg_w2v_vectors ')
plt.xlabel('Dimension 1')
plt.ylabel('Dimension 2')
plt.scatter(for_tsne_df['Dimension_x'], for_tsne_df['Dimension_y'], c=for_tsne_df['Score'].apply(lambda x: colors[x]))
plt.show()
...
tfidf_w2v_vectors_5000=tfidf_w2v_vectors[:5000]#project_tfidf_w2v_vectors
state_one_hot_5000=state_one_hot[:5000]
categories_one_hot_5000=categories_one_hot[:5000]
sub_categories_one_hot_5000=sub_categories_one_hot[:5000]
teacher_prefix_one_hot_5000=teacher_prefix_one_hot[:5000]
...
X = hstack((tfidf_w2v_vectors_5000,
            state_one_hot_5000,
            categories_one_hot_5000,
            sub_categories_one_hot_5000,
            teacher_prefix_one_hot_5000,
...
for_tsne_df = pd.DataFrame(data=for_tsne, columns=['Dimension_x','Dimension_y','Score'])
colors = {0:'red', 1:'blue'}
plt.title('t-SNE for categorical, numerical features + tfidf_w2v_vectors ')
plt.xlabel('Dimension 1')
plt.ylabel('Dimension 2')
plt.scatter(for_tsne_df['Dimension_x'], for_tsne_df['Dimension_y'], c=for_tsne_df['Score'].apply(lambda x: colors[x]))
plt.show()
...
#combining all the features and plotting tsne
...
for_tsne_df = pd.DataFrame(data=for_tsne, columns=['Dimension_x','Dimension_y','Score'])
colors = {0:'red', 1:'blue'}
plt.title('t-SNE by concatinating all the featuers  ')
plt.xlabel('Dimension 1')
plt.ylabel('Dimension 2')
plt.scatter(for_tsne_df['Dimension_x'], for_tsne_df['Dimension_y'], c=for_tsne_df['Score'].apply(lambda x: colors[x]))
plt.show()
...
# 1. Summary for plot 1: 't-SNE for categorical, numerical features + project_title(BOW)
#     1.1 Cluster formation can be seen, but no seperation can be seen based on our classification task 
# 2. Summary for plot 2: 't-SNE for categorical, numerical features + project_title(tfidf)
#     2.1 Clustering is much better as compared to BOW vectors but still no seperation can be seen based on our classification task
# 3. Summary for plot 3: 't-SNE for categorical, numerical features + avg_w2v_vectors
#    3.1 Clustering is better than  plot 1 but still no seperation can be seen 
#   
