...
group = pd.read_csv("/kaggle/input/dataset/game_overview.csv")
...
# group["tags"] = group["tags"].map(listStr)

# df["tags"]= "movie"
# test["tags"] = "movie"
# for i in range(group.shape[0]):
#     df["tags"][df["title"]==group["title"][i]] = group["tags"][i]
#     test["tags"][test["title"]==group["title"][i]] = group["tags"][i]

# df["user_review"] =   df["tags"]+ ' '+ df["user_review"]
# test["user_review"] =  test["tags"]+ ' '+ test["user_review"] 
...
test['Cleansed'] = test['user_review'].apply(cleansing)
...
test['user_suggestion'] = [round(i[0]) for i in pred.tolist()]
test[['review_id','user_suggestion']].to_csv('BERT.csv',header=True,index=False)
