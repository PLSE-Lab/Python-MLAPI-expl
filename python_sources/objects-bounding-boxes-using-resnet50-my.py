...
# <br>
# 
# This is is starter kernel, mainly for learning purposes and getting started. There is so much to learn more and improve. I am using imageai package along with resnet50 pretrained model.
# 
# For the baseline model, one can refer to a great kernel shared by Miha Skalic: [Classifier as a bounding-box predictor](https://www.kaggle.com/mihaskalic/classifier-as-a-bounding-box-predictor)
# 
...
get_ipython().system('cp -r ../input/imageai2/imageai-master/ImageAI-master/imageai/ imageai')
...
for i in range(n_rounds):
    ...
    for  j , prediction in enumerate(batch):
        ...
        detections = detector.detectCustomObjectsFromImage(input_image=image_path+image, output_image_path='image_with_box.png', minimum_percentage_probability=75)
        pred_str = ''
        labels = ''
        for eachObjct in detections:
            if eachObjct['name'] in rev:
                pred_str += rev[eachObjct['name']] +", "+ str(float(eachObjct['percentage_probability'])  /100) + "0.1 0.1 0.9 0.9"
                pred_str += " "
                labels += eachObjct['name'] + ", " + str(round(float(eachObjct['percentage_probability'])/100, 1)) 
                labels += " | "
                if labels != "":
