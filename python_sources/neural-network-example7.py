...
class NN:
    ...
    #See http://keras.io/ for parameter options--trying parameter changes #sigmoid
    def __init__(self, inputShape, layers, dropout = [], activation = 'relu', init = 'normal', loss = 'rmse', optimizer = 'adadelta', nb_epochs = 50, batch_size = 32, verbose = 1):
...
def make_dataset(useDummies = True, fillNANStrategy = "mean", useNormalization = True):
    ...
    X_train, X_test, y_train, y_test = train_test_split(train, labels, test_size=0.05,random_state=42)
    
    return X_train, X_test, y_train, y_test, test

print ("Creating dataset...") 
X_train, X_test, y_train, y_test, test = make_dataset(useDummies = True, fillNANStrategy = "mean", useNormalization = True)
    
clf = NN(inputShape = X_train.shape[1], layers = [128, 64], dropout = [0.05, 0.05], loss='mae', optimizer = 'adadelta', init = 'glorot_normal', nb_epochs = 5)

print ("Training model...")
clf.fit(X_train, y_train)

print ("Making predictions on CV...")
pred = clf.predict(X_test)
predClipped = np.clip(np.round(pred), 1, 8).astype(int)
score = accuracy_score(y_test, predClipped)
print ("Testing acc score is: " + str(score))
...
#[0.5, 0.5] 0.215488215488

#[0.5, 0.0] 0.388552188552
#[0.5, 0.25] 0.185521885522
#[0.5, 0.75] 0.185521885522
# [0.5,0.1] 0.289898989899

# [0.0, 0.5] 0.225925925926
#[0.25, 0.5] 0.215488215488
#[0.75, 0.5] 0.193939393939
# [1.0, 0.5] 0.220538720539
