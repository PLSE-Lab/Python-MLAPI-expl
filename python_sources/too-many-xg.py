...
model = xgb.train(plst, xgtrain, num_rounds, watchlist, early_stopping_rounds=4)
preds9 = model.predict(xgtest)

model = xgb.train(plst, xgtrain, num_rounds, watchlist, early_stopping_rounds=4)
preds10 = model.predict(xgtest)

#combine predictions
#since the metric only cares about relative rank we don't need to average
preds = preds1 + preds2 + preds3 + preds4 + preds5 + preds6 + preds7 + preds8 + preds9 + preds10

#generate solution
preds = pd.DataFrame({"Id": test_ind, "Hazard": preds})
preds = preds.set_index('Id')
preds.to_csv('toomany_xgboost.csv')