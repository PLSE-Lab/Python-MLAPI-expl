...
# I'm going to feed a better initial tour found by [the one of my previous kernels](https://www.kaggle.com/kostyaatarik/better-input-for-aka-2-opt) to [another one](https://www.kaggle.com/kostyaatarik/not-a-3-and-3-halves-opt).
...
for city_id in tqdm(cities.index):
    dists, neibs = kdt.query([XY[city_id]], 8)
    for triplet in combinations(neibs[0], 3):
        if all(triplet):
            triplets.add(tuple(sorted(triplet)))
    neibs = kdt.query_radius([XY[city_id]], 11, count_only=False, return_distance=False)
    for triplet in combinations(neibs[0], 3):
        if all(triplet):
            triplets.add(tuple(sorted(triplet)))
...
# 5. Load the initial path to start optimization from. I'll use the one generated by [my previous kernel](https://www.kaggle.com/kostyaatarik/better-input-for-aka-2-opt).

# In[ ]:


path = np.array(pd.read_csv('../input/better-input-for-aka-2-opt/1515905.0538262606.csv').Path)
