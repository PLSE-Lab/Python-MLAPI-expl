...
get_ipython().system('pip install --user git+https://github.com/jhfjhfj1/autokeras.git')
...
df=pd.read_csv("../input/fashion-small/fashion_small/fashion_small/styles.csv",error_bad_lines =False, warn_bad_lines=False)
df.info()
df.head()
...
df.groupby(["year","season"]).size()


# In[ ]:


df.groupby("masterCategory").size()


# In[ ]:


articleType=df.groupby("articleType").size().sort_values(ascending=False)
articleType.name="articleType n"
articleType=articleType[articleType>300].reset_index(drop=False)
articleType.index.name="ncat"
articleType=articleType.reset_index(drop=False)
articleType
...
get_ipython().system('ls ../input -Rlh')


# In[ ]:


categories=pd.merge(df,articleType)[["id","ncat"]]

categories.ncat.plot.hist()
categories["path"]=categories.id.apply(lambda i:f"../input/fashion-small/fashion_small/fashion_small/resized_images/{i}.jpg")
categories['File Name'] =categories.id.apply(lambda i:f"{i}.jpg")
categories['Label']=categories.ncat
...
categories["exists"]= categories["path"].apply(os.path.isfile)

categories=categories[categories.exists].sample(frac=1)
...
categories[['File Name','Label']].to_csv("categories.csv" ,index=False)
lval=int( len(categories)*0.2)+1
train=categories.iloc[:-lval]
val=categories.iloc[-lval:]
train[['File Name','Label']].to_csv("train.csv" ,index=False)
val[['File Name','Label']].to_csv("val.csv" ,index=False)


# In[ ]:


basepath="../input/fashion-small/fashion_small/fashion_small/resized_images/"
...
x_test, y_test =ak.image.image_supervised.load_image_dataset("val.csv",basepath)
x_test.shape
...
x_train, y_train=ak.image.image_supervised.load_image_dataset("train.csv",basepath)
