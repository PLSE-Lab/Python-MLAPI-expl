...
clf2.fit(train, label_log)
et_pred = clf2.predict(test)

for x in range(1, 4):
    clf2 = ensemble.ExtraTreesRegressor(n_jobs=-1, n_estimators=100, random_state=x)
    clf2.fit(train, label_log)
    et_pred = et_pred + clf2.predict(test)
...
clf2.fit(train, label_log)
gb_pred = clf2.predict(test)

for x in range(1, 4):
    clf2 = ensemble.GradientBoostingRegressor(n_estimators=200, random_state=x, max_depth=9, subsample=0.95)
    clf2.fit(train, label_log)
    gb_pred = gb_pred + clf2.predict(test)
...
#preds = np.expm1( preds1/5 )

preds = pd.DataFrame({"id": idx, "cost": et_pred})
preds.to_csv('benchmark_et.csv', index=False)

preds = pd.DataFrame({"id": idx, "cost": gb_pred})
preds.to_csv('benchmark_gbr.csv', index=False)

#preds = pd.DataFrame({"id": idx, "cost": preds})
#preds.to_csv('benchmark_xgb.csv', index=False)
