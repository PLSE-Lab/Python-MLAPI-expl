...
#  As same as test set, I also created translated validation set. Even though test set is now translated to English, it looks still different from orignal English text. To evaluate your model performance, it makes more sense if you use translated validation set which is translated by same way as test set.  
#  This kernel is also forked from [Abhishek's kernel](https://www.kaggle.com/abhishek/bert-multi-lingual-tpu-training-8-cores), please check it out!
...
df_train = df_train_full.sample(frac=1).reset_index(drop=True).head(1000)
...
def _run():
    ...
    #--------------------------------------------------------------------------------
    # Here is english translated validation set
    #--------------------------------------------------------------------------------
    valid_en_dataset = BERTDatasetTraining(
        comment_text=df_valid_en.comment_text_en.values,
        targets=valid_targets,
        tokenizer=tokenizer,
        max_length=MAX_LEN
    )

    valid_en_sampler = torch.utils.data.distributed.DistributedSampler(
          valid_en_dataset,
          num_replicas=xm.xrt_world_size(),
          rank=xm.get_ordinal(),
          shuffle=False)

    valid_en_data_loader = torch.utils.data.DataLoader(
        valid_en_dataset,
        batch_size=64,
        sampler=valid_sampler,
        drop_last=False,
    ...
    for epoch in range(EPOCHS):
        ...
        xm.master_print(f'ORIGINAL VAL AUC = {auc}')
        
        # Check how your model perform for translated val set
        para_loader = pl.ParallelLoader(valid_en_data_loader, [device])
        o, t = eval_loop_fn(para_loader.per_device_loader(device), model, device)
        auc = metrics.roc_auc_score(np.array(t) >= 0.5, o)
        xm.master_print(f'TRANSLATED VAL AUC = {auc}')
