...
X_trn_feature = clf.fit_transform(X_train, y_train)
X_tst_feature = clf.transform(X_test)


pd.DataFrame(X_trn_feature).to_csv('X_train_alldata.csv',index=False)
pd.DataFrame(X_tst_feature).to_csv('X_test_alldata.csv',index=False)
