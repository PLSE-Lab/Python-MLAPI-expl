...
# I am trying to guess an upper bound of this game. I believe we can't go very far beyond LB 0.28-29. My assumption can be wrong if:
# 
# 1. There are fewer low-price entries in the test data 
# 2. Those entries can be classified automatically. 
# 
# I think we all did OK in this game!
...
def rmsle(preds,actuals):
    return np.sqrt(np.sum((np.log(np.array(preds)+1) - np.log(np.array(actuals)+1))**2 / len(preds)))
