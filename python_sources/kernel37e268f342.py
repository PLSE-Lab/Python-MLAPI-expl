...
def mygenerator(table):
    while 1:
        for i, x in table.iterrows():
            ...
            X = trans.resize(X, (64, 64), anti_aliasing=True)
...
model = resnet50_maskrcnn(1, input_img)#resnet_graph(input_img)
model.compile(optimizer=Adam(), loss="binary_crossentropy", metrics=["accuracy"])


stop_callback = keras.callbacks.EarlyStopping(monitor='val_loss')
history = model.fit_generator(mygenerator_batch(train_table, 8), callbacks=[stop_callback],steps_per_epoch=1, epochs=5, validation_data=mygenerator_batch(val_table, 8), validation_steps=1, verbose=2, shuffle=True)
