...
gatrain['dl'] = pd.Series(gatrain.index/len(gatrain.index),index=gatrain.index)
gatest['dl'] = pd.Series(gatest.index/len(gatest.index),index=gatest.index)
...
Xtrain = hstack((Xtr_brand, Xtr_model,nvtr), format='csr')
Xtest =  hstack((Xte_brand, Xte_model,nvtst), format='csr')
...
clf = xgb.train(params, d_train, 500, watchlist, early_stopping_rounds=25)

pred = clf.predict(xgb.DMatrix(Xtest))

pred = pd.DataFrame(pred, index = gatest.index, columns=targetencoder.classes_)
pred.head()
pred.to_csv('sparse_xgb_end.csv', index=True)
