...
if __name__ == '__main__':
    ...
    # Data preprocessing
    y_train = keras.utils.to_categorical(Y, CLASS_NUM)
    #y_test = keras.utils.to_categorical(y_test, CLASS_NUM)
    
    # color preprocessing
    x_train, x_test_sub = color_preprocessing(X, X_sub)
    ...
    resnet.save('wresnet_v1.hdf5')
    
    
    # Load best
    #resnet.load_weights('model_0.hdf5')
    # color preprocessing
    #x_train, x_test_sub = color_preprocessing(X, X_sub)
    
    print('save predicts for Train')
    predictions = np.array(resnet.predict(x_train))
    #predictions = np.argmax(predictions, axis=1)
    #out = np.column_stack((range(predictions.shape[0]), predictions))
    np.savetxt('predicts_train.csv', predictions,)

    # SUBMIT
    
    print('predict for submit')
    predictions = resnet.predict(x_test_sub)
    predictions = np.argmax(predictions, axis=1)
    out = np.column_stack((range(predictions.shape[0]), predictions))
