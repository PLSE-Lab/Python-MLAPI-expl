...
X = dataset.iloc[:, [2,4,5,7,9]].values
Y = dataset.iloc[:, 1].values
X_test = dataset_test.iloc[:, [1,3,4,6,8]].values
...
imputer.fit(X[:, 0:5]) #upper bound is excluded thats why 3
...
#fit this imputer object to our matrix X
imputer_test.fit(X_test[:, 0:5]) #upper bound is excluded thats why 3

#replacing the missing data by mean
X_test[:, 0:5] = imputer_test.transform(X_test[:, 0:5])
