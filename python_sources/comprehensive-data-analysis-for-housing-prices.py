...
# # DATA EXPLORATION ON HOUSING PRICES
# 
# ----------
# 
# Credits to Pedro Marcelino for the original Kernel.

# 1. <b>Understand the problem</b>. We look at each variable and do a philosophical analysis about their meaning and importance for this problem.
# 2. <b>Univariable study</b>. We  focus on the dependent variable ('SalePrice') and try to know a little bit more about it.
# 3. <b>Multivariate study</b>. We try to understand how the dependent variable and independent variables relate.
# 4. <b>Basic cleaning</b>. We clean the dataset and handle the missing data, outliers and categorical variables.
# 5. <b>Test assumptions</b>. We check if our data meets the assumptions required by most multivariate techniques.
# 
...
# To understand our data, we can look at each variable and try to understand their meaning and relevance to this problem. Time consuming, but it will give us the flavour of our dataset.
...
# 
# After this we can filter the spreadsheet and look carefully to the variables with 'High' 'Expectation'. Then, we can rush into some scatter plots between those variables and 'SalePrice', filling in the 'Conclusion' column which is just the correction of our expectations.
# 
# * OverallQual (which is a variable that we don't know how it was computed; a funny exercise would be to predict 'OverallQual' using all the other variables available).
# * YearBuilt.
# * TotalBsmtSF.
# * GrLivArea.
# 
# We ended up with two 'building' variables ('OverallQual' and 'YearBuilt') and two 'space' variables ('TotalBsmtSF' and 'GrLivArea'). This might be a little bit unexpected as it goes against the real estate mantra that all that matters is 'location, location and location'. It is possible that this quick data examination process was a bit harsh for categorical variables. For example, we can expect the 'Neigborhood' variable to be more relevant, but after the data examination we would ended up excluding it. Maybe this is related to the use of scatter plots instead of boxplots, which are more suitable for categorical variables visualization. The way we visualize data often influences our conclusions.
# 
# However, the main point of here is to think a little about our data and expectactions.

# # 2. First things first: analysing 'SalePrice'
# 
# 'SalePrice' is the reason of our quest. It's like when we're going to a party. We always have a reason to be there. Usually, women are that reason.
# 
...
#histogram
sns.distplot(df_train['SalePrice'],bins= 80);
#alternative:
...
var = 'OverallQual'
f, ax = plt.subplots(figsize=(9, 6))   #for the size of plot
...
fig = sns.boxplot(df_train['YearBuilt'], df_train['SalePrice'])
fig.axis(ymin=0, ymax=800000);
plt.xticks(rotation=90);
plt.grid()

#plt.bar(df_train['YearBuilt'], df_train['SalePrice'])
#plt.show()
...
sns.heatmap(df_train.corr(),linewidths=0.0,vmax=.8, 
            square=True,cmp = colormap, annot=False)'''
print(corrmat.head())
...
# 
# Another thing that's worth the attention was the 'SalePrice' correlations. We can see our well-known 'GrLivArea', 'TotalBsmtSF', and 'OverallQual' saying a big 'Hi!', but we can also see many other variables that should be taken into account. That's what we will do next.
...
# * Ah... 'YearBuilt'... It seems that 'YearBuilt' is slightly correlated with 'SalePrice'. HAlthough thinking about 'YearBuilt' because we could do a little bit of time-series analysis to get this right
# 
...
#print(df_train.isnull().count().sort_values(ascending = False))


# Let's analyse this to understand how to handle the missing data.
# 
# We'll consider that when more than 15% of the data is missing, we should delete the corresponding variable and pretend it never existed. This means that we will not try any trick to fill the missing data in these cases. According to this, there is a set of variables (e.g. 'PoolQC', 'MiscFeature', 'Alley', etc.) that we should delete. The point is: will we miss this data? Maybe no! None of these variables seem to be very important, since most of them are not aspects in which we think about when buying a house (maybe that's the reason why data is missing?). Moreover, looking closer at the variables, we could say that variables like 'PoolQC', 'MiscFeature' and 'FireplaceQu' are strong candidates for outliers, so we'll be happy to delete them.
# 
# In what concerns the remaining cases, we can see that 'Garage*X*' variables have the same number of missing data. Maybe the missing data refers to the same set of observations (although we will not check it; it's just 5% and we should not spend 20$ in 5$ problems). Since the most important information regarding garages is expressed by 'GarageCars' and considering that we are just talking about 5% of missing data, We'll delete the mentioned 'Garage*X*' variables. The same logic applies to 'Bsmt*X*' variables.
# 
...
# 
# * The two values with bigger 'GrLivArea' seem strange and they are not following the crowd. We can speculate why this is happening. Maybe they refer to agricultural area and that could explain the low price. We're not sure about this but can be quite confident that these two points are not representative of the typical case. Therefore, we'll define them as outliers and delete them.
...
# We can feel tempted to eliminate some observations (e.g. TotalBsmtSF > 3000) but we can live with that, so we'll not do anything.
...
# # <b>References</b>
# * [Pedro Marcelino Blog](http://pmarcelino.com)
