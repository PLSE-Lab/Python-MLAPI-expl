...
mean_x14 = train[['X14', 'y']].groupby(['X14'], as_index=False).median()
mean_x14.columns = ['X14', 'mean_x14']
train = pd.merge(train, mean_x14, on='X14', how='outer')


mean_x27 = train[['X27', 'y']].groupby(['X27'], as_index=False).median()
mean_x27.columns = ['X27', 'mean_x27']
train = pd.merge(train, mean_x27, on='X27', how='outer')
...
test = pd.merge(test, mean_x14, on='X14', how='left')
test['mean_x14'].fillna(test['mean_x14'].dropna().median(), inplace=True)

test = pd.merge(test, mean_x27, on='X27', how='left')
test['mean_x27'].fillna(test['mean_x27'].dropna().median(), inplace=True)
