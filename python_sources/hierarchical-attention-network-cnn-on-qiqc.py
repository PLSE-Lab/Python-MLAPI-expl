...
# This kernel is based off two ideas from two papers: <br />
...
def build_graph(max_sentence_len):
    ...
    with tf.variable_scope('layer_gather_conv'):
        ...
        _, tf_padded_final_2_conv =             tf.while_loop(while_cond_conv, body_conv, [i_conv, tf_padded_final_conv],shape_invariants=[i_conv.get_shape(),tf.TensorShape([None,sent_cutoff_seq,feature_map_size * 3])])
    ...
    return probs, logits, inputs,batch_sequence_lengths, sentence_batch_len,             sentence_index_offsets,  sentence_batch_length_2, tf_y_final_2, ylen_2,             learning_rate_input, train_step, confusion_matrix, cross_entropy_mean,             loss, global_step, predicted_indices, keep_fc, keep_conv
...
def build_session(train_file, glove_file,mmap_loc,chkpoint_dir,train_tensorboard_dir,valid_tensorboard_dir):
    ...
    with tf.Graph().as_default() as gr:
        final_probs, logits,  inputs, batch_sequence_lengths, sentence_batch_len,        sentence_index_offsets, sentence_batch_length_2, tf_y_final_2, ylen_2,         learning_rate_input, train_step, confusion_matrix, cross_entropy_mean,         loss, global_step, predicted_indices, keep_fc, keep_conv =             build_graph(cutoff_seq)
    ...
    with tf.Session(graph=gr,config=tf.ConfigProto(log_device_placement=False)) as sess:
        ...
        for i in range(0,num_epochs):
            ...
            prob, _, train_confusion_matrix, train_loss, y_2, pred_indices =                 sess.run([final_probs,train_step,confusion_matrix, loss, tf_y_final_2 ,predicted_indices], feed_dict = {
                    inputs : qn_npy,
...
def inference(test_file, glove_file, mmap_loc, chkpoint_dir, out_file):
    ...
    with tf.Session(graph=gr,config=tf.ConfigProto(log_device_placement=False)) as sess:
        ...
        while (i <= test_len):
            ...
            prob, y_2, pred_indices =                 sess.run([final_probs, tf_y_final_2, predicted_indices], feed_dict={
                    inputs: qn_npy,
                    batch_sequence_lengths: qn_batch_len,
                    sentence_batch_len: sentence_len,
                    sentence_index_offsets: np_offsets_len,
