...
# 
# In this class we will plot corona virus data
...
# # This is for Nepal Earthquake 2015/04/25

# In[ ]:


df = pd.read_csv('../input/nepal-earthquake/query (1).csv',header=0)

df.index = pd.to_datetime(df['time'])
df['time'] = df.index.strftime('%Y-%m-%d %H:00:00')
fig = px.scatter_geo(df,lat='latitude',lon='longitude',color='mag',animation_frame='time',color_continuous_scale='Jet',range_color=(5.,8.))
fig.show()
