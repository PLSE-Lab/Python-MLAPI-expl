...
# Enable TPU distribution strategy in tensorflow
...
# Using an LR ramp up because fine-tuning a pre-trained model. Ramps up over 5 epochs
# Learning rate schedule - reduce the learning rate when expected plateau in model performance is detected
# This will reduce the LR with hopes to finetune the weights
...
# Keras callback on the LR, lrfn inputs new LR
...
# The TPU cannot write the model to this VM directly (TPUs can only write to GCS).
# We copy the model to this VM manually and then save it locally.
...
# Changed to use MobileNetV2 instead of Xception
# Changed slightly with new comments about learning rate schedule
