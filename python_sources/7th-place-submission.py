...
get_ipython().run_cell_magic('time', '', '!pip install /kaggle/input/tf-115-dependencies/tensorboard-1.15.0-py3-none-any.whl\n!pip install /kaggle/input/tf-115-dependencies/tensorflow_estimator-1.15.1-py2.py3-none-any.whl\n!pip install /kaggle/input/tf-115/tensorflow_gpu-1.15.0-cp36-cp36m-manylinux2010_x86_64.whl \n!pip install /kaggle/input/tf-115-dependencies/bert_tensorflow-1.0.1-py2.py3-none-any.whl')
...
def my_compute_predictions(example):
  ...
  if predictions:
    for i in range(min(nbest,len(predictions))):
      ...
      if (long_span.start_token_idx,long_span.end_token_idx,          short_span.start_token_idx,short_span.end_token_idx) not in seen_labels:
        nbest_summary.append(summary)
        seen_labels.add((long_span.start_token_idx,long_span.end_token_idx,                        short_span.start_token_idx,short_span.end_token_idx))
...
get_ipython().run_cell_magic('time', '', 'eval_examples = read_nq_examples(input_file=FLAGS.predict_file, is_training=False) \nprint(len(eval_examples))')


# In[ ]:


get_ipython().run_cell_magic('time', '', 'candidates_dict = read_candidates(FLAGS.predict_file) \nprint(len(candidates_dict))')
...
# model L: _wwm_stride_192_neg_0.01_0.04-64-2.00E-05 (model d in writeup)
...
pkl_lst = [x+'.pkl' for x in pred_files]
pkl_lst
...
nq_pred_dict_en = ensemble_nbest_to_nq_pred_dict(nbest_test_lst, ensemble_wts, yes_thr, no_thr)
...
test_answers_df = test_answers_df.rename(columns={0:'predictions'})
...
(pd.read_csv('submission.csv').PredictionString=='YES').sum(), (pd.read_csv('submission.csv').PredictionString=='NO').sum()
