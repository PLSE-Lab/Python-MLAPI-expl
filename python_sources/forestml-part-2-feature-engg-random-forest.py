...
val_X_sbs = seqbacksel_rf.transform(val_X)
...
valsbs_pred = classifier_rf.predict(val_X_sbs)
...
X_sbs = seqbacksel_rf.transform(X)
...
classifier_rffin = RandomForestClassifier(random_state=42)

classifier_rffin.fit(X_sbs, y.values.ravel())
...
test_ids = test["Id"]
test_pred = classifier_rffin.predict(test_X_sbs)
