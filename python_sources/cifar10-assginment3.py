...
num_each_class = {cla : 0 for cla in classes}

for _,label in dataset:
    
    for cla in classes:
        
        if classes[label] == cla:
            num_each_class[cla] += 1
...
# creating lists of random batches of data...
...
def accuracy(outputs, labels):
    # output is an array of probs
    _, preds = torch.max(outputs, dim=1)
    # number of correct predictions/ total amount of items
...
class CIFAR10Model(ImageClassificationBase):
    ...
    def __init__(self):
        super().__init__()
        # layer 1
        self.linear1 = nn.Linear(input_size, in_1)
        
        # hidden layer 1
        self.linear2 = nn.Linear(in_1, in_2)
        
        # hidden layer 2
        self.linear3 = nn.Linear(in_2, in_3)
        
        # layer 2
        self.linear4 = nn.Linear(in_3, output_size)
...
lrs = [0.1, 0.05, 0.01, 0.005, 0.001, 0.0001, 0.00001]
...
test_acc = 0.5637737512588501
test_loss = 1.270632028579712
