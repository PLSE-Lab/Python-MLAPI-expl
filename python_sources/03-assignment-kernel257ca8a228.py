...
dataset_size = len(dataset) #No. of images in training dataset 
print(dataset_size)
...
for _, frequencies in dataset:
    label = classes[frequencies]
...
history += fit(10, 1e-2, model, train_loader, val_loader)  #for 10 iterations with lr 1e-2


# In[ ]:


history += fit(10, 1e-4, model, train_loader, val_loader)  #for 10 iter decreasing learning ratew to 1e-4


# Not significant change in validation accuracy and validation loss. Lets try increasing epochs /iterations

# In[ ]:


history += fit(20, 1e-2, model, train_loader, val_loader) #increasing epochs to 20 for lr 1e-2


# Validation accuracy is improving and validation losses decreasing

# In[ ]:


history += fit(40, 1e-2, model, train_loader, val_loader)


# Increasing epochs didnt increase validation accuracy much
...
# ****reducing the learning rate improved validation accuracy and reduced validation loss
