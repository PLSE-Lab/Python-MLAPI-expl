...
target = trainlabels
print('Training...')
extc = ExtraTreesClassifier(n_estimators=1200,max_features= 19,criterion= 'entropy',min_samples_split= 1,
                            max_depth= 42, min_samples_leaf= 1, n_jobs = -1)      

extc.fit(X_train,target) 
x_pred = extc.predict_proba(X_train)
print(log_loss(trainlabels, np.clip(x_pred[:,1]*1.065, 1e-6, 1-1e-6)))
print('Predict...')
y_pred = extc.predict_proba(X_test)
#print y_pred

submission = pd.DataFrame({'Id': testids,
                           'Pred': np.clip(y_pred[:,1]*1.065, 1e-6, 1-1e-6)})
