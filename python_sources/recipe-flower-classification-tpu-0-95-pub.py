...
# 0. Use the [getting started notebook](https://www.kaggle.com/mgornergoogle/getting-started-with-100-flowers-on-tpu/)
# 1. Use Transfer Learning with fine tuning (to avoid over fitting, use small networks such as Inception, DenseNet) 
# 2. Use rearning rate scheduling
# 3. Use reasonable data augmentation and plot samples (added resize and crop)  
# 4. Use Label smoothing (to have a better generalization)
# 
# Note: This is not the best code, but a very good start!
...
# rng = [i for i in range(25 if EPOCHS<12 else EPOCHS)]
...
# ### Freeze the pre trained model and train the added layers 
...
history = model.fit(get_training_dataset(), steps_per_epoch=STEPS_PER_EPOCH, epochs=5, validation_data=get_validation_dataset()) #validation_steps=50
