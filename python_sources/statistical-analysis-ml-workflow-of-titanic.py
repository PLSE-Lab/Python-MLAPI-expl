...
# Now, I don't feel comfortable hanging around with 687 null values that we fakely named "N". So, let's do something about that. We can use pythons ***groupby*** function to get the average mean of each cabin letter. 
...
# Now, these means can help us determine the unknown cabins, if we compare each unknown cabin rows with the given mean's above. Let's write a simple function so that we can given cabin names based on the means. 
...
# Here the data point's in this graph is continous and therefore the problem is a regression one. However, what if we have data that looks like the following, 
# 
# 
# Therefore, we can use Linear Regression to put line like above, Howe
# 
# So, we have talk 
# 
# 
...
# <h5 align="right">Nelson Mandela</h5> 
...
# So, our accuracy score is 0.7993. This may not seem to be much for now, but we will improve upon that. 
