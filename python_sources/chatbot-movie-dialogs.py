...
# # Data loading
# There are some problems with formatting (eg. the tab character is not quoted properly), so we're doing some loading manually.

# In[ ]:


line_data = pd.read_csv('/kaggle/input/movie-dialog-corpus/movie_lines.tsv', encoding='utf-8-sig',header=None)
...
conversation_data = pd.read_csv('/kaggle/input/movie-dialog-corpus/movie_conversations.tsv', encoding='utf-8-sig', sep='\t', header=None)
...
training_history = model.fit_generator(generate_examples(batch_size=128, hist_length=128), epochs=32, steps_per_epoch=1024)
