...
test = model.predict(df_test.values.reshape(21000,28,28,1))
le.inverse_transform(np.argmax(test, axis=1))
...
make_submission_file(df_submission, 'FAA_mlp_kernel_rip_off_submission_13.csv')
...
predictions = le.inverse_transform(np.argmax(y_train, axis=1))
...
predictions = le.inverse_transform(np.argmax(y_test, axis=1))
