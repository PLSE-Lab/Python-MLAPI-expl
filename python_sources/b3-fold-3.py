...
# ## Thanks to [@shonenkov](https://www.kaggle.com/shonenkov) for this kernel!
...
get_ipython().run_cell_magic('time', '', "\nmarking = pd.read_csv(f'{DATA_PATH}/marking.csv')\nmarking['patient_id'] = marking['patient_id'].fillna(marking['image_id'])\nmarking['sex'] = marking['sex'].fillna('unknown')\nmarking['anatom_site_general_challenge'] = marking['anatom_site_general_challenge'].fillna('unknown')\nmarking['age_approx'] = marking['age_approx'].fillna(round(marking['age_approx'].mean()))\nmarking = marking.set_index('image_id')")


# # StratifyGroupKFold
# 
# I think group by patient_id is very important. Also I think that stratify by age, sex, target, source, anatom_site_general_challenge also useful.
...
def get_stratify_group(row):
    ...
    stratify_group += f'_{row["age_approx"] // 5}'
...
df_folds = marking[['patient_id', 'stratify_group', 'target', 'source']]
df_folds.loc[:, 'fold'] = 0

for fold_number, (train_index, val_index) in enumerate(skf.split(X=df_folds.index, y=df_folds['stratify_group'], groups=df_folds['patient_id'])):
