...
# # Intro_before_Intro
# 
# This kernel helped me win Silver and 57th place! It is a slight variation of [SchNet Starter Kit](https://www.kaggle.com/toshik/schnet-starter-kit) by @toshik.
# 
# [What else wento into #57 blend is summarized here](https://www.kaggle.com/c/champs-scalar-coupling/discussion/106359#)
# 
# One must run 8 kernels such as this one separately for each coupling type and then combine everything into a single submission.
# I expect LB ~ -1.97 with the parameters that I used (see below). Since this variation is actually ~20% faster version of what I actually used (due to testing only on a single type), one could increase the #epochs listed below by 20-30% and possibly get -2.0 to -2.1 score (just guessing here, I don't want to waste ~50hrs of GPU time to find out) while stil being within GPU kernel limit.
# 
# batch_size and # epochs used (**by no means these are optimal**):
# 
# * 1JHN 9 100
# * 2JHN 13 80
# * 3JHN 14 80
...
TYPE_VAL = '1JHN'  # change to the required type, must run 8 kernels to cover the train set.
def load_dataset():
    ...
    train = train[train['type'] == TYPE_VAL].copy()
    train = train.reset_index(drop=True).copy()
    test = test[test['type'] == TYPE_VAL].copy()
...
batch_size = 9  #change batch size according to coupling type per the table at the top or to your own value
...
trainer = training.Trainer(updater, (1, 'epoch'), out="result")  #change #epochs according to coupling type per the table at the top or to your own va
...
class TypeWiseEvaluator(Evaluator):
    ...
    def evaluate(self):
        ...
        if self.is_submit:
            ...
            submit.to_csv(f'kernel_schnet_{TYPE_VAL}.csv', index=False)
...
trainer.extend(
    TypeWiseEvaluator(iterator=valid_iter, target=model, converter=coupling_converter, 
                      name='valid', device=0, is_validate=False))  ##Not checking validation speeds things up. Change to True if you need validation score
trainer.extend(
...
submit = pd.read_csv(f'kernel_schnet_{TYPE_VAL}.csv')
display(submit.head())
