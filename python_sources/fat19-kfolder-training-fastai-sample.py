...
PREPROCESSED = Path('../input/fat19-dataprepare')
WORK = Path('work')
Path(WORK).mkdir(exist_ok=True, parents=True)

PKL_TRN_CURATED = PREPROCESSED/'mels_train_curated.pkl'
PKL_TRN_NOISY_BEST50S = PREPROCESSED/'mels_trn_noisy_best50s.pkl'

trn_curated = pickle.load(open(PKL_TRN_CURATED, 'rb'))
trn_noisy_best50s = pickle.load(open(PKL_TRN_NOISY_BEST50S, 'rb'))

X_curated_data = trn_curated['data']
X_noisy_best50s_data = trn_noisy_best50s['data']

trn_curated_df = trn_curated.drop(['data'],axis=1)
trn_noisy50s_df = trn_noisy_best50s.drop(['data'],axis=1)

df = pd.concat([trn_curated_df, trn_noisy50s_df], ignore_index=True, sort=True)
X_train = list(X_curated_data) + list(X_noisy_best50s_data)
...
for fold_, (trn_idx, val_idx) in enumerate(folds.split(df['fname'],df['labels'])):
    tfms = get_transforms(do_flip=False, max_rotate=0, max_lighting=0.1, max_zoom=0, max_warp=0.)
    src = (ImageList.from_df(df, WORK)
    ...
    learn.fit_one_cycle(3, 2e-3,callbacks=[SaveModelCallback(learn, every='improvement', monitor='lwlrap', name='best')])
...
#     learn = load_learner(WORK, file='../input/simple-model-fat2019/FAT2019_Simple'+str(i)+'.pkl',test = test)
#     preds, _ = learn.TTA(ds_type=DatasetType.Test)
#     print(preds.shape)
#     predictions = predictions + preds
# predictions = predictions/num_folds

# test_df[learn.data.classes] = predictions
