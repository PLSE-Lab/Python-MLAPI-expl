...
df_all = pd.merge( df_all, sessions_data, on = 'id', how='left')
...
df_all['ac'] = pd.to_datetime(df_all['date_account_created'])
...
df_all['fa'] = pd.to_datetime(df_all['timestamp_first_active'],format='%Y%m%d%H%M%S')
...
df_all['today'] = pd.to_datetime(datetime.now().strftime("%Y-%m-%d"))
df_all['lifetime'] = (df_all['today']- df_all['ac'])/np.timedelta64(1, 'D')
df_all['recentivity']= ( df_all['today'] - df_all['fa'])/np.timedelta64(1, 'D')
df_all = df_all.drop(['today','ac','fa'], axis=1)
# sys.exit()
#Age
