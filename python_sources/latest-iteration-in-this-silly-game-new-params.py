...
num_boost_round = 510  # From Bruno's original CV, I think
...
first_result["price_doc"] = np.exp( .705*np.log(first_result.price_doc_louis) + 
                                    .295*np.log(first_result.price_doc_bruno) )  # multiplies out to .5 & .2
result = first_result.merge(gunja_output, on="id", suffixes=['_follow','_gunja'])

result["price_doc"] = np.exp( .72*np.log(result.price_doc_follow) + 
