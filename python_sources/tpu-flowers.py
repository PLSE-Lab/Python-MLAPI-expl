...
#train_label=[]
#for image, label in load_dataset(TRAINING_FILENAMES, labeled=True):
...
model.save_weights('enetb6.h5')


# In[ ]:


#model.load_weights('/kaggle/input/pretrainedweights/enetb7.h5')
#model.save('enetb7_model.h5')


# # Training curves model 1
...
#model2.load_weights('/kaggle/input/pretrainedweights/dnet201.h5')#loaded after training in separate sessions
#model2.save('dnet201_model.h5')
...
#display_training_curves(history2.history['loss'])#, history.history['val_loss'], 'loss', 211)
#display_training_curves(history2.history['sparse_categorical_accuracy'])#, history.history['val_sparse_categorical_accuracy'], 'accuracy', 212)
...
#probabilities = (0.52*model.predict(test_images_ds)+0.48*model2.predict(test_images_ds))
...
#training_data_full=get_training_dataset().unbatch()
