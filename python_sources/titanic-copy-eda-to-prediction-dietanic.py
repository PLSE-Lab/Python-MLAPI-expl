...
# I copied [this kernel](https://www.kaggle.com/ash316/eda-to-prediction-dietanic).
# 
...
# Only 38.4% of passwengers survived the accident.
...
# Women's survival rate is bigger than men's.
...
# We can clearly see that Passengers Of Pclass 1 were given a very hight priority while rescue. Even though the number of Passengers in Pclass 3 were a lot higher, still the number of survival from them is very low, somewhere around 25%.
# 
...
# We use FactorPlot in this case, because they make the sepeartion of categorical values easy.
# 
...
# We need to replace Nan values in Age features. We can assign them the mean age of the dataset. But the problem is, there were many people with many different ages. 
# 
# We can check the Name feature. Looking upon the feature, we can see that the names have a saluation like Mr or Mrs. Thus we can assign the mean values of Mr and Mrs to the respective groups.
...
# We can check that there isn't any null values.
...
# 
# 4) Port Q had almost 95% of the passengers where from Pclass3.
...
# 
# 3) Port Q looks to be unlukist for Men, as almost all were from Pclass 3.
...
# 
# The reason is ** Pclass**. The crsstab shows that Person with SibSp > 3 were all in Pclass3. It is imminent that all the large familes in Pclass3 died.
...
# There looks to be a large distribution in the fares of Passengers in Pclass1 and this distribution goes on decreasing as the standards reduces. As this is also continous, we can convert into discrete values by using [binning](https://en.wikipedia.org/wiki/Data_binning).
...
# **Parch+SibSp** : Having 1-2 siblings, spouse on board 1-3 Parents shows a greater chance of probability rather than being alone or having a large family travelling with you.
...
# The highest correlation is between SibSp and Parch i.e 0.41. So we can carry on with all features.
...
test_data['Age_band'].value_counts().to_frame().style.background_gradient(cmap='summer_r')
...
# There isn't any special correlation.
...
# Let's check the accuracies over various values of n_neighbors.
...
# ### Bagging
# Baggine is a general ensemble method. It works by applying similar classifiers on small partitions of the dataset and then taking the average of all the predictions. Due to the averaging, there is reduction in variance. Unlike Voting Classifier, Bagging makes use of similar classifiers.
...
# The weak learner or estimator in this case is a Decision Tree. But we can change the default base_estimator to nay algorithm of our choice.
...
pd.DataFrame(prediction[['PassengerId', 'Survived']]).to_csv('submission.csv', index=False)
