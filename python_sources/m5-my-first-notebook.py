...
def create_dt(is_train = True, nrows = None, first_day = 1200):
    prices = pd.read_csv("D:/Kaggle/m5-forecasting-accuracy/sell_prices.csv", dtype = PRICE_DTYPES)
    for col, col_dtype in PRICE_DTYPES.items():
        if col_dtype == "category":
            prices[col] = prices[col].cat.codes.astype("int16")
            prices[col] -= prices[col].min()
            
    cal = pd.read_csv("D:/Kaggle/m5-forecasting-accuracy/calendar.csv", dtype = CAL_DTYPES)
    ...
    dt = pd.read_csv("D:/Kaggle/m5-forecasting-accuracy/sales_train_validation.csv", 
