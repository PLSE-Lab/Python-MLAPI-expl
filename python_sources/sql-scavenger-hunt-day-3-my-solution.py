...
# 
# > **Important note:** Make sure that you add the BigQuery dataset you're querying to your kernel. Otherwise you'll get an error becuase you won't have access to the dataset.
...
# Yep, our results were, in fact, returned sorted! Now let's take a quick peek to figure out which days are the most dangerous:
...
# Your code goes here :)
accidents_by_hour_q = """SELECT EXTRACT(HOUR FROM timestamp_of_crash),
                                COUNT(consecutive_number)
                    FROM `bigquery-public-data.nhtsa_traffic_fatalities.accident_2015`
                    GROUP BY EXTRACT(HOUR FROM timestamp_of_crash)
                    ORDER BY COUNT(consecutive_number) DESC"""
accidents.estimate_query_size(accidents_by_hour_q)


# In[ ]:


accidents_by_hour = accidents.query_to_pandas_safe(accidents_by_hour_q)
...
plt.title('accidents by hour with most accidents happening at 6pm followed closely by 8pm')


# In[ ]:


# Stated with the most hit-and-run accidents
...
accidents.estimate_query_size(hit_run_query)
...
fig = plt.figure(figsize=(12,5))
plt.bar(hit_run.registration_state_name, hit_run.f0_)
