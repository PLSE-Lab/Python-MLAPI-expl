...
# The Kernel shows how the best result benfit from adding some regularisation.[](http://)
...
# #### Function for achoring offense moving left from {0,0}
...
oof_prediction = np.zeros(shape=(X.shape[0]))
...
for fold_id, (tr_idx, vl_idx) in enumerate(FOLD_LIST):
    ...
    oof_prediction[vl_idx] += model.predict(x_vl).reshape(-1)
    models.append(model)
...
plt.figure()
plt.hist(oof_prediction, bins=100)
...
for fold_id, (tr_idx, vl_idx) in enumerate(FOLD_LIST):
    ...
    model.fit(x_tr, y_tr, callbacks=[metric], epochs=10, batch_size=1024, validation_data=[x_vl, y_vl])
