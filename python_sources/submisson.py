...
def LeaveOneOut(data1, data2, columnName, useLOO=False):
    ...
    if(useLOO):
        x = ((x*x.shape[0])-outcomes)/(x.shape[0]-0.5)
...
def GrabData():
    ...
    cols = [['Id','L1_S24_F1559', 'L3_S32_F3851',
             'L1_S24_F1827', 'L1_S24_F1582',
             'L3_S32_F3854', 'L1_S24_F1510',
             'L1_S24_F1525'
                 ],
            ['Id','L3_S30_D3541','L3_S29_D3428','L3_S34_D3877','L3_S33_D3870','L3_S35_D3910', 'L1_S24_D1828', 'L0_S15_D401', 'L3_S39_D3974', 'L1_S25_D1887', 'L3_S32_D3852'],
            ['Id', 'L1_S24_F1846', 'L3_S32_F3850',
...
#    STATIONS = ['S32', 'S33', 'S34', 'S35', 'S24', 'S25', 'S15', 'S30', 'S29', 'S39']
...
#    train_result = blindtraindata.merge(train_date, how='right', on='Id')
    
#    test_date_part = pd.read_csv('../input/test_date.csv', nrows=10000)
#    test_date_cols = test_date_part.drop('Id', axis=1).count().reset_index().sort_values(by=0, ascending=False)
#    test_date_cols['station'] = test_date_cols['index'].apply(lambda s: s.split('_')[1])
#    test_date_cols = test_date_cols[test_date_cols['station'].isin(STATIONS)]
#    test_date_cols = test_date_cols.drop_duplicates('station', keep='first')['index'].tolist()
    
#    print(date_cols)
#    test_date = pd.read_csv('../input/test_date.csv', usecols=['Id'] + test_date_cols)
...
#    testdata['Response'] = 0 
#    test_result = testdata.merge(test_date, how='right', on='Id')
...
def newGrab():
    ...
    for station in STATIONS:
        test_date[station] = 1 * (test_date[station] >= 0)
    responseTest = pd.read_csv('../input/test_numeric.csv', usecols=['Id'])
    responseTest['Response'] = 0 
    print(response.shape)
    test = responseTest.merge(train_date, how='left', on='Id')
    
    
    visibletraindata = train[::2]
    blindtraindata = train[1::2]
    print(blindtraindata.columns)
    for i in range(2):
        for col in blindtraindata.columns:
            print(col)
            if col != "Response" :
                blindtraindata.loc[:, col] = LeaveOneOut(visibletraindata,
                                                     blindtraindata,
                                                     col, False).values
                test.loc[:, col] = LeaveOneOut(visibletraindata,
                                               test, col, False).values
    del visibletraindata
    gc.collect()
    
    test[['Id', 'Response']].to_csv('xgbsubmission'+'.csv',
                                          index=False)
    
    
    return blindtraindata, test
