...
text = [[q_input, q_input_mask, q_segment_mask, a_input, a_input_mask, a_segment_mask]        for q_input, q_input_mask, q_segment_mask, a_input, a_input_mask, a_segment_mask in         zip(q_inputs, q_input_masks, q_segment_masks, a_inputs, a_input_masks, a_segment_masks)]
...
t_text = [[t_q_input, t_q_input_mask, t_q_segment_mask, t_a_input, t_a_input_mask, t_a_segment_mask]           for t_q_input, t_q_input_mask, t_q_segment_mask, t_a_input, t_a_input_mask, t_a_segment_mask in           zip(t_q_inputs, t_q_input_masks, t_q_segment_masks, t_a_inputs, t_a_input_masks, t_a_segment_masks)]
...
for fold, (train_idx, valid_idx) in enumerate(gkf):   
    if fold in [0, 1, 2]:
        ...
        model.cuda()
        optimizer = torch.optim.Adam(model.parameters(), lr=2e-5)
        train_set = TensorDataset(torch.LongTensor(np.array(data['txt'])[train_idx]),                                  torch.FloatTensor(np.array(data['label'])[train_idx])) 
        dev_set = TensorDataset(torch.LongTensor(np.array(data['txt'])[valid_idx]),                                  torch.FloatTensor(np.array(data['label'])[valid_idx]))
