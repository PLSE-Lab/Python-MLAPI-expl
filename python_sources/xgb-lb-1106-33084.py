...
shift = 200
COMB_FEATURE = 'cat80,cat87,cat57,cat23,cat12,cat2,cat9'.split(',')
...
if __name__ == "__main__":
    ...
    train['loss'] = np.log(train['loss'].values+200)
    ...
    m_cat_f = []
    cat_feature = [n for n in train_test.columns if n.startswith('cat')]
    for cat in cat_feature:
        if len(train_test[cat].unique()) > 2:
            m_cat_f.append(cat)
            
    print(m_cat_f)
    m_cat_f.remove('cat116')
    
    for col in m_cat_f:
        print('getting dummy for:',col)
        dummy = pd.get_dummies(train_test[col],prefix=col+'_').astype('bool')
        train_test = pd.concat([train_test,dummy],axis=1,copy=False)
        del dummy
        #print(train_test.columns)
        gc.collect()


    train_test.drop(m_cat_f,inplace=True,axis=1)
    gc.collect()
    ...
    train.to_csv('trainInteraction.csv.gz', index=True, index_label='id',compression='gzip')
