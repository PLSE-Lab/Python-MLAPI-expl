...
if __name__ == '__main__':
    ...
    # Initialize SVD
    svd = TruncatedSVD(n_components=410)
    ...
    for i in range(len(train.id)):
        s=(" ").join(["q"+ z for z in train["query"][i].split(" ")]) + " " + (" ").join(["z"+ z for z in train.product_title[i].split(" ")]) + " " + BeautifulSoup(train.product_description[i]).get_text(" ")
        s=re.sub("[^a-zA-Z0-9]"," ", s)
        s= (" ").join([stemmer.stem(z) for z in s.split(" ")])
        s_data.append(s)
        s_labels.append(str(train["median_relevance"][i]))
    for i in range(len(test.id)):
        s=(" ").join(["q"+ z for z in test["query"][i].split(" ")]) + " " + (" ").join(["z"+ z for z in test.product_title[i].split(" ")]) + " " + BeautifulSoup(test.product_description[i]).get_text()
        s=re.sub("[^a-zA-Z0-9]"," ", s)
        s= (" ").join([stemmer.stem(z) for z in s.split(" ")])
        t_data.append(s)
