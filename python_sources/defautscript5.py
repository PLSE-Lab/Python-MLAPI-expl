...
random_forest_model = RandomForestClassifier(n_estimators = 34)
...
# SVM - Scaler vector Machine
# Train on the normalized training data we created earlier
print ('')
print ('Training Scalar Vector Machine...')
# scalar_vector_machine_model=svm.SVC(degree=3, gamma='auto', kernel='linear')
scalar_vector_machine_model=svm.LinearSVC(C=1.0, class_weight=None, dual=False, fit_intercept=True,
     intercept_scaling=5.6, loss='squared_hinge', max_iter=10000,
     multi_class='ovr', penalty='l1', random_state=None, tol=0.0001,
     verbose=0)



scalar_vector_machine_model.fit(trainnorm[:,2:],trainnorm[:,0].astype(int))

# predict on normalized test data
print ('')
print ('Predicting using Scalar Vector Machine...')
svm_predicted_output = scalar_vector_machine_model.predict(testnorm[:,1:])
svm_predicted_output_train=scalar_vector_machine_model.predict(trainnorm[:,2:]) 

# Accuracy value
print ('')
svm_predicted_value= metrics.accuracy_score(trainnorm[:,0].astype(int), svm_predicted_output_train)
print ('Accuracy value: ', svm_predicted_value) 

# Write results to a csv
scalar_vector_machine_result = np.c_[testnorm[:,0].astype(int), svm_predicted_output.astype(int)]
dataframe_svm_result = pd.DataFrame(scalar_vector_machine_result[:,0:2], columns=['PassengerId', 'Survived'])

dataframe_svm_result.to_csv('scaler_vector_machine.csv', index=False)