...
for i in range(len(test['numFam'])):
    ...
    elif test['numFam'][i] <= 7 and test['numFam'][i] >= 4:
        test.loc[i, 'numFam'] = 'media'
    elif test['numFam'][i] < 4 and test['numFam'][i] > 0:
...
dummies = []
cols = ['Sex', 'Embarked', 'Title', 'AgeGroup', 'numFam']
for col in cols:
    dummies.append(pd.get_dummies(train[col]))
titanic_dummies = pd.concat(dummies, axis=1)
train = pd.concat((train, titanic_dummies), axis=1)
train = train.drop(['Sex', 'Embarked','Title', 'AgeGroup', 'numFam'], axis=1)
...
test = test.drop(['Sex', 'Embarked', 'Title', 'AgeGroup', 'numFam'], axis=1)
...
train = train.drop(['Survived', 'Ticket', 'Cabin', 'Name', 'PassengerId', 'Age'], axis=1)
test = test.drop(['Ticket', 'Cabin', 'Name', 'PassengerId', 'Age'], axis=1)
