...
def eval_qwk_lgb_regr(y_true, y_pred):
    """
    Fast cappa eval function for lgb.
    array([1.10338149, 1.73633227, 2.2427726 ])
    """
#     y_pred[y_pred <= 1.12232214] = 0
#     y_pred[np.where(np.logical_and(y_pred > 1.12232214, y_pred <= 1.73925866))] = 1
#     y_pred[np.where(np.logical_and(y_pred > 1.73925866, y_pred <= 2.22506454))] = 2
#     y_pred[y_pred > 2.22506454] = 3
# array([1.12119976, 1.73094009, 2.17857588])
    y_pred[y_pred <= 1.12119976] = 0
    y_pred[np.where(np.logical_and(y_pred > 1.12119976, y_pred <= 1.73094009))] = 1
    y_pred[np.where(np.logical_and(y_pred > 1.73094009, y_pred <= 2.17857588))] = 2
    y_pred[y_pred > 2.17857588] = 3
    # y_pred = y_pred.reshape(len(np.unique(y_true)), -1).argmax(axis=0)
    return 'cappa', qwk(y_true, y_pred), True
...
def get_data(user_sample, test_set=False):
    ...
    for i, session in user_sample.groupby('game_session', sort=False):
        ...
        if (session_type == 'Assessment') & (test_set or len(session)>1):
            ...
            # meta
            features['meta_len_session'] = len(session)
            features['meta_i'] = i
            features['meta_set'] = 1 if test_set else 0
...
def get_train_and_test(train, test):
    compiled_train = []
    compiled_semi_train = []
    for ins_id, user_sample in tqdm(test.groupby('installation_id', sort = False), total = 10000):
        compiled_semi_train += get_data(user_sample, test_set=True) # test_set = False
    for i, (ins_id, user_sample) in tqdm(enumerate(train.groupby('installation_id', sort = False)), total = 17000):
        compiled_train += get_data(user_sample, test_set=False)
    reduce_train = pd.DataFrame(compiled_train)
    reduce_semi_train = pd.DataFrame(compiled_semi_train)
    categoricals = ['session_title']
    return reduce_train, reduce_semi_train, categoricals
...
my_train, my_semi_train, categoricals = get_train_and_test(train, test)
...
def preprocess(reduce_train, reduce_test):
    for df in [reduce_train, reduce_test]:
        ...
        df['installation_event_code_count_mean'] = df.groupby(['installation_id'])['sum_event_code_count']            .apply(lambda x: x.shift().expanding().mean())
        #df['installation_event_code_count_std'] = df.groupby(['installation_id'])['sum_event_code_count'].transform('std')
        df['installation_session_count'] = df.groupby(['installation_id'])['Clip']            .apply(lambda x: x.shift().expanding().count())
        df['installation_duration_mean'] = df.groupby(['installation_id'])['duration_mean']            .apply(lambda x: x.shift().expanding().mean())
        #df['installation_duration_std'] = df.groupby(['installation_id'])['duration_mean'].transform('std')
        df['installation_title_nunique'] = df.groupby(['installation_id'])['session_title']            .apply(lambda x: x.shift().expanding().apply(lambda s: len(np.unique(s)))) #transform('nunique')
        
    features = reduce_train.loc[(reduce_train.sum(axis=1) != 0), (reduce_train.sum(axis=0) != 0)].columns # delete useless columns
    features = [x for x in features if x not in ['accuracy_group', 'installation_id']] + ['acc_' + title for title in assess_titles]
    return reduce_train, reduce_test, features

print(my_train.shape, my_semi_train.shape)
my_train, my_semi_train, features = preprocess(my_train, my_semi_train)
print(my_train.shape, my_semi_train.shape)
reduce_train = pd.concat([my_train, my_semi_train[my_semi_train["meta_len_session"] > 1]]).reset_index(drop=True)
reduce_test = my_semi_train[(my_semi_train["meta_len_session"] == 1)].reset_index(drop=True)
...
cols_to_drop += [c for c in reduce_train.columns if "meta" in str(c)]
print(cols_to_drop)
