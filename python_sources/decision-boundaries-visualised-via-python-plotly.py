...
# 
# Let us now plot some decision boundaries of linear models namely a Linear Support Vector Machine, a logistic regression and a Naive Bayes model and we observe how the boundaries differ from the tree-based models.
...
# **Takeaway from the plots**
# 
# Unlike the tree-based models where the decision boundaries managed to capture and reflect the fact that the dataset consisted of two classes, these linear models however are not able to. We can clearly see that the decision surfaces incorporate multiple shades of color which are not accurately containing either the red or blue class. In fact the decision surfaces seem to be grouping both red and blue data points together. 
