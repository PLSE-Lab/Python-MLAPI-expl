...
b1 = tf.Variable(np.abs(np.array((tf.truncated_normal(shape=[100])))))
b2 = tf.Variable(np.abs(np.array((tf.truncated_normal(shape=[64])))))
b3 = tf.Variable(np.abs(np.array((tf.truncated_normal(shape=[16])))))
b4 = tf.Variable(np.abs(np.array((tf.truncated_normal(shape=[1])))))
...
loss = tf.reduce_mean(tf.square(mx_b / y))
trainingStep = tf.train.AdamOptimizer(lr).minimize(loss)
