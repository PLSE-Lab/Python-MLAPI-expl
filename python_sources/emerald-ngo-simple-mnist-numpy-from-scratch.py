...
# Emerald Ngo/ ID: 15804570
# Math 110B
# Professor Chen/ TA: Kat Dover
# Project 1
...
def h(X,W,b):
    ...
    # layer 2 activation
    a2 = relu(z1)
    # layer 2 (hidden layer) -> layer 3 
    z2 = np.matmul(a2, W[1]) + b[1]

    # New layer: layer 3 -> layer 4 (output layer)
    a3 = relu(z2)
    z3 = np.matmul(a3, W[2]) 
...
def backprop(W,b,X,y,alpha):
    ...
    # New layer: layer 3 -> layer 2
    delta2 = np.matmul(delta3, W[2].T)*(z2>0)
    grad_W1 = np.matmul(a2.T, delta2)

    # New layer: layer 2 -> layer 1
    ...
    #Add one more term to the derivatives
...
# initialization
# The new hidden layer has dimensions (256,256).
np.random.seed(1127)
