...
# 
# As we can see, our tree-based models are able to capture very non-linear boundaries as evinced by the jagged edges and kinks in the decision boundaries. For the first two datasets, which are the circle and moon sets, we clearly see that the models have been able to sufficiently predict the separations between the two classes. However this is not so the case for the third blob set. 

# ### Linear Decision Boundaries
# 
# Let us now plot some decision boundaries of linear models and see how the boundaries differ from the tree-based models.

# In[ ]:


names = ["Linear SVM","Logistic Regression", "Naive Bayes"]
linearclassifiers = [
...
trees = RandomForestClassifier(max_depth=3, n_estimators=10, random_state=0)
trees.fit(X, y)

trees_overfit = RandomForestClassifier(max_depth=200, n_estimators=10, random_state=0)
...
# **Takeaway from the Plots**
# 
# The decision boundaries are very telling in both 
...
# **Takeaway from the Plots**
# 
# The decision boundaries do vary quite drastically for different values of the regularization parameter C. For a very small C ( the first plot), this refers to a regime in which the Logistic Regression model is highly regularized. This is reflected in the decision boundaries 
