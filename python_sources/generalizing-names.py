...
# ### This kernel is from Matei I.'s Taming the BERT. I tried swapping words with different ones to make generic sentences. It reduced the score by 0.02. Nevertheless posting it to save time for those contemplating it. 
# 
...
def change_names(data_file):
    ...
    new_name_list = ["First Person","Second Person","Third Person","Fourth Person",                     "Fifth Person","Sixth Person","Seventh Person","Eigth Person",                     "Nineth Person", "Tenth Person", "Eleventh Person","Twelfth Person",                    "Thirteenth Person","Fourteenth Person","Fifteenth Person"] 
    data = data_file
        ...
        for ent in doc.ents:
            if ent.label_ == "PERSON":
                ...
                if captured_name in dict_of_replaced.keys():
                    data.loc[i,"Text"] = data.loc[i,"Text"][: ent.start_char - resi] +                                     dict_of_replaced[captured_name] + data.loc[i,"Text"][ent.end_char - resi:] 
                else:
                    replace = 1
