...
# This is the second time that I have rewritten the kernel of [Interactive Porto Insights - A Plot.ly Tutorial](https://www.kaggle.com/arthurtok/interactive-porto-insights-a-plot-ly-tutorial) made by [Anisotropic](https://www.kaggle.com/arthurtok).
...
# - Correlation Heatmap plot: Ispect the correlation between the different features
...
# 1. Data Quality Checks: Visualsing and evaluating all missing/Null values
# 2. Features inspection and filtering: Correlation and feature Mutual information plots against the target variable. Inspection of the Binary, categorical and other variables. 
...
print('Thre train dataset contains {} rows and {} columns'.format(rows, columns))
...
# Hmmm, the target varalbe is rather imbalanced so it might be something to keep in mind. An imbalanced target will prove quite
...
# sns.heatmap(train_cat.corr(), linewidths=0.1, vmax=1.0, sqaure=True, cmap=colormap, linecolor='white', annot='False')
data = [
    go.Heatmap(
...
# 
# Another aspect of the data that we may want to inspect would be the columns that only contain binary values, ie, where values take on only either of the two values 1 or 0. Procededing, we store all columns that contain these binary values and then generate a vertical plotly barplot of these binary values as follows:
...
# Furthermore, we could also display a sorted list of all the features ranked by order of their importances, from highest to lowest via the same plotly barplots as follows:
...
layout=dict(
    title='Barplot of Feature Imprtances',
...
# Interestingly we observe that in both Random Forest and Gradient Boosted learning models, the most important feature that both models picked out was the column:
# ps_car_13.
# 
