...
# This notebook is exact replica of sban's kernel **Objects + Bounding Boxes using Resnet50 - ImageAI**
# Only difference is that imageai now outputs bounding box points for detected objects in the image.
# This kernel reflects this update in the prediction string
# Many thanks to @sban
...
get_ipython().system('cp -r ../input/imageairepo3/imageai/imageai imageai')
...
model_weight_path = "../input/imageairepo3/imageai/resnet50_v2.0.1.h5"
    ...
    for j, prediction in enumerate(batch):
        ...
        for eachObject in detections:    
            if eachObject["name"] in rev:
                pred_str += rev[eachObject["name"]] + " " + str(float(eachObject["percentage_probability"])/100) +" "+ str(eachObject["box_points"])
                pred_str += " "
