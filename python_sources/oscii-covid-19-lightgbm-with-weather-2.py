...
day_before_valid = 85-7 # 3-18 day  before of validation
...
df_place = df_traintest8[df_traintest8['place_id'].duplicated()==False].reset_index(drop=True)
df_place['STN'] = ''
...
df_traintest9[df_traintest9['place_id']=='China/Hubei'].head()
...
len(df_preds.ForecastId.unique())


# In[ ]:


len(df_preds_pri.ForecastId.unique())


# In[ ]:


df_preds[~df_preds.ForecastId.isnull()][['ForecastId', 'cases_pred', 'fatal_pred']]    .to_csv('oscii_public.csv', index=False)
df_preds_pri[~df_preds_pri.ForecastId.isnull()][['ForecastId', 'cases_pred', 'fatal_pred']]    .to_csv('oscii_private.csv', index=False)


# In[ ]:


df_preds.columns[100:]
