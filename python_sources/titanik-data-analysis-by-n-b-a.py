...
# 1. Embarked: port where passenger embarked (C = Cherbourg, Q = Queentown, S = Southampton)
...
# * There is a std in survivol of passenger with parch=3
...
# * Passengers who pay higher fare have better survival rate. Fare can be used as categorical training.
...
# Sex is not informative for age prediction, age distrubution seems to be same.
...
# 1st class passengers are older than 2nd, 2nd class passengers are older than 3rd class.
...
# Age is not related with sex but it is correlated with parch, sibsp and pclass.

# In[ ]:


index_non_age = list(train_df["Age"][train_df["Age"].isnull()].index)
