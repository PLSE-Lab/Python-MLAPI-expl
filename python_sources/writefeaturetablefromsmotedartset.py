...
def predict_chunk(df_, clfs_, meta_, features, featurize_configs, train_mean):
    ...
    #I don't care about the predictions right now
    #right now I'm just trying to get the features
...
def process_test(clfs, 
    ...
    for i_c, df in enumerate(pd.read_csv('../input/test_set.csv', chunksize=chunks, iterator=True)):
        ...
        full_test = predict_chunk(df_=df,
                                 clfs_=clfs,
                                 meta_=meta_test,
                                 features=features,
        ...
        if i_c == 0:
            full_test.to_csv(filename, header=True, mode='a', index=False)
        else:
            full_test.to_csv(filename, header=False, mode='a', index=False)
    ...
    full_test = predict_chunk(df_=remain_df,
                             clfs_=clfs,
                             meta_=meta_test,
                             features=features,
                             featurize_configs=featurize_configs,
...
def main(argc, argv):
    ...
    full_train.to_csv('trainingFeatures1039.csv')
    ...
    filename = 'feat_{:.6f}_{}.csv'.format(score, 
