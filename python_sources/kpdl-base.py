...
def choose_vectorizer(option, name='tf_idf'):
    if option == 'generate':
        if name == 'tf_idf':
            vectorizer = TfidfVectorizer(tokenizer = tokenize,ngram_range=(1,3), min_df=5, max_df= 0.8, max_features= 15000, sublinear_tf=True)
        else:
            vectorizer = CountVectorizer(tokenizer = tokenize, ngram_range=(1,2), max_df=0.8, min_df=5, max_features = 15000, sublinear_tf=True)
    elif option == 'load':
        if name == 'tf_idf':
...
X_train, X_test, y_train, y_test = train_test_split(data['Content'], data['Label'], test_size = .4, shuffle = False)
X_train, X_valid, y_train, y_valid = train_test_split(X_train, y_train, test_size = .2, shuffle = False)
...
X_valid_df = pd.DataFrame(X_valid, columns=['Content'])
...
model.compile(optimizer=optimizers.Adam(0.0005), loss='binary_crossentropy', metrics=['acc', f1_m,precision_m, recall_m])

model.summary()
es = EarlyStopping(monitor='val_f1_m', mode='max', verbose=1, patience=21)
...
train_concat = np.concatenate((np.array(X_train_df['Content'].values).reshape(-1, 1), np.array(y_train_label).reshape(-1, 1), np.array(predict_train_label).reshape(-1, 1)), axis=-1)
valid_concat = np.concatenate((np.array(X_valid_df['Content'].values).reshape(-1, 1), np.array(y_valid_label).reshape(-1, 1), np.array(predict_valid_label).reshape(-1, 1)), axis=-1)
test_concat = np.concatenate((np.array(X_test_df['Content'].values).reshape(-1, 1), np.array(y_test_label).reshape(-1, 1), np.array(predict_test_label).reshape(-1, 1)), axis=-1)
...
train_concat_predict_df = pd.DataFrame(train_concat, columns=['Content', 'True_Label', 'Predict'])
valid_concat_predict_df = pd.DataFrame(valid_concat, columns=['Content', 'True_Label', 'Predict'])
test_concat_predict_df = pd.DataFrame(test_concat, columns=['Content', 'True_Label', 'Predict'])
...
for label in labels:
    ...
    for row in df_content:
        if row[1] != row[2]:
            wrong.append(row)
    df_wrong = pd.DataFrame(wrong, columns=['Label', 'true', 'predict'])
    df_wrong.to_csv(label+'_test.csv')
...
for label in labels:
    ...
    for row in df_content:
        if row[1] != row[2]:
            wrong.append(row)
    df_wrong = pd.DataFrame(wrong, columns=['Label', 'true', 'predict'])
    df_wrong.to_csv(label+'_valid.csv')
...
for label in labels:
    ...
    for row in df_content:
        if row[1] != row[2]:
            wrong.append(row)
    df_wrong = pd.DataFrame(wrong, columns=['Label', 'true', 'predict'])
    df_wrong.to_csv(label+'_train.csv')
    print(label, df_wrong.head())
