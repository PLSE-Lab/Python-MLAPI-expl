...
# Transfer learning using ResNet50 on the plant-seedlings-classification dataset of kaggle. A maximum accuracy of 80% with a min validation loss of 0.9 was achieved.
...
get_ipython().system('cp ../input/input/input/keras-pretrained-models/resnet50_weights_tf_dim_ordering_tf_kernels_notop.h5 ~/.keras/models/')
...
def resnet50_model(num_classes=None):
    ...
    x=Conv2D(256, kernel_size=(1,1),strides=1)(model.output)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)    
    x=Conv2D(128, kernel_size=(1,1),strides=1)(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x=MaxPooling2D(pool_size=(2,2))(x)
    x=Dropout(0.25)(x)
    x=Conv2D(64, kernel_size=(1,1),strides=1)(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x=Conv2D(32, kernel_size=(1,1),strides=1)(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x=MaxPooling2D(pool_size=(2,2))(x)
    x=Dropout(0.25)(x)
...
# model=load_model('model61.05-0.96-0.80.h5', custom_objects={'fscore':fscore})
