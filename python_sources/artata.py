...
num_feats = ["bathrooms", "bedrooms", "latitude", "longitude", "price",
             ...
             "bedPerbath","bedDiffbath","bedPlusbath","bedPerc"
             ]
...
df_test.loc[new_manager_ixes,['count']] = 1
df_test.loc[new_manager_ixes,['one_manager']] = 0
...
## compute fractions and count for each manager
temp=pd.DataFrame()
temp = pd.concat([df_train.building_id,pd.get_dummies(yxa)], axis = 1).groupby('building_id').mean()
temp.columns = ['high_frac','low_frac', 'medium_frac']
temp['count'] = df_train.groupby('building_id').count().iloc[:,1]
temp['one_building'] = np.where(temp['count']==1, 1, 0)
## compute skill
temp['building_skill'] = temp['high_frac']*2 + temp['medium_frac']
##get ixes for unranked managers...
unranked_building_ixes = temp['count']>0
## ... and ranked ones
ranked_building_ixes = unranked_building_ixes
## compute mean values from ranked managers and assign them to unranked ones
mean_values = temp.loc[ranked_building_ixes, ['high_frac','low_frac', 'medium_frac','building_skill']].mean()
temp.loc[unranked_building_ixes,['high_frac','low_frac', 'medium_frac','building_skill']] = mean_values.values
temp.loc[unranked_building_ixes,['one_building']] =0
## inner join to assign manager features to the managers in the training dataframe
df_train.drop(['count','high_frac','low_frac', 'medium_frac'], axis=1, inplace=True)
df_test.drop(['count','high_frac','low_frac', 'medium_frac'], axis=1, inplace=True)
...
new_building_ixes = df_test['high_frac'].isnull()
df_test.loc[new_building_ixes,['high_frac','low_frac', 'medium_frac','building_skill']] = mean_values.values
df_test.loc[new_building_ixes,['count']] = 1
df_test.loc[new_building_ixes,['one_building']] = 0
...
num_feats.append('building_skill')
...
sub.to_csv("arta1_rf.csv", index=False)
print ('Finish...')
