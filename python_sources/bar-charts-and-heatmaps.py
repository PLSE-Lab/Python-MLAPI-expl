...
# You may notice that the code is slightly shorter than what we used in [the previous tutorial](https://www.kaggle.com/alexisbcook/hello-seaborn).  In this case, since the row labels (from the `'Month'` column) don't correspond to dates, we don't add `parse_dates=True` in the parentheses.  But, we keep the first two pieces of text as before, to provide both: 
