...
df_macro['cpi'] = df_macro['cpi']/315

df_macro['cpi'] = np.where(df_macro.cpi.isnull(), 1, df_macro.cpi)
...
df_all['price_doc'] = df_all['price_doc'] / df_all['cpi']

ylog_all = np.log1p(df_all['price_doc'].values)

ylog_train_all = np.log1p(df_all[df_all['env']=='train']['price_doc'].values)

test_cpi_series = df_all['cpi'].values
test_cpi_series = df_all[df_all['env']=='test']['cpi'].values
