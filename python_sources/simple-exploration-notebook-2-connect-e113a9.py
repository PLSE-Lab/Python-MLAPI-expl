...
interest_values = [interest_dict[i] for i in train_df['interest_level'].values]
...
#ax.legend(interest_dict.keys(), interest_dict.values())
...
C = interest_values
m.hexbin(x, y, C=C, gridsize=500,
         bins=None, cmap=cm.YlOrRd_r);
m.colorbar()
...
nchar_desc = train_df['description'].apply(len,1)
...
train_df['nwords_desc'] = [len(desc.split()) for desc in train_df['description']]
...
[x for x in set(desc.split()) for desc in train_df['description']]
