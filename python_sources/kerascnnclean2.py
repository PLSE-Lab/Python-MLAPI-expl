...
def get_im_cv2(path):
    img = cv2.imread(path)
    resized = cv2.resize(img, (32, 32), cv2.INTER_LINEAR).flatten() #use cv2.resize(img, (64, 64), cv2.INTER_LINEAR)
    return [path, resized]
...
def fitAndPredict():
    ...
    model = create_model()
    x_train,x_val_train,y_train,y_val_train = train_test_split(train_data,train_target,test_size=0.25, random_state=35)
    print("fitting data")
    model.fit(x_train,y_train, nb_epoch=50, batch_size=128, verbose=1)
