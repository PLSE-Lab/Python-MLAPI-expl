...
for i, volume_id_ in enumerate(yticks):
    ax.plot(hits_df[hits_df['volume_id'] == volume_id_]['x'],            hits_df[hits_df['volume_id'] == volume_id_]['volume_id'],            hits_df[hits_df['volume_id'] == volume_id_]['z'],            'o', alpha=0.2, color=colors[i], label='Volume_id:{}'.format(volume_id_))
ax.set_xlabel('x')
