...
# **Q: How many images does the testing dataset contain?** (I changed this question, as i suppose this is the correct one looking at the next variable name"test_dataset_size"
...
for _, label in dataset:
    if classes[label] not in dict_classes:
        dict_classes[classes[label]] = 1
    else:
        dict_classes[classes[label]] += 1
dict_classes
...
history += fit(400, 1e-3, model, train_loader, val_loader)


# In[ ]:


history += fit(200, 5e-4, model, train_loader, val_loader)


# In[ ]:


history += fit(100, 5e-3, model, train_loader, val_loader)


# In[ ]:


history += fit(300, 1e-4, model, train_loader, val_loader)
...
arch = "4 layers (1024,1024,1024,10)"
...
test_acc = 0.552539050579071
test_loss = 1.6391712427139282
