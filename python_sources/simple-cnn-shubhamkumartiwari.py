...
jovian.commit(project='CNN-for-Kaggle-Project', environment=None)
...
def fit(epochs, max_lr, model, train_loader, val_loader,  
                  weight_decay=0, grad_clip=None, opt_func=torch.optim.SGD):   # one_cycle
    torch.cuda.empty_cache()
    history = []
...
# model fits 70% faster after switch from SGD to Adam, and switching to resnet
num_epochs = 5
...
history = fit(num_epochs, s_lr, model, train_dl, val_dl, weight_decay=weight_decay, grad_clip=grad_clip, opt_func=opt_func)
# history = fit(num_epochs, lr, model, train_dl, val_dl, weight_decay=weight_decay, grad_clip=grad_clip, opt_func=opt_func)
...
sub_fname = 'data_submission_3.csv'
