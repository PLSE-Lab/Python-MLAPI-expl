...
infbdd=Q1-1.5 * IQR 
supbdd=Q3+1.5 * IQR 
train['price_doc'].ix[train['price_doc']>int(2.6*np.exp(supbdd))] = int(2.5*np.exp(supbdd))
...
model = xgb.train(dict(xgb_params, silent=0), dtrain, num_boost_round=489)
...
output.to_csv('try_miao.csv', index=False)
