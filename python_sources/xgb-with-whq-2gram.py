...
def main():
	...
	train_qs = train_qs.apply(lambda x:x.translate(transtab))
	...
	def word_shares(row):
		q1_list = str(row['question1']).lower().translate(transtab).split()
		q1 = set(q1_list)
		q1words = q1.difference(stops)
		if len(q1words) == 0:
			return '0:0:0:0:0:0'
    			
		q2_list = str(row['question2']).lower().translate(transtab).split()
		q2 = set(q2_list)
		...
		R2 = len(shared_words) / (len(q1words) + len(q2words) - len(shared_words)) #jaccard
