...
submission_df = pd.read_csv('../input/gender_submission.csv')
...
voting_classifier = model_ops.voting_classifier(['BaggingClassifier', 'AdaBoostClassifier'])
...
"""
for model in ['RandomForestClassifier', 'XGBClassifier', 'AdaBoostClassifier', 'GradientBoostingClassifier', 'VotingClassifier']:
    model_ops.classifiers[model]['Estimator'].fit(train_df.drop(['Survived'], axis=1), train_df.Survived)
    submission_df.Survived = model_ops.classifiers[model]['Estimator'].predict(test_df)
    submission_df.to_csv(f"./{model}_predictions.csv", index=False)



# Later will implement Stacked Ensemble technique