...
num_channels = 3               # 3 for RGB, 1 for mono
pad = 'same'
...
model.add(MaxPool2D(pool_size=(2,2), strides=2, padding=pad))
model.add(Dropout(drop))
model.add(Conv2D(filters=128, kernel_size=(3,3), activation='relu', padding=pad))
model.add(Conv2D(filters=128, kernel_size=(3,3), activation='relu', padding=pad))
model.add(MaxPool2D(pool_size=(2,2), strides=2, padding=pad))
model.add(Dropout(drop))
model.add(Conv2D(filters=256, kernel_size=(3,3), activation='relu', padding=pad))
model.add(Conv2D(filters=256, kernel_size=(3,3), activation='relu', padding=pad))
model.add(MaxPool2D(pool_size=(2,2), strides=2, padding=pad))
model.add(Dropout(drop))
model.add(Conv2D(filters=256, kernel_size=(3,3), activation='relu', padding=pad))
...
def my_decode_preds(preds, vindices):
    ...
    for i in range(len(preds)):
        ...
        if preds[i][bestidx] < 0.6:
            ...
            qualif = "Probably "
        output.append(((qualif + vindices[bestidx] + " " + str(round(preds[i][bestidx],2)))))
    return(output)
