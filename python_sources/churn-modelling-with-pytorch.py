...
# Starting with reading the dataset file and checking the first few rows

# In[ ]:


dataset_df = pd.read_csv('../input/Churn_Modelling.csv')
dataset_df.head()
...
dataset_df.drop(['RowNumber', 'CustomerId'], axis=1).describe()
...
# RowNumber, CustomerId, and Surname features seem irrelevant for predicting the labels. Let's drop it.

# In[ ]:


dataset_df = dataset_df.drop(['RowNumber', 'CustomerId', 'Surname'], axis=1)
dataset_df.head()
...
# Almost there! Creating a model here!|
...
model = ChurnClassifierWithNALU(in_features=13, out_features=2, nalu_sizes=[], hidden_sizes=[500, 500, 500, 500], dropout=0.2)
print(model)
...
# # Visualizing the training progresses
