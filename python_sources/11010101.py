...
train_test['f4_4'] = train_test[ID_COLUMN].iloc[::-1].diff().fillna(9999999).astype(int)
...
test =  xgb.DMatrix(np.array(test))
...
res = xgb.train(xgb_params, dtrain, num_boost_round=5)
# num_boost_round=10, 
#                feval=eval_mcc,maximize=True
ypred = 1*(res.predict(test) > prior )

print(np.sum(ypred))


submission = pd.DataFrame({"Id": ID,"Response": ypred})
submission[['Id', 'Response']].to_csv('1'+'.csv',index=False)
