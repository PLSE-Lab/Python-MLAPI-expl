...
def maxval(row):
    ...
    return p
print(predictions[:5])
labels=np.apply_along_axis(maxval, 1, predictions)
print(labels[:5])

pd.DataFrame({"ImageId":list(range(1,len(predictions)+1)),"Label":labels}).to_csv("results.csv",index=False,header=True)
