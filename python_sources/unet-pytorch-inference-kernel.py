...
ckpt_path = "../input/mymodel1/model_9.pth"
...
sub = pd.read_csv('../input/mydata/submission_my1.csv')
sub.index = sub['ImageId_ClassId']
print(sub.head())
# start prediction
predictions = []
for i, batch in enumerate(tqdm(testset)):
    ...
    for fname, preds in zip(fnames, batch_preds):
        for cls, pred in enumerate(preds):
            ...
            rrr = sub.loc[name]['EncodedPixels']
            if pd.isnull(rrr):
                rrr =' '
            predictions.append([name, rrr])
