...
sample_submission = pd.read_csv('../input/test_sample_submission.csv')
#type(sample_submission)

train_masks = pd.read_csv('../input/train_masks.csv')
print(type(train_masks))

# We have to load the filepaths for the images
# Then convert each one into a numpy array using imread
# Then convert them all to greyscale 
train_pathHolder = []
for subdir, dirs, files in os.walk('../input/test_images'):
    for file in files:
        #print(file)
        train_pathHolder.append(os.path.join(subdir, file))
train_pathHolder.sort()
#print(train_pathHolder)
#print(len(train_pathHolder))
...
train_numpy =[]
for train_path in train_pathHolder:
    image=cv2.imread(train_path)
    image = resize(image,(256,256), mode='constant', preserve_range=True) # This is where we resize image to be constant
...
model.train(train_numpy,train_masks, learning_rate=config.LEARNING_RATE, epochs=1, layers='heads')
model.train(train_numpy,train_masks, learning_rate=config.LEARNING_RATE / 10,epochs=2, layers="all")
