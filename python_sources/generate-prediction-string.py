...
# OID Mapping Dictionary (Object ID)
...
# Prediction string= OID + Prob + xmin+ ymin+ xmax +ymax    
# sample pred_str
...
# generate pred_str for submission    
# workflow() is an arbitrary function representing all other steps
...
for image_id in tqdm(image_ids):
    ...
    image_path = f'../input/test/open-images-2019-object-detection/{image_id}.jpg'
    i+=1
    v_boxes, v_labels, v_scores=workflow(image_path)
    pred_str=''
    print(image_id)
    for i in range(len(v_labels)):
        pred_str=pred_str + rev[v_labels[i]]+' '+ str(v_scores[i]/100)+ ' '
        pred_str=pred_str+ str(v_boxes[i].xmin/1000)+' '+str(v_boxes[i].ymin/1000)+' '
        pred_str=pred_str+str(v_boxes[i].xmax/1000)+' '+str(v_boxes[i].ymax/1000)+' '
    predictions.append(format_prediction_string(image_id, pred_str))
