...
# **I want to show why it was not worth publishing such kernels...** Now it will not blow up LB.
...
#minor changes as an example and many ways to improve - blends, oof, stacking etc.

submission = pd.read_csv('../input/public-sin-lb-1-38/blending_sin.csv')
s_gp = pd.read_csv('../input/santander-gp-clustering-ii/sergeipluscluster.csv')
s_46 = pd.read_csv('../input/santander-46-features-add-andrew-s-feature/reduced_set_submission.csv')
s_ah = pd.read_csv('../input/an-honest-approach/submit.csv')

submission['target'] = submission['target']*0.2 + s_ah['target']*0.8

test_leak["compiled_leak"].update(submission.iloc[test_leak.loc[test_leak["compiled_leak"]==0].index]['target'])
