...
import datetime
import pandas as pd
from required.utils_laj import *
from required.data_processing import get_CMAPSSData, get_PHM08Data, data_augmentation, analyse_Data
...
def CNNLSTM(dataset, file_no, Train=False, trj_wise=False, plot=False):
    ...
    conv1 = conv_layer(X, filters=18, kernel_size=2, strides=1, padding='same', batch_norm=False, is_train=is_train,scope='conv_1')
        ...
        if Train == True:
            ...
            if trj_wise:
                ...
                for itr in range(trj_iteration):
                    ...
                    # plt.show()
                data = {'engine_no':engine_no,'RUL_Predicted':pred_list,'RUL_Expected':expected_list}
                df = pd.DataFrame(data) 
                export =  df.to_csv("final.csv") 
...
if __name__ == "__main__":
    ...
    from required.data_processing import RESCALE, test_engine_id
