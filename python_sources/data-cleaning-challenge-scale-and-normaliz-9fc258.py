...
# 
# # Ok! Let's start picking some variables of the CalCOFI dataset
# 
...
# We are interested in just some columns, and the numerical ones, so let's create a subset with only the columns that we are going to work with.
...
# Before we go further, we can check how many missing values that we have. 
...
# So we have a lot! Now should be interesting replace the missing values with the mean of the column. However, we have to be careful with some columns and think through the best way to fill them.  For that, we have to check the difference between the graphs of each distribution without the missing values vs filled with the mean. If they have a normal distribution (or close to one), make sense fill with the mean, if not, we'll have an unwanted spike in the graph.
...
# Not so different, hum? We'll not have a problem.
# 
# Now, what about oxygen saturation?
...
# See that is something strange? Seems not so weird yet, however, let's normalize with box-cox so we can see more clearly.
...
# That is a clear difference now! So, in this case, it should be best using .fillna(0), as using .dropna() could lead to errors if you are going to try correlate variables.
# 
# **Do not considerate yet the next parts, I'm still working**

# Now, let's take a look into some variables like the **temperature of water** (T_degC), **salinity** (Salnty), **oxygen saturation** (O2Satq), **depth** (Depthm).
