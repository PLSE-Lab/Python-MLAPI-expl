...
# One thing **@martinpiotte** metioned in his kernel but I forgot to implement is to pad the bounding boxes a bit, since they tend to crop out parts of the flukes.
...
class HWIDataset(FilesDataset):
    ...
    def get_x(self, i):
        ...
        if not (x0 >= x1 or y0 >= y1):
            img = img[max(y0 - PADDING,0):min(y1 + PADDING,img.shape[0]), 
                      max(x0 - PADDING,0):min(x1 + PADDING,img.shape[1]),:]
