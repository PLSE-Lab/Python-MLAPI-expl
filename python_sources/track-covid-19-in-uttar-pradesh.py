...
# # Table of Contents
# 
# 1. [Population Percentage of Uttar Pradesh](#Population)
# 
# 2. [Cases in Uttar Pradesh](#Cases)
# 
...
# 
# 8. [Non virus deaths in Uttar Pradesh](#Non_Virus_Deaths)
# 
...
#Pick the data of Uttar Pradesh

covid = India_covid[India_covid['State/UnionTerritory']=='Uttar Pradesh'].reset_index(drop=True).drop(['State/UnionTerritory', 'Sno', 'Time'], axis = 1)


population = India_population[India_population['State / Union Territory']=='Uttar Pradesh'].reset_index(drop=True).drop(['State / Union Territory', 'Sno'], axis = 1)

ICMRTestingLabs = ICMRTestingLabs[ICMRTestingLabs['state']=='Uttar Pradesh'].reset_index(drop=True).drop('state', axis = 1)

HospitalBeds = HospitalBeds[HospitalBeds['State/UT']=='Uttar Pradesh'].reset_index(drop=True).drop(['State/UT', 'Sno'], axis = 1)

TestingDetails = TestingDetails[TestingDetails['State']=='Uttar Pradesh'].drop('State', axis = 1)
...
for i in range(0, len(covid)):
    if((i>=0 and i<=8) or (i>=28 and i<=39) or (i>=58 and i<=69) or (i>=89 and i<=100)):
...
# # **Population Percentage of Uttar Pradesh**  <a name = "Population"> </a>

# In[ ]:


Total = India_population['Population'].sum()
UP = population['Population']
labels = ["Rest of the India", 'UP']
values = [float(Total - UP), float(UP)]
...
fig.update_layout(title_text= 'Population of Uttar Pradesh', showlegend = False)

fig.show()


# # **Cases in Uttar Pradesh** <a name = 'Cases'></a>
...
UP_confirmed = covid.Confirmed.iloc[-1]
UP_deaths = covid.Deaths.iloc[-1]
UP_cured = covid.Cured.iloc[-1]


confirm = [Total_confirmed, UP_confirmed]
cured = [Total_cured, UP_cured]
deaths = [Total_deaths, UP_deaths]
...
# # **Confirmed Cases in Uttar Pradesh** <a name= 'Confirmed'></a>
...
# # **Active Cases in Uttar Pradesh** <a name = 'Active'></a>
...
# # **Covid Deaths in Uttar Pradesh** <a name = 'Deaths'></a>
...
# # **Cured Cases in Uttar Pradesh** <a name = "Cured"> </a>
...
# # **Non virus deaths in Uttar Pradesh** <a name = "Non_Virus_Deaths"></a>
...
UP = non_virus_deaths.groupby('state')['deaths'].sum()['UP']
...
fig = go.Figure(data = [go.Pie(title = dict(text = 'Non virus deaths', position = 'top left', font = dict(color = 'rgb(0,0,0)')),
                               ...
                               pull = [.05,0], textfont = dict(color = 'white'), insidetextorientation = 'horizontal')])
...
UP = non_virus_deaths[non_virus_deaths['state']=='UP']['reason'].sum()
...
UP_reason = pd.DataFrame(group_list(UP), columns = ['Reason', 'Count'])
UP_reason.sort_values(by = ['Count'], ascending = False, inplace = True)
UP_reason.reset_index(drop = True, inplace = True)


Total_total_count = Total_reason['Count'].sum()
UP_total_count = UP_reason['Count'].sum()
...
UP_other = UP_total_count - (UP_reason['Count'][0] + UP_reason['Count'][1]
                             + UP_reason['Count'][2] + UP_reason['Count'][3] + UP_reason['Count'][4])
...
UP_y_data = [int(UP_reason['Count'][0]/UP_total_count*100), int(UP_reason['Count'][1]/UP_total_count*100),
           int(UP_reason['Count'][2]/UP_total_count*100), int(UP_reason['Count'][3]/UP_total_count*100),
           int(UP_reason['Count'][4]/UP_total_count*100), int(UP_other/UP_total_count*100)]
...
UP_x_data = [UP_reason['Reason'][0], UP_reason['Reason'][1], UP_reason['Reason'][2], UP_reason['Reason'][3], UP_reason['Reason'][4], 'Other']
...
fig.append_trace(go.Bar(
    ...
    name='Non Virus Deaths in Uttar Pradesh',
    orientation='h',
), 1, 2)
...
UP_y = np.array(UP_y_data)

# Adding labels
for ydn, yd, xdn, xd in zip(UP_y, Total_y, UP_x_data,  Total_x_data):
