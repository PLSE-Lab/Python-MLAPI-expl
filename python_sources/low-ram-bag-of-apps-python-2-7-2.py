#MT: Adding sdv and more rounds + other xgb parameters
...
svd = TruncatedSVD(n_components = 430,n_iter=5) #Dimensionality reduction
svd.fit(train)

train = svd.transform(train)
test = svd.transform(test)

X_train, X_val, y_train, y_val = train_test_split(train, Y, train_size=.95)
...
result.to_csv('test_Result300.csv', index=True, index_label='device_id')
