...
class NN:
    ...
    #See http://keras.io/ for parameter options--trying parameter changes
    def __init__(self, inputShape, layers, dropout = [], activation = 'sigmoid', init = 'normal', loss = 'rmse', optimizer = 'adadelta', nb_epochs = 50, batch_size = 32, verbose = 1):
...
def make_dataset(useDummies = True, fillNANStrategy = "mean", useNormalization = True):
    ...
    X_train, X_test, y_train, y_test = train_test_split(train, labels, test_size=0.1,train_size=0.9,random_state=42)
...
clf = NN(inputShape = X_train.shape[1], layers = [128, 64], dropout = [0.5, 0.5], loss='mae', optimizer = 'adadelta', init = 'glorot_normal', nb_epochs = 5)
