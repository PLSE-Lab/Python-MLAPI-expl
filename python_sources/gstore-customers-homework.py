...
for fold_, (trn_, val_) in enumerate(folds):
    ...
    merge_preds[val_] = oof_lgb_preds[val_] * 0.6 + oof_xgb_preds[val_] * 0.2 +    oof_cat_preds[val_] * 0.1 + oof_forest_preds[val_] * 0.1
...
for model in ["lgb", "xgb", "cat", "forest"]:
    trn_pred_list = X_train[['fullVisitorId', 'predictions_' + model]].groupby('fullVisitorId')        .apply(lambda df: list(df["predictions_" + model]))        .apply(lambda x: {'pred_' + str(i) + "_" + model: pred for i, pred in enumerate(x) if i <= 43})
...
full_list_test = []
for model in ["lgb", "xgb", "cat", "forest"]:
    test_pred_list = X_test[['fullVisitorId', 'predictions_' + model]].groupby('fullVisitorId')        .apply(lambda df: list(df["predictions_" + model]))        .apply(lambda x: {'pred_' + str(i) + "_" + model: pred for i, pred in enumerate(x) if i <= 43})
