...
captions = open(captions_path, 'r').read().split("\n")  ## caption for all images first column and image name and captions for image
...
temp = captions[index].split("#")
from IPython.display import Image, display
z = Image(os.path.join(images_path, temp[0]))
display(z)
...
for img in x_train:
    if img == '':
        continue
    caption = "<start> "+ getRandomCaption(tokens[img]) + " <end>"
...
size = len(word_2_indices)
first_word = indices_2_word[0]

#adding first word in end of dict
word_2_indices[first_word] = size
indices_2_word[size] = first_word

#adding <PAD> as first word in dict
word_2_indices['<PAD>'] = 0
indices_2_word[0] = '<PAD>'

#adding <UNK> as llast word in dict
word_2_indices['<UNK>'] = size + 1
indices_2_word[size + 1] = '<UNK>'
...
num_of_images = padded_sequences.shape[0]
...
with open( "captions.pkl", "wb" ) as pickle_f:
    pickle.dump(captions, pickle_f )

with open( "next_words.pkl", "wb" ) as pickle_f:
    pickle.dump(next_words, pickle_f )  
...
with open('train_encoded_images.pkl', 'rb') as f:
...
with open( "images.pkl", "wb" ) as pickle_f:
...
with open( "image_names.pkl", "wb" ) as pickle_f:
    pickle.dump(image_names, pickle_f )
...
with open('next_words.pkl', 'rb') as f:
    next_words = pickle.load(f, encoding="bytes")
...
with open('image_names.pkl', 'rb') as f:
    imag = pickle.load(f, encoding="bytes")
...
hist = model.fit([images, captions], next_words, batch_size=256, epochs=200)
...
img = "../input/flickr_data/Flickr_Data/Images/1028205764_7e8df9a2ea.jpg"
...
candidate = ['Two', 'girls', 'are','shown', 'upside' ,'down']
...
file = open("word_2_indices.pkl","wb")
...
file = open("indices_2_word.pkl","wb")
