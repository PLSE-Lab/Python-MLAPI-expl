...
get_ipython().system('pip install lapjv==1.3.1')
...
# (a, b), c = data[0]class TrainingData(Sequence):
    def __init__(self, score, steps=1000, batch_size=32):
        """
        @param score the cost matrix for the picture matching
        @param steps the number of epoch we are planning with this score matrix
...
with open('score_matrix.pickle', 'wb') as fobj:
    pickle.dump(score, fobj)

# Generate the subsmission file.
prepare_submission(0.99, 'submission.csv')
prepare_submission(0.97, 'submission_.csv')
