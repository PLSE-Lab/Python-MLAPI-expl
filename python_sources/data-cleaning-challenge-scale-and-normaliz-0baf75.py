...
# 
# In a practical world, getting good grades doesn't exactly depend on how much time we spend studying. So a student can score more than another student even if she has spent fewer hours studying than the other student. So I think, scaling is not required. However, we need normalization probably because we're building a linear regression model.
# 
# * You're still working on your grades study, but you want to include information on how students perform on several fitness tests as well. You have information on how many jumping jacks and push-ups each student can complete in a minute. However, you notice that students perform far more jumping jacks than push-ups: the average for the former is 40, and for the latter only 10. Should you scale or normalize these variables?
# 
# It's clear that one jumping jack is not statistically or physically equivalent to doing one push-up. Also the average of both the exercises vary by a significant amount (almost 4 times). Hence, scaling is required so that we can compare these variables on an equal scale. We need normalization also because linear regression is a machine learning technique which assumes our data to be normally distributed.
# 
...
goal1 = kickstarters_2017.goal
scaled_goal = minmax_scaling(goal1, columns = [0])

fig, ax = plt.subplots(1, 2)
sns.distplot(goal1, ax = ax[0])
ax[0].set_title('Original Data')
...
# For pledged column
pos_index_pledge = kickstarters_2017.pledged > 0
pos_pledges_col = kickstarters_2017.pledged.loc[pos_index_pledge]

normalized_pledge = stats.boxcox(pos_pledges_col)[0]

fig, ax = plt.subplots(1, 2)
sns.distplot(pos_pledges_col, ax = ax[0])
ax[0].set_title('Original Data')
