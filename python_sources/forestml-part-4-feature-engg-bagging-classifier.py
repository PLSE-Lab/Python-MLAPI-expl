...
seqbacksel_bg = SFS(classifier_bg, k_features = (30, 50),
                    forward = False, floating = False,
                    scoring = 'accuracy', cv = 5, 
                    n_jobs = -1)
seqbacksel_bg = seqbacksel_bg.fit(train_X, train_y.values.ravel())


print('best combination (ACC: %.3f): %s\n' % (seqbacksel_bg.k_score_, seqbacksel_bg.k_feature_idx_))
print('all subsets:\n', seqbacksel_bg.subsets_)
plot_sfs(seqbacksel_bg.get_metric_dict(), kind='std_err');


# In[ ]:


train_X_sbs = seqbacksel_bg.transform(train_X)
val_X_sbs = seqbacksel_bg.transform(val_X)


# In[ ]:


classifier_bg.fit(train_X_sbs, train_y.values.ravel())
...
valsbs_pred = classifier_bg.predict(val_X_sbs)
...
X_sbs = seqbacksel_bg.transform(X)
test_X_sbs = seqbacksel_bg.transform(test_X)


# In[ ]:


classifier_bgfin = BaggingClassifier(random_state=42)
classifier_bgfin.fit(X_sbs, y.values.ravel())
...
test_ids = test["Id"]
test_pred = classifier_bgfin.predict(test_X_sbs)
