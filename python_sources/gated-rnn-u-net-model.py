...
# # Overview
# ## Model
# The model choice goes one step beyond the [Recursive U-Net](https://www.kaggle.com/kmader/cxr-recursive-u-net-model) where the output segmentation is fed into the same model again as a _seed_ segmentation for making the next prediction. 
# Here we add an additional output called _gate_ which is used to combine the output with the previous segmentation (using multiplication).
...
fit_model(simple_unet, epochs=10, loss_func='binary_crossentropy', batch_size=16)
...
def build_rec_unet(recursive_steps, add_noise=False):
...
def show_multistep(in_model, in_df, samples=2):
    ...
    for (c_idx, c_row), n_axs in zip(in_df.sample(samples, random_state=0).iterrows(), 
        ...
        for i, (last_seg, seg_name) in enumerate(zip(out_segs, in_model.output_names)):
            ...
            m_axs[j+1, 0].set_title(seg_name)


# In[ ]:


show_multistep(rec_unet, sample_aug_df, samples=4)


# In[ ]:


show_multistep(rec_unet, valid_df)
