...
def filter_specific(vf2):
    ...
    vf2['NombreCliente'] = vf2['NombreCliente'].replace(['.*COMERCIAL.*','.*BODEGA.*','.*DEPOSITO.*',                                                            '.*ABARROTES.*','.*MERCADO.*','.*CAMBIO.*',                                                        '.*MARKET.*','.*MART .*','.*MINI .*',                                                        '.*PLAZA.*','.*MISC.*','.*ELEVEN.*','.*SEVEN.*','.*EXP.*',                                                         '.*SNACK.*', '.*PAPELERIA.*', '.*CARNICERIA.*',                                                         '.*LOCAL.*','.*COMODIN.*','.*PROVIDENCIA.*'
                                                        ],'General Market/Mart'\
                                                       , regex=True)
