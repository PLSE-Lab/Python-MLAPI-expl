...
# <h2> Welcome to MARK5826 Week 5!</h2>
# 
# Lecturer In Charge: Junbum Kwon;Teaching Assistant: Daniel Han-Chen & James Lin
# 
# In  week 4, we focused on **CONTENT OPTIMIZATION**. That week was the first week where we will use **machine learning**.
# 
# This week (week 5), we will focus more on machine learning, in the Internet of Things space.
# 
# We will use Multinomial Logisitic Regression (Classifying data into classes) which was coded up using Keras / Tensorflow - a Deep Learning Library.
# 
# You can also create some basic Neural Networks using the LinearModel class.
# 
# <h2>AIM</h2>
# 
# Given IOT sensor data, predict if a elderly patient will fall down in a hospital.
# 
# <h2>TABLEAU</h2>
# 
# We will be also using TABLEAU this week.
# 
...
# 
# <h2>GROUP INTERIM Presentation + Report - DUE NEXT WEEK</h2>
# 
# You will be presenting according to a randomly generated list NEXT FRIDAY. If you cannot make the time, please email us. We will release the list this weekend.
# 
# The Interim Report is due NEXT SATURDAY 10pm.
# 
# <h2>INDIVIDUAL INTERIM REPORT Due week 7 WEDNESDAY 10pm</h2>
# There is NO Individual Interim Presentation. However, you need to submit an overview of what you will do for your project by week 7 Wednesday 10pm.
# 
...
# 
# 4.[Softmax / Multinomial Logistic Regression](#Softmax / Multinomial Logistic Regression)
# 
# 5.[Deep Learning - Neural Networks](#Deep Learning - Neural Networks)
# 
# 6.[Lab Questions](#Lab)
# 
...
# 
# Read the data in just like in other weeks. Use FILES and READ
...
# 
# Below, I want you to understand why I decided to plot the data below.
# 
# Notice the trends evident in the data.
# 
# The TARGET VARIABLE is ACTIVITY (1,2,3,4,5), where:
# 
# [0,'Standing'], 
# [1,'Walking'], 
# [2,'Sitting'], 
...
plot(data = data, x = 'activity', y = 'eeg', style = 'histogram', logy = True)


# In[ ]:


plot(data = data, x = 'activity', y = 'circluation', style = 'violinplot', logy = True)


# In[ ]:


plot(data = data, x = 'activity', y = 'hr', style = 'barplot')


# In[ ]:


plot(data = data, x = 'activity', y = 'bp', style = 'boxplot', logy = True)


# In[ ]:


plot(data = data, x = 'time', y = 'bp', style = 'scatterplot', colour = 'activity')


# <a id='Regression vs Classification'></a>
# <h1> 3. Regression vs Classification</h1>
# 
# In Machine Learning and Data Science, there are 2 main tasks - Regression and Classification.
# 
# Regression was what we did last week. We wanted to predict RATINGS which was a continuous scale --> (4,5, 4.55, 4.6, ..., 5)
# 
# However, Classfication is predicting CLASSES (like Cats or Dogs, Survived or Dead or 1,2,3)
# 
# Classfication can be solved using **Logistic Regression**.
# 
# <img src="https://upload.wikimedia.org/wikipedia/commons/6/6d/Exam_pass_logistic_curve.jpeg" style="width: 500px;"/>
# 
# **Logistic Regression** tries to fit data to output ONLY numbers between 0 and 1. This can be used to predict 2 classes (cats OR dogs) / (1 OR 2).
# 
# 0 can be cats while 1 can be dogs.
# 
# Now, the problem is how about 3 or 10 classes? Then, we have to use MULTINOMIAL LOGISTIC REGRESSION or SOFTMAX REGRESSION.
# 
# Instead of using the logistic or sigmoid function as the final squishing function (squeezes all output from 0 to 1), we use a SOFTMAX function to squeeze all MULTI output from 0 to 1.
# 
# Remember LINEAR REGRESSION expressed as a graph / function?
# 
# <img src="https://drive.google.com/uc?id=1dMdJh99jbw8hD0_DEtiw0SbYjZupAN6_" style="width: 500px;"/>
# 
# The squishing function used in LINEAR REGRESSION is called a LINEAR function. It squeezes the output from -infinity to +infinity.
# 
# Below, is the formula / graphical diagram of a MULTINOMIAL LOGISTIC / SOFTMAX REGRESSION model.
# 
# <img src="https://drive.google.com/uc?id=1qFJaN8cjq9i0Amua_mZabdsIL_8H4EBr" style="width: 500px;"/>
# 
# First, let's clean up the data. Use CLEAN to clear all missing values.
...
# We then want to separate the TARGET "Activity"(Y)  from the data (X). If not, then the model can see Y, which is cheating.
...
# Next, 1,2,3,4,5 in Y really is confusing. Let's replace it with the appropriate labels using REPLACE. We will call the resulting list of values STATES

# In[ ]:


states = replace(Y, [[0,'Standing'], 
             [1,'Walking'], 
             [2,'Sitting'], 
             [3,'Falling'], 
             [4,'Cramps'], 
...
# Now, let's fit a naive LINEAR REGRESSION model on X-->Y. Let's see what happens. Use LINEARMODEL the FIT.
...
# Clearly, plotting the data looks very very bad.
...
# You can also use the SCORE function, and you can see a relatively 71% error. Very bad.
...
# Now, let's try LOGISTIC REGRESSION (predicting 0 or 1.) Let's predict if the person was WALKING or NOT WALKING.
# 
# Use TERNARY to get only WALKING., and True = 1, False = 0.

# In[ ]:


walking = ternary(states, '=="Walking"', 1, 0)
tally(walking)


# Fit a LOGISTIC MODEL using LINEARMODEL but logistic = TRUE.
# 
# You can see some print statements of LOSS and EPOCH.
# 
# EPOCH means optimization step. At each time, the model tries to learn. LOSS is how much error the model finds. The lower, the better.
# 
# VAL_LOSS means validation loss. The lower, the better. It's the loss on 20% of the data which the model didnt see during fitting.
...
# Let's look at the coefficients using MODEL.COEFFICIENTS

# In[ ]:


model.coefficients(plot = True)


# You can also PLOT your predictions. It'll output a CONFUSION MATRIX.
# 
# What this shows, is how many % of the data was classfied correctly as the correct classes. The TRUE classes are on the LEFT, and predicted classes on the BOTTOM.
# 
# The DARKER the left-right diagonal, the better!
# 
# So below, 75-80%ish percent of 1s were classified correctly as 1.

# In[ ]:


model.plot(model.predict(X), walking)


# We can also score the model using SCORE. This produces a CORRELATION coefficient, where larger absolute values of 0.3 are better.
# 
# You can see the model isn't that good.

# In[ ]:


model.score(model.predict(X), walking)


# Next, we want to EXPLAIN the model right? We need to interpret what the model is saying. Use ANALYSE.
# 
# Below says if I change the column by Effect (add 1 or minus 1 means if value was +1 or -1 --> since data was STANDARDISED means >1 std from mean or <1 std from mean),
# 
# how much % probability does it increase to classify the data as Y=1 (walking).
# 
# The larger the % prob, the more impact it has on the logistic regression model.
...
# <a id='Softmax / Multinomial Logistic Regression'></a>
# <h1> 4. Softmax / Multinomial Logistic Regression</h1>
# 
# Now, let's run the SOFTMAX regression / multi class logistic / multinomial logistic regression model on ALL 5 classes.
# 
# Use LOGISTIC = TRUE, and the model will auto determine this (you don't have to do anything else).
...
model.plot(model.predict(X), states)


# You can see above how the plot highlights the model can classify WALKING 60ish% correctly, but STANDING 40ish %.
# 
# CRAMPING / RUNNING / SITTING are very bad. Very hopeless. FALLING a little good.
# 
# Let's analyse / inspect the coefficients and see what contributes to each state.
...
# Note, since there are 5 classes, each class has there own coefficients. Above, you can see EEG contributing positively to Cramps, falling, running and sitting, but negatively to standing and walking. But, the coefficients are a bit confusing to just look at it face value.
...
# If you use ANALYSE, you can see a clearer picture. If we MINUS 1 on EEG, Probability of someone STANDING == 100%.
# 
# If essentially means if we decrease EEG by 1 standard deviation from the mean, the chance of STANDING is guaranteed 100%.
# 
# Use SCORE to see what's the result. Also not very good.

# In[ ]:


model.score(model.predict(X), states)


# <a id='Deep Learning - Neural Networks'></a>
# <h1> 5. Deep Learning / Neural Networks</h1>
# 
# We will also try out a basic Neural Network on this problem! Use the LAYERS = 1,2,3 etc to provide the number of layers the model should have.
# 
# The larger the model, the more COMPLICATED, and it''ll take much more time to train.
# 
# YOU DO NOT need to know how NNs (Neural nets) work, just that you can use them.
# 
# NNs are very hard to interpret, hence when calling COEFFICIENTS, you'll get some complex stuff.
# 
# When calling ANALYSE --> you can only see the model layout and loss history graph.
# 
# <img src="https://drive.google.com/uc?id=1ZSxtATEMxyT6yNfOBBDBDzi-8eqeykwR" style="width: 500px;"/>
# 
# You can see the clear resemblance to LOGISTIC REGRESSION.
# 
# <img src="https://drive.google.com/uc?id=1qFJaN8cjq9i0Amua_mZabdsIL_8H4EBr" style="width: 300px;"/>
...
# You can see the loss has dramatically reduced when compared to normal SOFTMAX regression.
# 
# However, it DOES NOT mean Neural Nets better than old SOFTMAX reg. It's way too complex to interpret, so a bit useless when you want to ACT on something.
# 
# It can only be used for prediction purposes.
...
# When calling ANALYSE, you'll see the model layout I used, and also the HISTORY of loss / val_loss reductions
...
# (Q1) Use TERNARY and train a LOGISTIC REGRESSION model to predict if someone is FALLING or NOT FALLING.
# 
# (Q2) Interpret the model above, explaining what is the most important factor which contributes to FALLING.
# 
# (Q3) Explain why you believe you model is good OR bad. Use SCORE / the CONFUSION_MATRIX
