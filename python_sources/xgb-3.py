...
def cap_share(x):
    return np.sum(1 for c in x if c.isupper())/float(len(x)+1)
...
train_df['num_cap_share'] = train_df['description'].apply(cap_share)
test_df['num_cap_share'] = test_df['description'].apply(cap_share)
...
'num_cap_share','num_email','num_phone_nr']
