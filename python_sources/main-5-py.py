...
train.Age = train.Age.map( lambda x: math.ceil(x / 10) - 1 ).astype(int)
...
test.Age = test.Age.map( lambda x: math.ceil(x / 10) - 1 ).astype(int)
...
print('Training...')
forest = rfc(n_estimators = 1050, n_jobs = -1, criterion = "entropy", random_state = 100, max_features = "auto")
ada_forest = ada(forest, n_estimators = 100, random_state = 150)
