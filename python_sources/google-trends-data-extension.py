...
# # Extending the data set using Google trends data
# 
#  - first let us visualize some of the trends against the actual visits.  
#  - since we cant download, and looks like CORS doesn't seem to be enabled for kaggle's kernels, I'll manually put the trends in a json object here in the kernel, I'll show you how to use the script used to fetch trends after the visualizations.
...
# ## Manually adding some pages titles trends
# ##### The trends files are @ https://github.com/MohmadAyman/web-traffic-forecast/tree/master/trends
# this is not the full list, there is a limit for individual google trends API calls one can make, so everyone is welcome to run the script and make a PR with what could fetch to finish the list.
...
# ## Trends vs Views
# until a decent amount of trends is available, I cant really figure out how useful it is.
...
# ## This is how to fetch the trends locally, you are welcome to make PRs whenever you fetch more trends. 
# there is a limit to the usage of the trends API.
...
# ### I'm not very sure how useful is the search trends data yet, some seems correlated others not, but until the whole trends data is available I can't really say for sure. 

# ### this is my very first public kaggle's kernel, it might be a little messy, but I hope it would be useful.
