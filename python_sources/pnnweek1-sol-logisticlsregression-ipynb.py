...
def test_unvectorized(X_test, Y_test, w, b):
    ...
    for j in range(m_test):
        ...
        if ((o >= 0.5) and (Y_test[0, j] == 1))         or ((o < 0.5) and (Y_test[0, j] == 0)):
            corrects +=1
...
# Test on our test data. Please wait for some time. The accuracy should be better than 89.2%. This high score 89.2% is the baseline, achieved by do nothing rather than predicting all images are not a "seven" :p. 
...
# 
# Finally, we can do minibatch training, it is the same as batch training (the formula) but one iteration runs over a subset of the whole dataset at a time, and those subsets (minibatches) are shuffled before training. Note how I did that minibatch splitting and shuffling using our $numpy.random$ 
