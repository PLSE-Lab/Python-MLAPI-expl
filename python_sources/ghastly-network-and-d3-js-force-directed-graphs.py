...
# 
# For the first chapter, we will investigate the top Tri-and-Bi grams (contiguous sequence of 3 and 2 terms respectively) associated to each author and perhaps visualize this relationship more intuitively with a network diagram.
# 
...
# ## NetworkX Graphs
# 
# Cool. Now that we have generated our Trigram counts on an author basis we can start generating our NetworkX graphs. NetworkX is a Python package for the creation, manipulation, and study of the structure, dynamics, and functions of network and graphs.
# The thing to note here is that we will be returning the top 20 Trigrams attributed to each author.
...
# From the network graph we can see that there are certain popular trigrams used recurringly by our three spooky authors. These trigrams however seem commonplace in the sense that they involve combinations of very common words, therefore a higher probability by virtue of that fact of being shared by the same author. 
# 
# Therefore it might be more interesting to observe the trigram network after removing stopwords and hence will be exactly what we do next.
# 
# 
...
# 1. Generating the Bigram network ( for the Top 20 Bigrams) we get:

# In[ ]:


del g
g = nx.from_pandas_edgelist(bigram_df,
                            source='author',
                            target='index')
plt.figure(figsize=(20, 20))
cmap = plt.cm.RdYlGn
...
# The code to generate the top 20 stopwords attributed to each author is given via the follow construct
