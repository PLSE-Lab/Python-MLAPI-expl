...
ridge.fit(train_df_munged, label_df)
# Run prediction on training set to get a rough idea of how well it does.
y_pred = ridge.predict(train_df_munged)
y_test = label_df
rmse_ridge=rmse(y_test, y_pred)
print("Ridge score on training set: ", rmse_ridge)

# Run prediction on the Kaggle test set.
y_pred_ridge = ridge.predict(test_df_munged)
y_pred_ridge=y_pred_ridge.reshape(y_pred_lasso.shape)
...
y_pred =  (y_pred_xgb*0.6 + y_pred_lasso*0.3 + y_pred_ridge*0.1)
print(y_pred.shape)
print("OK2")
