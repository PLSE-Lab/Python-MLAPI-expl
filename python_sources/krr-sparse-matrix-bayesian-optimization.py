...
# Scale only numerical data
    ...
    start_time = timer(None)
    print("\n# Global Optimization Search for KRR Parameters C and gamma\n")
    ...
    krrBO.maximize(init_points=20, n_iter=100, acq="ei", xi=0.01)
    print("-" * 53)
    timer(start_time)

    best_RMSE = round((-1.0 * krrBO.res['max']['max_val']), 6)
    alpha = krrBO.res['max']['max_params']['alpha']
    gamma = krrBO.res['max']['max_params']['log2gamma']

    print("\n Best RMSE value: %f" % best_RMSE)
    print(" Best KRR parameters:  alpha = %f  log2(gamma) = %f" % (alpha, gamma))

    start_time = timer(None)
    print("\n# Making Prediction")

    krr = KernelRidge(kernel='rbf', alpha=alpha, gamma=math.pow(2,gamma))

    x_true = np.array(train['SalePrice'])
    x_pred = np.exp(cross_val_predict(krr, X=train_data, y=target, cv=folds))
# Normalized prediction error clipped to -40% to 40% range
    ...
    plt.savefig('./HousePrices-KRR-' + str(folds) + 'fold-train-predictions-01-v1.png')
    plt.show(block=False)

    krr.fit(train_data, target)
    y_pred = np.exp(krr.predict(test_data))
    ...
    now = datetime.now()
    sub_file = 'submission_KRR_' + str(best_RMSE) + '_' + str(now.strftime("%Y-%m-%d-%H-%M")) + '.csv'
    print("\n Writing Submission File: %s" % sub_file)
    result.to_csv(sub_file, index=True, index_label='Id')
    timer(start_time)

    history_df = pd.DataFrame(krrBO.res['all']['params'])
    history_df2 = pd.DataFrame(krrBO.res['all']['values'])
    ...
    history_df.to_csv("./HousePrices-KRR-" + str(folds) + "fold-01-v1-grid.csv")
    print("\n Grid Search Results Saved:  HousePrices-KRR-%dfold-01-v1-grid.csv" % folds)

    x, y, z = history_df['alpha'].values, history_df['log2(gamma)'].values, history_df['RMSE'].values
# Set up a regular grid of interpolation points
    xi, yi = np.linspace(-0.1, 2.1, 100), np.linspace(-16.5, -9.5, 100)
...
# Interpolate
    ...
    plt.figure(2)
    plt.title("Interpolated density distribution of alpha vs gamma")
    plt.imshow(zi, vmin=z.min(), vmax=z.max(), origin='lower',
           extent=[-0.1, 2.1, -16.5, -9.5], interpolation = 'lanczos')
    plt.scatter(x, y, c=z)
    plt.colorbar()
    plt.xlabel('alpha')
    plt.ylabel('log2(gamma)')
    plt.autoscale(tight=True)
    plt.savefig('./HousePrices-KRR-' + str(folds) + 'fold-01-v1.png')
    plt.show(block=False)
    print("\n Optimization Plot Saved:  HousePrices-KRR-%dfold-01-v1.png\n" % folds)
