...
# 
# Here we are not only going to use transfer learning, but we're also going to try fine tuning
...
def pretrainedNetwork(xtrain,ytrain,xtest,ytest,pretrainedmodel,pretrainedweights,classweight,numclasses,numepochs,optimizer,labels,fine_tuned=False,layers=None):
...
# So lets run this again, but this time use fine tuning rather than transfer learning

# In[16]:


pretrainedNetwork(X_train, y_trainHot, X_test, y_testHot,pretrained_model_1,weight_path1,class_weight1,10,8,optimizer1,map_characters1,True,vgg16_layers_to_freeze)


# OK so just by using fine tuning rather than transfer learning, we've gone from ~77% to ~85% accuracy
# 
...
# again, try InceptionV3 with fine tuning instead of transfer learning

# In[18]:


pretrainedNetwork(X_train, y_trainHot, X_test, y_testHot,pretrained_model_2,weight_path2,class_weight1,10,8,optimizer1,map_characters1, True, inception_layers_to_freeze)


#  We were able to identify the species of monkey in a given image with an accuracy rate of approximately 80% with transfer learning and over 85% with fine tuning.  Given 10 species of monkey, that is much better than random chance!
