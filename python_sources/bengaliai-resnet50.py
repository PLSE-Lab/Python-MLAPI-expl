...
base_model =  EfficientNetB0(weights =None, include_top = False,                                  pooling = 'avg', input_shape = (128, 128, 1))
x = base_model.output
# x = GlobalAveragePooling2D()(base_model.output)
x = Dropout(0.25)(x)
x = Dense(1000, activation='relu')(x)
...
model = Model(inputs = base_model.input, outputs = [vowel_diacritic_fc1,                                                     grapheme_root_fc1,                                                     consonant_diacritic_fc1])
model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=["accuracy"])
...
for sample, test_image_ids in get_test_batch():    
    ...
    vowel_diacritic, grapheme_root, consonant_diacritic = model.predict_generator(test_gen,                                                                                   verbose=1)
