...
# %%
train_data = pd.read_csv('../input/covid19-global-forecasting-week-3/train.csv', na_filter=False, parse_dates=[4])
test_data = pd.read_csv('../input/covid19-global-forecasting-week-3/test.csv',na_filter=False)
...
for index,row in states.iterrows():
    ...
    public_training_dates = train_x.loc[(train_x < '2020-03-26')]
    public_test_dates = test_x.loc[(test_x <'2020-04-09')]
    private_test_dates = test_x.loc[(test_x >= '2020-04-09')]
    le.transform(public_training_dates)

    public_training_fatal = train_data.loc[ (train_data['Country_Region']==dummy[1]) & (train_data['Province_State']==dummy[0]) & (train_data['Date']<'2020-03-26')]["Fatalities"].to_numpy()
    public_training_case = train_data.loc[ (train_data['Country_Region']==dummy[1]) & (train_data['Province_State']==dummy[0]) & (train_data['Date']<'2020-03-26')]["ConfirmedCases"].to_numpy()
    ...
    '''
    public_split_svr_rbf_fatal = GridSearchCV(SVR(), tuned_parameters)
    public_split_svr_rbf_case = GridSearchCV(SVR(), tuned_parameters)
    private_split_svr_rbf_fatal = GridSearchCV(SVR(), tuned_parameters)
    private_split_svr_rbf_case = GridSearchCV(SVR(), tuned_parameters)
    ...
    output = pd.DataFrame({'ForecastId': test_id, 'ConfirmedCases': (test_predictions), 'Fatalities': (test_predictions_fatalities)})
