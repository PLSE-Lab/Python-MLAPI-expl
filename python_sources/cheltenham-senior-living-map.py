...
# Stabbing
k=d[d.title.str.match(r'EMS: STABBING')]
...
for i in range(0,len(lat)):
    ...
    s+="['%s ', %s, %s, %s,'https://storage.googleapis.com/montco-stats/images/stabbing.png'],\n" % (displayTitle,lat[i],lng[i],i)



# Traffic only
k=d[d.title.str.match(r'Traffic.*')]
t=k[['title','timeStamp','lat','lng','desc','twp']]
title=t.title.tolist()
desc=t.desc.tolist()
twp=t.twp.tolist()
...
for i in range(0,len(lat)):
    displayTitle="%s %s %s %s" % (title[i],desc[i],twp[i],timeStamp[i])
    displayTitle=displayTitle.replace('\n',' ')
    s+="['%s ', %s, %s, %s,'https://storage.googleapis.com/montco-stats/images/traffic.png'],\n" % (displayTitle,lat[i],lng[i],i)



# Fire
k=d[d.title.str.match(r'Fire.*')]
t=k[['title','timeStamp','lat','lng','desc','twp']]
title=t.title.tolist()
desc=t.desc.tolist()
twp=t.twp.tolist()
...
for i in range(0,len(lat)):
    displayTitle="%s %s %s %s" % (title[i],desc[i],twp[i],timeStamp[i])
    displayTitle=displayTitle.replace('\n',' ')
    s+="['%s ', %s, %s, %s,'https://storage.googleapis.com/montco-stats/images/fire.png'],\n" % (displayTitle,lat[i],lng[i],i)


# EMS
k=d[d.title.str.match(r'EMS.*')]
t=k[['title','timeStamp','lat','lng','desc','twp']]
title=t.title.tolist()
desc=t.desc.tolist()
twp=t.twp.tolist()
