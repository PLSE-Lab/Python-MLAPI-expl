...
nn_model.add(Dense(train.shape[1],256))
nn_model.add(Activation('tanh'))
nn_model.add(BatchNormalization(256))
nn_model.add(Dropout(0.25))
nn_model.add(Dense(256,256))
nn_model.add(Activation('tanh'))
nn_model.add(BatchNormalization(256))
nn_model.add(Dropout(0.8))
nn_model.add(Dense(256,1))


# Compile and train model.
nn_model.compile(loss='mse', optimizer=Adadelta())
nn_model.fit(train, target, nb_epoch=32, batch_size=32, verbose=2, validation_split=0.2)
