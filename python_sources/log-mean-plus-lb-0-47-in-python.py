...
train['Demanda_uni_equil'] = train['Demanda_uni_equil'].apply(lambda x: 1.0035*np.log1p(x + 0.01) - 0.0035)
...
gc.collect()
print (test.shape)
test.loc[:, 'Demanda_uni_equil'] = test.loc[:, 'MeanPCA'].apply(np.expm1)*0.75 +\
                            test.loc[:, 'MeanPR'].apply(np.expm1)*0.15 + 0.015
indeks = test['Demanda_uni_equil'].isnull()
test.loc[indeks, 'Demanda_uni_equil'] = test.loc[indeks, 'MeanPR'].apply(np.expm1)*0.77 + 0.17
indeks = test['Demanda_uni_equil'].isnull()
test.loc[indeks, 'Demanda_uni_equil'] = test.loc[indeks, 'MeanC'].apply(np.expm1)*0.87 + 0.87
indeks = test['Demanda_uni_equil'].isnull()
test.loc[indeks, 'Demanda_uni_equil'] = test.loc[indeks, 'MeanPA'].apply(np.expm1)*0.6 + 0.6
indeks = test['Demanda_uni_equil'].isnull()
test.loc[indeks, 'Demanda_uni_equil'] = test.loc[indeks, 'MeanP'].apply(np.expm1)*0.40 + 1
indeks = test['Demanda_uni_equil'].isnull()
