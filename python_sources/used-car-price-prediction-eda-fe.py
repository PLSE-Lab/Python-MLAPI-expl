...
# Technology for feature importance building: xgb, lgbm, linreg
...
drop_columns = ['url', 'region', 'region_url', 'title_status', 'vin', 'size', 'image_url','description', 'lat','long', 'paint_color']
...
for col in categorical_columns:
    if col in car.columns:
        ...
        car[col] = le.transform(list(car[col].astype(str).values))
...
car_stat = car.describe(percentiles = [.05,.1, .9,.95])
car_stat


# ### Selecting the conditions for effective filter of abnormal values

# In[ ]:


car_stat.loc['max',:]-car_stat.loc['95%',:]


# In[ ]:


car_stat.loc['95%',:]-car_stat.loc['90%',:]


# In[ ]:


(car_stat.loc['max',:]-car_stat.loc['95%',:])/(car_stat.loc['95%',:]-car_stat.loc['90%',:])


# In[ ]:


car_stat.loc['10%',:]-car_stat.loc['5%',:]


# In[ ]:


df_stat.loc['5%',:]-df_stat.loc['min',:]


# In[ ]:


(car_stat.loc['5%',:]-car_stat.loc['min',:])/(car_stat.loc['10%',:]-car_stat.loc['5%',:])
...
# # 5 Feature Engineering : tuning models and building the feature importance diagrams
...
# # 6 Comparison of the all feature importance diagrams
