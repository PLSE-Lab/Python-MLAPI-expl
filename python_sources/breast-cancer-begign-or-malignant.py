...
# In this problem we have to use 30 different columns and we have to predict the Stage of Breast Cancer M (Malignant) and B (Bengin) This analysis has been done using Basic Machine Learning Algorithm with detailed explanation This is good for beginners like as me Lets start. 4.Attribute Information:
# 1) ID number
...
# Here Mean means the means of the all cells, standard Error of all cell and worst means the worst cell In [1]:
# In [2]:
# 
...
# from this graph we can see that there is a more number of bengin stage of cancer which can be cured
...
# by taking all features accuracy increased but not so much so according to Razor's rule simpler method is better by the way now lets check the important features in the prediction
...
# the accuracy for RandomForest increases it means the value are more catogrical in Worst part
...
# **observation**
# Accuracy is 100 % means over fitting but cross validation scores are not good 3 so accuracy cant be considered only factor here
# 
# 
...
# cross validation score are also not bad
...
# In next segment I will try to explain the tuning of parameter for different models Then using those parameter we will try to forecast
...
# **observation**
# 
# The SVM is working fine with good parameter it shows us what is the use of running of parameters In the first by using default I was getting only 70 % accuracy But with tuned parameter it is 95 % Same we can do for Random Forest classifier I will not do that if someone is using this as reference please do for Random Forest Classifier also
