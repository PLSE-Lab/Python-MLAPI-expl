...
# ###  Ensemble of XGBoost, LGBM & LGBM with impact encoding
# 
# Our solution is an ensemble of 3 models.
# 
# Our XGBoost model is uses this [kernel](https://www.kaggle.com/reppy4620/xgboost) with some aggregates added with parameter tuning. Our single XGBoost has LB score off 0.452
# 
# We use same features from this [kernel](https://www.kaggle.com/reppy4620/xgboost) and apply LGBM on it with hyperparameter tuning. Our single LGB has a LB score of 0.457
# 
# Lastly we use impact encoding LGBM from this [kernel](https://www.kaggle.com/rookzeno/ensemble-other-people-kernel) and apply LGBM on it with hyperparameter tuning.
# 
# Finally we ensemble all these kernel predictions using weighted average.
...
aggs = {
    'image_size': ['sum', 'mean', 'var', 'std'],
    'width': ['sum', 'mean', 'var', 'std'],
    'height': ['sum', 'mean', 'var', 'std'],
}
...
def lgb_train(params, X_train, X_test):
...
oof_train_lgb , oof_test_lgb = lgb_train(lgb_params, X_train_non_null, X_test_non_null)
...
print(test_predictions_lgb.dtype)
print(test_predictions.dtype)
...
ensembled_prediction = (0.3*(test_predictions.astype(np.int32)))+(0.3*(test_predictions_lgb.astype(np.int32))) + (0.4*(submission3.astype(np.int32)))
print(pd.value_counts(ensembled_prediction, normalize=True).sort_index())


# In[ ]:


pd.value_counts(ensembled_prediction.round().astype(np.int32))


# In[ ]:


submission = pd.DataFrame({'PetID': test['PetID'].values, 'AdoptionSpeed': ensembled_prediction.round().astype(np.int32)})
