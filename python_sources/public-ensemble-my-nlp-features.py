...
### use name length as an extra features
X_temp['name_length'] = X['Name'].apply(lambda x:  len(x) if type(x) is str else 0)
...
### extra text analysis with embeddings
from pymagnitude import *
import nltk

corpus = [nltk.word_tokenize(text) for text in tqdm(X_tempy['Description'].fillna('<MISSING>').values)] 

def word2vec_agg(description):
    vectors = Magnitude("../input/magnitude-embeddings/glove.6B.300d.magnitude")
    return(np.mean(vectors.query(description),axis=0))

# Parallel processing of data:
descriptions = Parallel(n_jobs=6, verbose=1)(
    delayed(word2vec_agg)(description) for description in corpus)


n_components=10
svd_ = TruncatedSVD(n_components=n_components, random_state=1337)
word2vec_cols = svd_.fit_transform(descriptions)
word2vec_cols = pd.DataFrame(word2vec_cols)
word2vec_cols = word2vec_cols.add_prefix('SVD_{}_'.format("word2vecs"))

X_temp = pd.concat([X_temp, word2vec_cols], axis=1)
...
#skip uneccessary re-processing and re-use X_temp
