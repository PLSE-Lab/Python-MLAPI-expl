...
def get_hashes(arr):
    ...
    renta1 = str(abs(hash(renta)) % 10)
    fecha_alta1 = str(abs(hash(fecha_alta)) % 10)
    ...
    if 1:
        # Fixed set
        sub.append((pais_residencia,renta1,fecha_alta1, sexo, age, ind_nuevo, segmento, ind_empleado, ind_actividad_cliente, indresi))
        ...
        sub.append((pais_residencia, renta1,sexo, age, segmento, ind_empleado, ind_actividad_cliente, indresi))
...
def run_solution():
    ...
    sub_file = os.path.join('SB_0311R' + '.csv')
    out = open(sub_file, "w")
