...
class iMetDataset(Dataset):
    def __init__(self, datafolder, datatype='train', idx=[], transform = transforms.Compose([transforms.RandomResizedCrop(128),transforms.ToTensor()]),                 labels_dict={}):
...
for fold_, (tr, val) in enumerate(splits):
    ...
    dataset = iMetDataset(datafolder='../input/imet-2019-fgvc6/train/', datatype='train', idx=tr,             transform=data_transforms_crop, labels_dict=img_class_dict)
    val_set = iMetDataset(datafolder='../input/imet-2019-fgvc6/train/', datatype='train', idx=val,        transform=data_transforms_crop_test, labels_dict=img_class_dict)
    ...
    lr = 1.5e-2
    optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, model_conv.parameters()),                                   lr, weight_decay=0.0002, momentum=0.9)
        ...
        for tr_batch_i, (data, target) in enumerate(train_loader):     
            ...
            if (tr_batch_i+1)%eval_step == 0:  
                ...
                if valid_f2 > valid_f2_max:
                    ...
                    print('Validation f2 increased ({:.6f} --> {:.6f}).  Saving model ...'.format(                    valid_f2_max, valid_f2))
