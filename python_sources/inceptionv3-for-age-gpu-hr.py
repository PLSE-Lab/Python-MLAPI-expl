...
# # Overview
# This is just a simple first attempt at a model using VGG16 as a basis and attempting to do regression directly on the age variable using full-resolution images (512x512) in attempt to match the winning solution [here](https://www.16bit.ai/blog/ml-and-future-of-radiology) which scored an ```mae_months``` on the test set of 4.2
# 
...
valid_gen = flow_from_dataframe(core_idg, valid_df.sample(1500), 
                             path_col = 'path',
                            y_col = 'boneage_zscore', 
                            target_size = IMG_SIZE,
                             color_mode = 'rgb',
