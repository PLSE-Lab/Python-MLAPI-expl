...
# Analyzing FIFA 2017 Player Faces
# --------------------------------
# 
# In this script we will explore a dataset of faces of approximately 1000 top male soccer players and approximately 150 top female soccer players.
...
# 
# Note that this is essentially the same code that I used in [visualizing PCA][1], but the data here is more interesting since it contains human faces.
# 
...
# **Reorganize the data** and show the mean face image and pixel-wise standard deviation image
...
class GaussianModel:
    ...
    def __init__(self, X, numBasisFunctions=10, objectPixels=None):
        ...
        PCAModel = decomposition.IncrementalPCA(n_components=numBasisFunctions, whiten=True, batch_size=512)
...
playerWithPicsData.ix[:,'Name'] = [x for x in listOfPlayerNames]
playerWithPicsData.ix[:,'Sex'] = ['Female' if x else 'Male' for x in isFemale]

for k, currRowInd in enumerate(recordRowInds):
    if currRowInd != 'False':
        playerWithPicsData.ix[k,'Age [years]'] = float(playerData.ix[currRowInd, 'Age'])
        playerWithPicsData.ix[k,'Height [cm]'] = float(playerData.ix[currRowInd, 'Height'].split(' ')[0])
        playerWithPicsData.ix[k,'Weight [kg]'] = float(playerData.ix[currRowInd, 'Weight'].split(' ')[0])
        playerWithPicsData.ix[k,'Nationality'] = playerData.ix[currRowInd, 'Nationality']
...
releventCols = np.array(playerWithPicsData.ix[:,'Sex'].notnull())
targetFeature = np.array(playerWithPicsData.ix[releventCols,'Sex'] == 'Male')[:,np.newaxis]
...
releventCols = np.array(playerWithPicsData.ix[:,'Age [years]'].notnull())
targetFeature = np.array(playerWithPicsData.ix[releventCols,'Age [years]'].tolist())[:,np.newaxis]
...
releventCols = np.array(playerWithPicsData.ix[:,'Height [cm]'].notnull())
targetFeature = np.array(playerWithPicsData.ix[releventCols,'Height [cm]'].tolist())[:,np.newaxis]
...
releventCols = np.array(playerWithPicsData.ix[:,'Weight [kg]'].notnull())
targetFeature = np.array(playerWithPicsData.ix[releventCols,'Weight [kg]'].tolist())[:,np.newaxis]
