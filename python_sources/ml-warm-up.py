...
trans = RFE(estimator=svc, n_features_to_select=k_select).fit(train_data, train_target)
ans = KNeighborsClassifier(n_neighbors=k_neigh).fit(trans.transform(train_data), train_target).predict(trans.transform(test_data))
good = sum([ans[i] == test_target[i] for i in range(len(ans))])
total = len(ans)
...
good = sum([ans[i] == test_target[i] for i in range(len(ans))])
total = len(ans)
print("SelectKBest + SVC + OneHotEncoder", "good =", good, "total =", total, "accuracy =", good / total * 100, "%", file = f)
if good / total > total_acc:
    total_acc = good / total
    total_pair = 6
    total_k_neigh = k_neigh
...
good = sum([ans[i] == test_target[i] for i in range(len(ans))])
total = len(ans)
print("SelectKBest + KNN + OneHotEncoder", "good =", good, "total =", total, "accuracy =", good / total * 100, "%", file = f)
if good / total > total_acc:
    total_acc = good / total
    total_pair = 4
    total_k_neigh = k_neigh
...
if total_pair in [0, 4]:
    trans = SelectKBest(k=total_k_select).fit(train_data, train_target)
    ans = KNeighborsClassifier(n_neighbors=total_k_neigh).fit(trans.transform(train_data), train_target).predict(trans.transform(predict_data))
