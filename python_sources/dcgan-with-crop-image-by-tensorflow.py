...
# The script is based on the book **deep interesing**
...
# The code below is based on [the amazing script.](https://www.kaggle.com/whizzkid/crop-images-using-bounding-box)
...
get_ipython().run_cell_magic('time', '', 'plt.figure(figsize=(10,10))\nfor i,image in enumerate(all_images):\n    im=get_crop_image(image)\n    \n    plt.subplot(3,3,i+1)\n    plt.axis("off")\n    plt.imshow(im)    \n    if(i==8):\n        break')
...
get_ipython().run_cell_magic('time', '', "path = '../input/all-dogs'\ndataset = 'all-dogs'\ndata_path = os.path.join(path, dataset)\nimages = glob.glob(os.path.join(data_path, '*.*')) \nprint(len(images))")
...
get_ipython().run_cell_magic('time', '', "for i in range(last_batch_size):\n    n = np.random.uniform(-1.0, 1.0, [batch_size, z_dim]).astype(np.float32)\n    gen_imgs = sess.run(g, feed_dict={noise: n, is_training: False})\n    gen_imgs = (gen_imgs + 1) / 2\n    imsave(os.path.join(GEN_DIR, f'sample_{n_batches}_{i}.png'), gen_imgs[i])")


# In[ ]:


get_ipython().run_cell_magic('time', '', "shutil.make_archive('images', 'zip', GEN_DIR)")
...
get_ipython().system('rm -rf generated_dogs/')


# In[ ]:


get_ipython().system('rm -rf samples_dogs/')
