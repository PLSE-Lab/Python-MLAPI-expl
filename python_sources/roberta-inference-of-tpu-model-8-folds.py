...
model6.to(device)
model6.load_state_dict(torch.load("../input/tweet-rob-model/model_5.bin"))
model6.eval()

model7 = TweetModel(conf=model_config)
model7.to(device)
model7.load_state_dict(torch.load("../input/tweet-rob-model/model_6.bin"))
model7.eval()

model8 = TweetModel(conf=model_config)
model8.to(device)
model8.load_state_dict(torch.load("../input/tweet-rob-model/model_7.bin"))
model8.eval()
...
with torch.no_grad():
    ...
    for bi, d in enumerate(tk0):
        ...
        outputs_end = (outputs_end1 + outputs_end2 + 
                       outputs_end3 + outputs_end4 + 
