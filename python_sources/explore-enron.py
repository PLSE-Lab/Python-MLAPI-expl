...
# 
# ---
# ### Examine components of the network

# In[ ]:


temp_df=emails_df.copy()
temp_df=temp_df[["Date","From","To","Subject","content"]]
temp_df=temp_df.sample(5000)
temp_df.head()


# **Credits** : jamestollefson
...
G = nx.from_pandas_dataframe(temp_df, 'From', 'To', edge_attr=['Date', 'Subject'])
plot = nv.ArcPlot(G)
plot.draw()
plt.show()
...
contents = ' '.join(emails_df.sample(10000)['content'])
fig, ax = plt.subplots(figsize=(16, 12))
wc = wordcloud.WordCloud(width=800, 
                         height=600, 
                         max_words=200,
