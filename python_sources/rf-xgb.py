...
nb_train_sample = int(len(y) * 0.83)
...
clf = UGradientBoostingClassifier(loss=loss, n_estimators=175, subsample=0.1,
...
submission.to_csv("xgboost_v5.csv", index=False)

print("Make predictions on the test set")
test_probs = (0.24*rf.predict_proba(test[features])[:,1]) + (0.25*gbm.predict(xgb.DMatrix(test[features])))+(0.26*predskeras) + (0.20*fb_preds) #is better
submission = pd.DataFrame({"id": test["id"], "prediction": test_probs})
submission.to_csv("rf_xgboost_keras_flatness_v5_024.csv", index=False)
