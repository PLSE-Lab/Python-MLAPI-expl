...
gp = GP()
y_true = np.cumsum(y, axis=1)
y_pred = np.zeros((y_true.shape[0],199))
y_pred[:,96:96+20] = gp.GrabPredictions(X)
y_pred = np.cumsum(y_pred, axis=1)
tr_s = ((y_true - y_pred) ** 2).sum(axis=1).sum(axis=0) /(199 * y_true.shape[0])
print(tr_s)
...
y_pred = np.zeros((y_true.shape[0],199))
y_pred[:,96:96+20] = gp.GrabPredictions(X)
knn.fit(y_pred[:,96:96+20],y)


# In[ ]:


y_knnpred = knn.predict_proba(y_pred[:,96:96+20])


# In[ ]:


y_knnpred = 1. - np.array([y_knnpred[int(c)][:,0] for c  in range(199) ]).T
...
y_true = np.cumsum(y, axis=1)
y_pred = np.cumsum(y_knnpred, axis=1)
tr_s = ((y_true - y_pred) ** 2).sum(axis=1).sum(axis=0) /(199 * y_true.shape[0])
print(tr_s)
tr_s = np.round(tr_s, 6)
tr_s
...
for (test_df, sample_prediction_df) in iter_test:
    ...
    y_knnpred = knn.predict_proba(ans)
    y_knnpred = 1. - np.array([y_knnpred[int(c)][:,0] for c  in range(199) ]).T
    y_pred = np.clip(np.cumsum(y_knnpred, axis=1), 0, 1).tolist()[0]
