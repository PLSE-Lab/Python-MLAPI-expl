...
# - We will explore all the columns and we'll impute the missing columns with the best fitted value.
...
# We already see that there are NaNs in some columns. So let's see where exactly and how many are they.
...
# ### LotFrontage/LotArea
# A number of values are missing and one possibility would be just  to impute the mean. However, there should actually be a correlation with LotArea, which has no missing values.
...
# Ok, that's not great.We assume that most of them are rectangular, using the square root might be an improvement. 
...
# All missing values are vanished.
