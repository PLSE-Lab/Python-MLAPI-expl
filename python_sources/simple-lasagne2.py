...
def train():
    ...
    learning_rate = theano.shared(np.float32(0.05))
    ...
    hidden1_num_units=425, 
    dropout1_p=0.4,
    hidden2_num_units=425, 
    dropout2_p=0.4,
    ...
    return net, mean, std, pca 
    

def submit(net, mean, std, pca):
    weather = load_weather()
    ...
    #
    out = csv.writer(open("lasagne1.3.csv", "w"))
...
if __name__ == "__main__":
    net, mean, std, pca = train()
    submit(net, mean, std, pca)
    
    from matplotlib import pyplot
    valid_loss1 = np.array([i["valid_loss"] for i in net.train_history_])
