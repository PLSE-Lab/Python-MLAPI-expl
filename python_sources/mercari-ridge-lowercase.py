...
class FMLayer(Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        if K.is_sparse(inputs):
            data = K.to_dense(inputs)
        else:
            data = inputs
        
        data = K.repeat(data, self.factor_rank)
        data = K.permute_dimensions(data, (0, 2, 1))
        
        print(self.kernel)
        print(type(input_shape[0]))
        print(input_shape[0])
        print(K.stack([1, input_shape[0], 1]))
        
        feats = K.repeat(self.kernel, input_shape[0])
        feats = K.permute_dimensions(feats, (1, 0, 2))

        factors = data * feats
        output = (K.square(K.sum(factors, axis=1)) - K.sum(K.square(factors), axis=1)) / 2
...
if __name__ == '__main__':
    ...
    iind, oind = ShuffleSplit(n_splits=1, train_size=0.8, test_size=None, random_state=358).split(y_train).__next__()
    ix = X_train[iind]
    iy = y_train[iind]
    ox = X_train[oind]
    oy = y_train[oind]
    ...
    def get_nn_model():
        inputs = Input(shape=[ix.shape[1]], dtype='float32', sparse=True)
        
        fm = FMLayer(128)(inputs)
        fm1 = Dense(64, activation='relu')(concatenate([inputs, fm]))
        fm2 = Dense(32, activation='relu')(concatenate([inputs, fm, fm1]))
        
        flat = Dense(128, activation='relu')(concatenate([inputs, fm, fm1, fm2]))
        flat = Dense(64, activation='relu')(flat)
        output = Dense(1, activation='relu')(flat)
        
        nn = Model(inputs, output)
        nn.compile(loss='mse', optimizer=Adam(lr=3e-3))
        return nn
        
    model = get_nn_model()
    for i in range(3):
        print(f'epoch {i + 1}')
        model.fit(x=ix, y=iy, batch_size=2**(11 + i), epochs=1, verbose=1)

    # from sklearn.ensemble import BaggingRegressor
    # ridge = Ridge(solver='auto', fit_intercept=True, alpha=0.4, max_iter=200, normalize=False, tol=0.01, random_state=0)
    # model = BaggingRegressor(base_estimator=ridge, n_estimators=12, max_samples=0.8, max_features=0.8, bootstrap=False, random_state=0)
    # model.fit(ix, iy)
    # print(f'[{time() - start_time}] Train Ridge completed. Iterations: {ridge.n_iter_}')
    
    print('train rmlse: %s, valid rmlse: %s' % (
    measure_handler(iy, model.predict(ix)), measure_handler(oy, model.predict(ox))))
