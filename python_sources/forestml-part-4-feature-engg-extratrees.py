...
plot_sfs(seqbacksel_et.get_metric_dict(), kind='std_err');


# In[ ]:


train_X_sbs = seqbacksel_et.transform(train_X)
val_X_sbs = seqbacksel_et.transform(val_X)


# In[ ]:


classifier_et.fit(train_X_sbs, train_y.values.ravel())
...
valsbs_pred = classifier_et.predict(val_X_sbs)
...
X_sbs = seqbacksel_et.transform(X)
test_X_sbs = seqbacksel_et.transform(test_X)


# In[ ]:


classifier_etfin = ExtraTreesClassifier(random_state=42)
classifier_etfin.fit(X_sbs, y.values.ravel())
...
test_ids = test["Id"]
test_pred = classifier_etfin.predict(test_X_sbs)
