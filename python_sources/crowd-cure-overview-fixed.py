...
# # Overview
# **This is the same kernel from Kevin Mander, but just fixing the red rectangles position with MIN function, I have been helped by a medical student. But in some examples it seems it could be wrong, maybe it depends of the annotator. **
...
test_rows = ann_df.sort_values(['anatomy','patientID', 'StudyInstanceUID','seriesUID', 'sliceIndex']).tail(750)
test_rows.T
...
fig, m_axs = plt.subplots(8,2, figsize = (30, 122))
for (_, c_row), c_ax in zip(test_rows.iterrows(), m_axs.flatten()):
    ...
    c_ax.set_title('{patientID} {anatomy}\n{StudyInstanceUID}\n{radiologist_status} {annotator}\n{sliceIndex}'.format(**c_row))
    c_ax.axis('off')
