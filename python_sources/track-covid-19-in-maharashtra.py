...
# 
# 
# - [Population Percentage of Maharashtra](#Population)
# 
# - [Cases in Maharashtra](#Cases)
# 
...
# 
# - [Non virus deaths in Maharashtra](#Non_Virus_Deaths)
# 
...
#Pick the data of Maharashtra

covid = India_covid[India_covid['State/UnionTerritory']=='Maharashtra'].reset_index(drop=True).drop(['State/UnionTerritory', 'Sno', 'Time'], axis = 1)

population = India_population[India_population['State / Union Territory']=='Maharashtra'].reset_index(drop=True).drop(['State / Union Territory', 'Sno'], axis = 1)

ICMRTestingLabs = ICMRTestingLabs[ICMRTestingLabs['state']=='Maharashtra'].reset_index(drop=True).drop('state', axis = 1)

HospitalBeds = HospitalBeds[HospitalBeds['State/UT']=='Maharashtra'].reset_index(drop=True).drop(['State/UT', 'Sno'], axis = 1)

TestingDetails = TestingDetails[TestingDetails['State']=='Maharashtra'].drop('State', axis = 1)
...
for i in range(0, len(covid)):
    if((i>=0 and i<=3) or (i>=23 and i<=34) or (i>=53 and i<=64) or (i>=84 and i<=95)):
...
# # **Population Percentage of Maharashtra**  <a name = "Population"> </a>

# In[ ]:


Total = India_population['Population'].sum()
MH = population['Population']
labels = ["Rest of the India", 'MH']
values = [float(Total - MH), float(MH)]
...
fig.update_layout(title_text= 'Population of Maharashtra', showlegend = False)

fig.show()


# # **Cases in Maharashtra** <a name = 'Cases'></a>
...
MH_confirmed = covid.Confirmed.iloc[-1]
MH_deaths = covid.Deaths.iloc[-1]
MH_cured = covid.Cured.iloc[-1]


confirm = [Total_confirmed, MH_confirmed]
cured = [Total_cured, MH_cured]
deaths = [Total_deaths, MH_deaths]
...
# Define pie charts
fig.add_trace(go.Pie(title = dict(text = 'Confirmed', position = 'top left', 
...
fig.add_trace(go.Pie(title = dict(text = 'Cured', position = 'top left', font = dict(color = 'rgb(0,0,0)')), labels=labels, values= cured, name='Cured', textinfo='label+ percent',
                     marker_colors=colors, pull = [.05,0],  insidetextorientation = 'horizontal'), 1, 2)
fig.add_trace(go.Pie(title = dict(text = 'Deaths', position = 'top left', font = dict(color = 'rgb(0,0,0)')), labels=labels, values=deaths, name='Deaths', textinfo='label+ percent',
...
# # **Confirmed Cases in Maharashtra** <a name= 'Confirmed'></a>
...
# # **Active Cases in Maharashtra** <a name = 'Active'></a>
...
# # **Covid Deaths in Maharashtra** <a name = 'Deaths'></a>
...
# # **Cured Cases in Maharashtra** <a name = "Cured"> </a>
...
# # **Non virus deaths in Maharashtra** <a name = "Non_Virus_Deaths"></a>
...
MH = non_virus_deaths.groupby('state')['deaths'].sum()['MH']
...
MH = non_virus_deaths[non_virus_deaths['state']=='MH']['reason'].sum()
...
MH_reason = pd.DataFrame(group_list(MH), columns = ['Reason', 'Count'])
MH_reason.sort_values(by = ['Count'], ascending = False, inplace = True)
MH_reason.reset_index(drop = True, inplace = True)


Total_total_count = Total_reason['Count'].sum()
MH_total_count = MH_reason['Count'].sum()
...
MH_other = MH_total_count - (MH_reason['Count'][0] + MH_reason['Count'][1]
                             + MH_reason['Count'][2] + MH_reason['Count'][3] + MH_reason['Count'][4])
...
MH_y_data = [int(MH_reason['Count'][0]/MH_total_count*100), int(MH_reason['Count'][1]/MH_total_count*100),
           int(MH_reason['Count'][2]/MH_total_count*100), int(MH_reason['Count'][3]/MH_total_count*100),
           int(MH_reason['Count'][4]/MH_total_count*100), int(MH_other/MH_total_count*100)]
...
MH_x_data = [MH_reason['Reason'][0], MH_reason['Reason'][1], MH_reason['Reason'][2], MH_reason['Reason'][3], MH_reason['Reason'][4], 'Other']
...
fig.append_trace(go.Bar(
    ...
    name='Non Virus Deaths in Maharashtra',
    orientation='h',
), 1, 2)
...
MH_y = np.array(MH_y_data)

# Adding labels
for ydn, yd, xdn, xd in zip(MH_y, Total_y, MH_x_data,  Total_x_data):
