...
timesData.university_name.value_counts()
...
layout = dict(title = 'Citation vs world rank of top 100 universities with 2011, 2012 and 2011 years',
              xaxis= dict(title= 'World Rank',ticklen= 5,zeroline= False),
...
import matplotlib.pyplot as plt
# prepare data frames
df2012 = timesData[timesData.year == 2012].iloc[:7,:]

y_saving = [each for each in df2012.research]
y_net_worth  = [float(each) for each in df2012.income]
x_saving = [each for each in df2012.university_name]
x_net_worth  = [each for each in df2012.university_name]
trace0 = go.Bar(
                x=y_saving,
                y=x_saving,
...
pie1_list = [float(each.replace(',', '.')) for each in df2014.num_students]  # str(2,4) => str(2.4) = > float(2.4) = 2.4
...
# data preparation
df2012 = timesData[timesData.year == 2012].iloc[:20,:]
num_students_size  = [float(each.replace(',', '.')) for each in df2012.num_students]
...
trace0 = go.Box(
    y=x2012.total_score,
    name = 'total score of universities in 2012',
    marker = dict(
        color = 'rgb(12, 12, 140)',
    )
)
