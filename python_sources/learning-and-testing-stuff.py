...
def munge(df):
    ...
    for key, group in lot_frontage_by_neighborhood:
        ...
        all_df.loc[idx, 'LotFrontage'] = 25 # replace with reasonable min    
    ...
    all_df['BsmtExposure'] = df['BsmtExposure'].map(
        {None: 0, 'No': 1, 'Mn': 2, 'Av': 3, 'Gd': 5}).astype(int)
    ...
    all_df['GarageFinish'] = df['GarageFinish'].map(
        {None: 0, 'Unf': 1, 'RFn': 2, 'Fin': 5}).astype(int)
    ...
    all_df['GarageYrBlt'].fillna(1950, inplace=True)
    ...
    all_df['BadHeating'] = df.HeatingQC.replace(
        {'Ex': -10, 'Gd': 1, 'TA': 2, 'Fa': 3, 'Po': 15})

    area_cols = [ 'MasVnrArea', 'BsmtFinSF1', 'BsmtFinSF2', 'BsmtUnfSF',
                 'TotalBsmtSF', '1stFlrSF', '2ndFlrSF', 'GrLivArea', 'GarageArea', 'WoodDeckSF', 
                 'OpenPorchSF', 'EnclosedPorch', '3SsnPorch', 'ScreenPorch', 'LowQualFinSF' ]
    all_df['TotalArea'] = all_df[area_cols].sum(axis=1)

    all_df['TotalArea1st2nd'] = all_df['1stFlrSF'] + all_df['2ndFlrSF'] * 5 # weird multiplier
...
best_alpha = 0.00098
regr2 = Lasso(alpha=best_alpha, max_iter=100000)

regr3 = ElasticNet(alpha=0.003)

regr4 = KernelRidge(alpha=0.3, kernel='polynomial', degree=3, coef0=1.9)
