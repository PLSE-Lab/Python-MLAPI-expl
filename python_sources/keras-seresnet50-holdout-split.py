...
def create_model():
    ...
    base_model =  seresnet50(weights = 'imagenet', include_top = False, input_shape = SHAPE)
    x = base_model.output
...
history = model.fit_generator(generator = data_generator_train,
                    ...
                    epochs = 13,
                    callbacks = [ModelCheckpointFull('seresnet50_holdout_model.h5')],
                    verbose = 1)
...
plt.savefig('seresnet50_loss_performance.png')
plt.clf()
...
plt.savefig('seresnet50_acc_performance.png')
plt.clf()

preds = predictions(test_df, model)
np.save('seresnet50_saved_preds.npy', preds)
...
test_df.to_csv('seresnet50_holdout_submission.csv', index = False)
