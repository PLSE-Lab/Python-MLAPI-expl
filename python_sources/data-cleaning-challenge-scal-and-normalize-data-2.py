...
'''Only the is pledged contains any non number entries.Dataset is relatively clean.
At some point i think i have explored this dataset to some extent.'''
print(kickstarters_2017.isnull().sum())
...
#                                                Sample Responses
# 
# For the First Example,Normalization makes more sense as we are trying to build a regression model which expects the data to be presented as a normal distribution.
# 
# While for the second example,Scaling makes more sense as the gap between the jumping jacks is present and also we can scale both the exercises in terms of calories exherted which i think would be close for both the cases.
...
'''Has almost the same information.Can Probably discard this column since the data is simillar?'''
...
# Try this tomorrow as practice(JFF).
