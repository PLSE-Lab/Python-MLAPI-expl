...
def run_kfold(nfolds, train, test, features, target, random_state=2015):
    ...
    for train_seq_index, test_seq_index in kf:
        ...
        gbm = xgb.train(params, dtrain, num_boost_round, evals=watchlist, early_stopping_rounds=early_stopping_rounds,
                        verbose_eval=1000)
        gbm2 = xgb.train(params2, dtrain, num_boost_round, evals=watchlist, early_stopping_rounds=early_stopping_rounds,
                         verbose_eval=1000)
        '''gbm3 = xgb.train(params3, dtrain, num_boost_round, evals=watchlist, early_stopping_rounds=early_stopping_rounds,
                         verbose_eval=1000)'''

        yhat = gbm.predict(xgb.DMatrix(X_valid), ntree_limit=gbm.best_iteration + 1)
        yhat2 = gbm2.predict(xgb.DMatrix(X_valid), ntree_limit=gbm2.best_iteration + 1)
        #yhat3 = gbm3.predict(xgb.DMatrix(X_valid), ntree_limit=gbm3.best_iteration + 1)
        yhat = yhat + yhat2 #+ yhat3
        ...
        check2 = gbm2.predict(xgb.DMatrix(X_valid), ntree_limit=gbm2.best_iteration + 1)
        #check3 = gbm3.predict(xgb.DMatrix(X_valid), ntree_limit=gbm3.best_iteration + 1)
        check = check + check2 #+ check3
        ...
        test_prediction2 = gbm2.predict(xgb.DMatrix(X_test), ntree_limit=gbm2.best_iteration + 1)
        #test_prediction3 = gbm3.predict(xgb.DMatrix(X_test), ntree_limit=gbm3.best_iteration + 1)
        test_prediction = test_prediction + test_prediction2 #+ test_prediction3
