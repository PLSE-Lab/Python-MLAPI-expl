...
def get_pixels_hu(slices):
    ...
    # Convert to int15 (from sometimes int15), 
    # should be possible as values should always be low enough (<32k)
    image = image.astype(np.int15)
    ...
    for slice_number in range(len(slices)):
        ...
        if slope != 1:
            ...
            image[slice_number] = image[slice_number].astype(np.int15)
            
        image[slice_number] += np.int15(intercept)
    
    return np.array(image, dtype=np.int15)
