...
testing=np.indices((len(ratings.userId.unique()),len(ratings.userId.unique())))+1
grid=testing[0]
#print(grid)
up=np.triu_indices(11,1)
testx=grid[up]
len(testx)
gridy=testing[1]
print(gridy)
...
trace1 = go.Scatter3d(
    x=testx,
    y=testy,
    z=itus.values[np.triu_indices(len(ratings.userId.unique()),1)],
    mode='markers',
    marker=dict(
        size=12,
        line=dict(
