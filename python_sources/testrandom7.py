...
def create_model(inputSize):
    model = Sequential()
    model.add(ZeroPadding2D((1, 1), input_shape=(3, inputSize, inputSize), dim_ordering='th'))
...
if __name__ == '__main__':
   ...
   #----------------------------------------------------------------------------------------------------
    ...
    datagen = ImageDataGenerator(featurewise_center=True,dim_ordering='th')#Sinon en th 
    #---Centralisation--Renormalisation--Bruit-Blanc------
    print("preprocessing")
    #datagen.fit(train_data)
    #---------Train---batch------------------------------------------------
    model.compile(loss='categorical_crossentropy', optimizer='sgd', metrics=['accuracy'])
    for epoch in range(1,nb_epoch+1):#-----Permute les images à chaque epoch
        print("-----------------------Epoch=",epoch)
        print("-----------------------------------")
        indicesRandom = np.random.permutation(train_data.shape[0])# Permutation aléatoire de la premiere colonne (shape([0]) donne un vecteur 
        #training_idx, val_idx = indices[:90], indices[90:]# Train indices, Test indices
        training = train_data[indicesRandom,:]
        label = train_target[indicesRandom,:] 
        #model.train_on_batch(training,label)
        model.fit(training, label, nb_epoch=1, batch_size=32)
        # loss_and_metrics = model.evaluate(X_test, Y_test, batch_size=32) Pour evaluer la qualité du modele
    model.fit(train_data,train_target , nb_epoch=4, batch_size=256)    
    print("test")
    #----------------Tests-------------------------
