...
# Inspired by 
# 
# * Alex Baseline kernel - https://www.kaggle.com/shonenkov/train-inference-gpu-baseline
# * Ivan's Improved version - https://www.kaggle.com/shonenkov/train-inference-gpu-baseline
# 
# I am creating an ensemble version to include both the versions and ensemble both models.
...
submission_1 = pd.DataFrame(result)
submission_1.head(5)


# Second Model : Improvement over Alex Version

# In[ ]:


class TrainGlobalConfig_2:
    num_workers = 4
    batch_size = 8
    n_epochs = 25
    lr = 0.002
...
def run_training_2():
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        pin_memory=False,
        drop_last=True,
        num_workers=TrainGlobalConfig_2.num_workers,
    )
    val_loader = torch.utils.data.DataLoader(
        validation_dataset, 
        batch_size=TrainGlobalConfig_2.batch_size,
    ...
    fitter = Fitter(model=net, device=device, config=TrainGlobalConfig_2)
...
for mode in range(0,4):
    submissions_2[mode].to_csv(f'submission_{mode}.csv', index=False)


# In[ ]:


submissions_2[0]['Label'] = (submissions_2[0]['Label']*3 + submissions_2[1]['Label'] + submissions_2[2]['Label'] + submissions_2[3]['Label']) / 6


# In[ ]:


submissions_2[0].head(5)
...
submissions_2[0]['Label'] = (submissions_2[0]['Label'] * 0.72289157 + submission_1['Label'] * 0.27710843)
submissions_2[0].head(5)
...
submissions_2[0].to_csv(f'submission.csv', index=False)
