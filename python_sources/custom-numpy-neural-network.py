...
class SchlerpReLU(Activation):
    ...
    def forward(self, x):
        ...
        if self.stable:
            x = np.clip(x, -700, 700)
        return ((x * self.alpha_pos) + self.ystep_pos) * (x > self.xstep_pos) +                ((self.m*x)+self.c) * (np.logical_and(self.xstep_neg <= x, self.c <= self.xstep_pos)) +                ((x * self.alpha_neg) + self.ystep_neg) * (x < self.xstep_neg)
    
    def backward(self, x):
     
        return self.alpha_pos * (x > self.xstep_pos) +                self.m * (np.logical_and(self.xstep_neg <= x, self.c <= self.xstep_pos)) +                self.alpha_neg * (x < self.xstep_neg)
...
class SchlerpTanh(Activation):
    ...
    def __init__(self, stable=True, pos_alpha=0.1, neg_alpha=0.01):
        ...
        self.pos_y_step = np.tanh(self.pos_x_step) -                           (self.pos_x_step * self.pos_alpha)
        self.neg_y_step = np.tanh(self.neg_x_step) -                           (self.neg_x_step * self.neg_alpha)
    
    def forward(self, x):
        if self.stable:
            x = np.clip(x, -700, 700)
        return ((x * self.pos_alpha) + self.pos_y_step) * (x > self.pos_x_step) +                np.tanh(x) * (np.logical_and(self.neg_x_step <= x, x <= self.pos_x_step)) +                ((x * self.neg_alpha) + self.neg_y_step) * (x < self.neg_x_step)
    
    def backward(self, x):
        return self.pos_alpha * (x > self.pos_x_step) +                (1.0 - np.square(np.tanh(x))) * (np.logical_and(self.neg_x_step <= x, x <= self.pos_x_step)) +                self.neg_alpha * (x < self.neg_x_step)
