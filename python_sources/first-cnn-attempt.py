...
X_train_batches = X_train.reshape(-1, Nbatchsize, 400)
y_train_batches = y_train.reshape(-1, Nbatchsize, 12)

Nbatches = len(X_train_batches)

# Following code largely adapted from the TensorFlow MNIST example
...
with tf.Session() as sess:
    ...
    for ee in range(Nepochs):
        for bb in range(Nbatches):
            if bb % 100 == 0:
                train_accuracy = accuracy.eval(feed_dict={x: X_train_batches[bb], y_: y_train_batches[bb], keep_prob: 1.0})
                print('[epoch %d batch %d] training accuracy: %g' % (ee, bb, train_accuracy))
            train_step.run(feed_dict={x: X_train_batches[bb], y_: y_train_batches[bb], keep_prob: 0.5})
