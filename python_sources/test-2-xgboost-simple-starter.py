__author__ = 'oekekezie: https://kaggle.com/oekekezie'
...
def map_column(table, f):
    ...
    for i in range(len(labels)):
        mappings[labels[i]] = i
    table.replace({f: mappings}, inplace=True)
...
def read_train_test():
    ...
    map_column(pbd, 'phone_brand')
    map_column(pbd, 'device_model')

    # Train
    print('Read train...')
    train = pd.read_csv("../input/gender_age_train.csv", dtype={'device_id': np.str})
    map_column(train, 'group')
...
print('Features [{}]: {}'.format(len(train.columns.values), sorted(train.columns.values)))
