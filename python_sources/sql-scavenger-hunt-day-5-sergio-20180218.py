...
# You can use two dashes (--) to add comments in SQL
query = """
        WITH commits_puthon AS
       (SELECT DISTINCT repo_name
          FROM `bigquery-public-data.github_repos.sample_files`
         WHERE path LIKE '%.py')
        SELECT cf.repo_name, COUNT(cf.commit) AS number_of_files
        FROM `bigquery-public-data.github_repos.sample_commits` as cf
        INNER JOIN commits_puthon AS L 
            ON L.repo_name = cf.repo_name
