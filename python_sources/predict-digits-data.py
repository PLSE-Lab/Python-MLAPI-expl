...
StepSize = 180 # Number of images used in each optimization step
...
def DefineModel () :
    ...
    CNNPredictor.compile(loss='categorical_crossentropy', optimizer="sgd", metrics=["accuracy"])
    return CNNPredictor
...
CNNPredictor.fit(XTrain, YTrain, batch_size=StepSize, nb_epoch=nb_epoch, verbose=2, validation_split=.1, callbacks=[early_stop])
