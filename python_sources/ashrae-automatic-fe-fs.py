...
# # ASHRAE: Automatic FE: Featuretools & FeatureSelection - check a new features

# Recently, I've been trying to adapt the kernel capabilities of the "Titanic: Machine Learning from Disaster" to this competition. And now I have some interesting **new ideas for features that can complement the best of the existing solutions for this competition**: median and skew of meteofeatures.
...
dfe = train.sample(n=5000, replace=True, random_state=1)
...
FE_all = set(FE_option1).union(set(FE_option2))
len(FE_all)
...
FE_general = set.intersection(set(FE_option1), set(FE_option2))
len(FE_all)
