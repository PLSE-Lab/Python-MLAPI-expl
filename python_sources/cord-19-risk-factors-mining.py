...
# # **INTRODUCTION**
# 
# This work is to help the medical community answer the posted question in Kaggle: [What is known about COVID-19 risk factors?](https://www.kaggle.com/allen-institute-for-ai/CORD-19-research-challenge/tasks) The resulting model enables to understand and keep up with the large amount of literature contained in the provided dataset, specifically:
# 
# 1. Data on potential risks factors
#     1. Smoking, pre-existing pulmonary disease
#     1. Co-infections (determine whether co-existing respiratory/viral infections make the virus more transmissible or virulent) and other co-morbidities
#     1. Neonates and pregnant women
#     1. Socio-economic and behavioral factors to understand the economic impact of the virus and whether there were differences.
...
terms2 = ['Basic reproductive number','Incubation period',
          'Serial interval','Modes of transmission',
          'Environmental factors',
          'Risk of fatality among symptomatic hospitalized patients',
          'Risk of fatality','Symptomatic patients',
          'Hospitalized patients',
          'Symptomatic hospitalized patients']

terms3 = ["High-risk groups","High risk patient",
          "Environmental factors"]

terms4 = ["High-risk population","Age","Sex"]

terms5 = ["Social distancing","Recommended measures","Travel"]

terms = terms11 + terms12 + terms13 + terms14
terms += terms2 + terms3 + terms4 + terms5
...
ax = cnt_s.plot(kind='barh', figsize=(12,25), 
                ...
                fontsize=16)
ax.set_alpha(0.8)
ax.set_title("What Is Known About COVID-19 Risk Factors?",
             fontsize=18)
...
td11 = 'Smoking, pre-existing pulmonary disease'
...
td13 = 'Neonates and pregnant women'
text = get_question_answer(terms13)
ta13 = get_text_area(text)

td14 = 'Socio-economic and behavioral factors'
text = get_question_answer(terms14)
ta14 = get_text_area(text)
                  
td2 = 'Transmission dynamics of the virus, including the basic reproductive number, incubation period, serial interval, modes of transmission and environmental factors'
text = get_question_answer(terms2)
ta2 = get_text_area(text)

td3 = 'Severity of disease, including risk of fatality among symptomatic hospitalized patients, and high-risk patient groups' 
text = get_question_answer(terms3)
ta3 = get_text_area(text)

td4 = 'Susceptibility of populations'
text = get_question_answer(terms4)
ta4 = get_text_area(text)

td5 = 'Public health mitigation measures that could be effective for control'
...
ac1_tas = [ta11,ta12,ta13,ta14]
ac1 = widgets.Accordion(children=ac1_tas)
ac1.set_title(0, td11)
ac1.set_title(1, td12)
ac1.set_title(2, td13)
ac1.set_title(3, td14)


# In[ ]:


ac2_tas = [ta2,ta3,ta4,ta5]
ac2 = widgets.Accordion(children=ac2_tas)
ac2.set_title(0, td2)
ac2.set_title(1, td3)
ac2.set_title(2, td4)
ac2.set_title(3, td5)
