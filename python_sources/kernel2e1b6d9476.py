...
print(df_test.shape)
# train_x, valid_x, train_y, valid_y = model_selection.train_test_split(df_train['text'], df_train['target'])

# label encode the target variable 
encoder = preprocessing.LabelEncoder()
df_train['target'] = encoder.fit_transform(df_train['target'])
token_pattern = "[\S]+"
count_vect = CountVectorizer(analyzer='word', token_pattern=token_pattern)
count_vect.fit(df_train['text'])
# transform the training and validation data using count vectorizer object
xtrain_count =  count_vect.transform(df_train['text'])
xvalid_count =  count_vect.transform(df_test['text'])
# word level tf-idf
tfidf_vect = TfidfVectorizer(analyzer='word', token_pattern=token_pattern, max_features=5000)
tfidf_vect.fit(df_train['text'])
xtrain_tfidf =  tfidf_vect.transform(df_train['text'])
xvalid_tfidf =  tfidf_vect.transform(df_test['text'])
D_train = xgboost.DMatrix(xtrain_tfidf, label=df_train['target'])
D_test = xgboost.DMatrix(xvalid_tfidf)
param = {
    'eta': 0.3, 
    'max_depth': 12,  
