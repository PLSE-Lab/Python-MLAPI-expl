...
#             'Fireplaces','HalfBath','GrLivArea','YearRemodAdd','TotalBsmtSF','BsmtFinSF1',
#            'BsmtFinSF2','BsmtUnfSF','BsmtFullBath','MiscVal','YrSold','WoodDeckSF']
X = home_data[numeric_cols]
X.drop(['Id'],axis = 1, inplace= True)
# my_model = SimpleImputer(strategy='median')
# X_new= pd.DataFrame(my_model.fit_transform(X))
X = X.fillna(X.mean())
...
# iowa_model = DecisionTreeRegressor(max_leaf_nodes=318, random_state=1)
# iowa_model.fit(train_X,train_y)
# val_predictions = iowa_model.predict(val_X)
# val_mae = mean_absolute_error(val_predictions, val_y)
...
# this code help we find the best_tree_size
...
rf_model_on_full_data = RandomForestRegressor(max_leaf_nodes=332, random_state=1)
...
test_preds = rf_model_on_full_data.predict(test_X1)
