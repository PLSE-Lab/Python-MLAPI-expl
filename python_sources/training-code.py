...
nb_classes = 18  # number of classes
...
def train(train_data_dir, validation_data_dir, model_path):
    ...
    model.fit_generator(train_generator,
                        samples_per_epoch=train_generator.samples//16,
                        nb_epoch=nb_epoch / 5,
                        validation_data=validation_generator,
                        nb_val_samples=validation_generator.samples//16,
...
def predictimage(image_path):
    ...
    inputShape = (299,299) # Assumes 3 channel image
...
if __name__ == '__main__':
    ...
    for i in range(1, 3):
        imagepath="data/test/test_"+str(i)+".jpg"
        newdata = [i,predictimage(imagepath)]
        submission.append(newdata)
        print(newdata)
    
    df = pd.DataFrame(submission,columns=['id','category'])
    df.to_csv('firstsubmission.csv', index=False)
