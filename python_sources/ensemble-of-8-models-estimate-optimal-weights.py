...
# ## Ensemble of 8 models - estimate optimal weights

# Objective of this notebook is to improve the notebook published by Alex Lekov given below. In the notebook it is not clear how the author determined the weights. We tried to improve the solution by applying optimization to determine the best weights.
# 
...
# ### Applying Optimization to determine best weights
...
def blend_models_predict(X1, **dd):
    return (dd['a']*elasticnet.predict(X1) + dd['b']*lasso.predict(X1) + 
            ...
            dd['g']*lightgbm.predict(X1) + dd['h']*stack_gen.predict(X1.values))


# In[ ]:


subm = np.exp(blend_models_predict(X_sub, **dd))
