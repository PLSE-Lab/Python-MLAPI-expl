...
df1 = pd.read_csv("../input/SPAM text message 20170820 - Data.csv")
df = df1.where((pd.notnull(df1)), '')

# Categorize Spam as 0 and Not spam as 1 for later comparison
df.loc[df["Category"] == 'ham', "Category",] = 1
df.loc[df["Category"] == 'spam', "Category",] = 0
# split data as label and text . System should be capable of predicting the label based on the  text
df_x = df['Message']
df_y = df['Category']
# split the table - 70 percent for training and 20 percent for test size
