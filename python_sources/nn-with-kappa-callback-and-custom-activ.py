...
class clsvalidation_kappa(Callback):  #inherits from Callback
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        current = ml_metrics.quadratic_weighted_kappa(self.y_val.values.ravel(), p.astype(int).ravel())
...
def out_activ(x):
    return T.ceil(T.abs_(T.tanh(x)) * 8)
...
class NN:
    ...
    def __init__(self, inputShape, layers, dropout = [], activation = 'relu', init = 'uniform', loss = 'rmse', optimizer = 'adadelta', nb_epochs = 50, batch_size = 32, verbose = 1):
        ...
        model.add(Dense(1, init = init, activation=out_activ)) #End in a single output node for regression style output
    ...
    def fit(self, X, y):
        self.model.fit(X.values, y.values, nb_epoch=self.nb_epochs, batch_size=self.batch_size,
                       verbose = self.verbose, callbacks=[val_call])
...
sss = StratifiedShuffleSplit(labels, n_iter=1, test_size=0.2, random_state=7)
train_index, test_index = list(sss)[0]
train, X_test = train.iloc[train_index], train.iloc[test_index]
labels, y_test = labels.iloc[train_index], labels.iloc[test_index]

# Instantiate the callback and NN
val_call = clsvalidation_kappa(validation_data=(X_test, y_test), patience=3) #instantiate object
    
clf = NN(inputShape = train.shape[1], layers = [128, 64], dropout = [0.5, 0.5], loss='mae', optimizer = 'adadelta', init = 'glorot_normal', nb_epochs = 5, verbose = 0)
...
predClipped = pred.astype(int) #convert to int
