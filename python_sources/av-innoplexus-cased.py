...
for i in range(5):
    ...
    learner.fit_one_cycle(6,3e-5,callbacks=cb)
    ...
    if i==4:
        get_ipython().system('rm -r /kaggle/working/models/best-model_4.pth')
...
bagg_preds2.to_csv('raw_preds_bert_cased.csv',index=False)
pd.DataFrame({'unique_hash':df_test.unique_hash,'sentiment':preds2}).to_csv('sub__bert_cased.csv',index=False)
