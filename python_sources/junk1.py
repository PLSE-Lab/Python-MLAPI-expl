...
model = xgb.train(dict(xgb_params, silent=0), dtrain, num_boost_round=374)
...
y_train = train["price_doc"] * mult + 15
x_train = train.drop(["id", "timestamp", "price_doc"], axis=1)
x_test = test.drop(["id", "timestamp"], axis=1)
...
first_result["price_doc"] = np.exp( .7135*np.log(first_result.price_doc_louis) + 
                                    .2865*np.log(first_result.price_doc_bruno) )
result = first_result.merge(gunja_output, on="id", suffixes=['_follow','_gunja'])

result["price_doc"] = np.exp( .715*np.log(result.price_doc_follow) + 
                              .285*np.log(result.price_doc_gunja) )
