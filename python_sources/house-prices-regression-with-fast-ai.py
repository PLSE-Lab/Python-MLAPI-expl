...
class Clean(TabularProc):
    def apply_train(self, df:DataFrame):
        for hasna in ['BsmtFinSF1', 'BsmtFinSF2', 'BsmtUnfSF', 'BsmtUnfSF', 'BsmtFullBath', 'BsmtHalfBath', 'GarageCars', 'GarageArea']:
            df[hasna] = df[hasna].fillna(0)
...
def root_mean_squared_logarithmic_error(pred:Tensor, targ:Tensor)->Rank0Tensor:
    return torch.sqrt(mean_squared_logarithmic_error(pred, targ))
...
learn.loss = root_mean_squared_logarithmic_error
learn.lr_find()
...
log_predictions, *_ = learn.get_preds(DatasetType.Test)
predictions = log_predictions.exp()[0:].flatten().numpy()
predictions
...
res_df = pd.DataFrame({'Id': test_df['Id'], 'SalePrice': predictions})
res_df.to_csv('housing_submission.csv', index=False)
