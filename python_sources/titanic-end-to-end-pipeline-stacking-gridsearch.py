...
# from sklearn.model_selection import KFold, ShuffleSplit #compat issue with sklearn pandas "object is not iterable"
...
train = pd.read_csv('../input/train.csv', delimiter=',', header=0) #Can't use column 0 Passenger ID as index, it's disappearing in the transform proc'
test = pd.read_csv('../input/test.csv', delimiter=',', header=0)
...
mapper = DataFrameMapper([
  ...
  (['Age'], StandardScaler()),
   ...
   ('CptDeck', LabelBinarizer()),
    (['CptTitleCat'], StandardScaler()),
    ('CptTitleLabel', LabelBinarizer()),
#    ('CptName', LabelBinarizer()),
#    (['CptNameFreq'], RobustScaler()),
    (['CptFamSize'], RobustScaler()),
    ('CptFamType', LabelBinarizer()),
    (['CptFareGroup'], RobustScaler()),
    (['CptAgeGroup'], StandardScaler()),
    (['CptFarePerson'], StandardScaler()),
   ('Embarked', LabelBinarizer()),
    # (['CptTicketFreq'], RobustScaler()),
   (['CptAgeClass'], RobustScaler())
    ])
...
clf_et = ExtraTreesClassifier( #0.78469
        n_estimators=1000,
        min_samples_split=4,
        min_samples_leaf=4,
        criterion= 'entropy',
...
clf_xgb = XGBClassifier( #0.74 :? crossv 0.84 std +/- 0.01
        n_estimators=30,
        min_child_weight=3,
        max_depth=7,
        learning_rate=0.2,
        colsample_bytree=0.9, #avoid overfitting
        subsample=0.5, #avoid overfitting
        gamma=0
        )

clf_rf = RandomForestClassifier( #0.78
        n_estimators=150,
        min_samples_split=4,
        min_samples_leaf=4,
        criterion= 'entropy',
...
clf_svc = SVC() #0.78469
clf_lr = LogisticRegression() #0.77990 cv 0.824 std +/- 0.023
clf_nb = GaussianNB()
clf_kn = KNeighborsClassifier()

stack_lr = LogisticRegression()

# clf_vote = EnsembleVoteClassifier(clfs=[clf_et, clf_xgb, clf_rf, clf_svc, clf_lr], weights=[1.5,2,1,1,2])

clf_stack = StackingClassifier([clf_et, clf_xgb, clf_rf, clf_svc, clf_lr, clf_nb, clf_kn],stack_lr)
...
def gridsearch():
    ...
    #         }
            ...
            # 30}
    param_grid = {
                    'xgboost__n_estimators' : [10,30,100, 150,200,300,1000],
            ...
            # GridSearchCV took 8324.85 seconds for 6804 candidate parameter settings.
            # Model with rank: 1
            # Mean validation score: 0.846 (std: 0.013)
            # Parameters: {'xgboost__colsample_bytree': 0.5, 'xgboost__min_child_weight': 1, 'xgboost__max_depth': 3, 'xgboost__n_estimato
            # rs': 100, 'xgboost__gamma': 0, 'xgboost__learning_rate': 0.2, 'xgboost__subsample': 1.0}

            # Model with rank: 1
            # Mean validation score: 0.846 (std: 0.020)
            # Parameters: {'xgboost__colsample_bytree': 0.7, 'xgboost__min_child_weight': 3, 'xgboost__max_depth': 5, 'xgboost__n_estimato
            # rs': 30, 'xgboost__gamma': 0.2, 'xgboost__learning_rate': 0.3, 'xgboost__subsample': 0.5}

            # Model with rank: 1
            # Mean validation score: 0.846 (std: 0.017)
            # Parameters: {'xgboost__colsample_bytree': 0.9, 'xgboost__min_child_weight': 3, 'xgboost__max_depth': 7, 'xgboost__n_estimato
            # rs': 30, 'xgboost__gamma': 0, 'xgboost__learning_rate': 0.2, 'xgboost__subsample': 0.5}

            # Model with rank: 1
            # Mean validation score: 0.846 (std: 0.008)
            # Parameters: {'xgboost__colsample_bytree': 0.9, 'xgboost__min_child_weight': 5, 'xgboost__max_depth': 7, 'xgboost__n_estimato
            # rs': 200, 'xgboost__gamma': 0.2, 'xgboost__learning_rate': 0.1, 'xgboost__subsample': 1.0}
