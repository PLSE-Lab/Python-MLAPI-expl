...
print('Not outlier interval:', [lower_bound, upper_bound])


# In[ ]:


print('Number of outliers:',np.sum((train['GrLivArea'] > upper_bound) | (train['GrLivArea'] < lower_bound)))
...
# We will now handle numerical missing values by imputing with the mean.
...
# Backward elimination has deleted 176 variables
...
print('Number of variables not eliminated:',len(opt_col))


# We have reduced our variables to 140 independent variables

# We will now run a linear regression on these variables only and compare error with previous model
#     
# We are going to remain consistent and have a 2/3 of the data for our training set and 1/3 for the testing set.
...
# 
# This model also over estimates the target variable at a few observations.
...
# 
# The test error still approximately doubles in comparison to the the training error, this is problematic as our model is not able to accurately predict unseen data. This could be because since Ridge only reduces coefficients to asyptotically zero and therefore all variables are still significant.
...
# ### Removing outliers
# 
# We initially didn't cater for outliers and kept them in our model, which resulted in similar results as above. After detecting and then removing outliers as shown in this notebook, all of our model's errors decreased and $R^2$ improved. We have therefore, however, chosen not to include those workings in this notebook because of time constraints and it would have been repetition of what we have displayed. Our take home though is that, it is important to remove outliers in order to get more acurate predictions.
# 
# ### Next steps
# 
# In moving forward and to improve our predicted model, we have noted that models such as NetElastic could achieve this as it is a combination of Ridge and Lasso regression and manages to balance the choice of completely eliminating a variable by reducing its coefficient to zero and keeping a variable by only reducing it to asymptotically close to zero.
# 
# By further exploration, we could also look into models such as XGBoost and Gradient Descent which reduces the residual error and therefore are great for prediction but their results are not easily interpretable.
