...
class Ball(object):
    ...
    def update(self, paddle, brickwall):
        ...
        if ((ballX + self._radius) >= paddleX and ballX <= (paddleX + paddleW))         and ((ballY + self._radius) >= paddleY and ballY <= (paddleY + paddleH)):
            self.__yVel *= -1
...
class Brick (pygame.sprite.Sprite):
    ...
    def collide(self, ball):
        ...
        if ((ballX + ball._radius) >= brickX and (ballX+ball._radius) <= (brickX + brickW))         and ((ballY - ball._radius) >= brickY and (ballY - ball._radius)             <= (brickY + brickH)):
            return True
        else:
            return False
