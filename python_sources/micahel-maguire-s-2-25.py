# -*- coding: utf-8 -*-
"""
Created on Fri Sep  2 11:42:32 2016
...
def build_sparse(data, feature):
    data2 = data.copy(deep=True) # Deep duplicate to avoid overwriting the original (if writing over the original in a function, the real original is overwritten)
    data2[feature] = lbl.fit_transform(data2[feature])
    cols = len(lbl.classes_)
    sparse_matrix = csr_matrix((np.ones(data2.shape[0]), (data2.row_num, data2[feature])), shape=(rows, cols))
...
app_dev['app_id'] = app_dev['app_id'].factorize()[0] # Fix scikit-learn useless NaN forced-check, which is supposedly 0 in LabelEncoder for NaN + not working on very large ints - always use factorize() if you can =)
#app_dev['app_id'].fillna(0, inplace=True) # Don't do this method because it does not work on very large ints => str() > int() forced conversion
