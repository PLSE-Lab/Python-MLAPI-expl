...
Source code for text processing Named Entity Recognizer NER, OCR, Spacy, Concordance
'''
import sys
import nltk
...
def wipr_data_processor():
    ...
    # concordance on string
    doc = '''ADDITIONAL INTEREST/CERTIFICATE RECIPIENT
ACORD 45 attached for additional names
INTEREST
RANK: 1 2 NAME AND ADDRESS
REFERENCE #: /2 3 ABC
CERTIFICATE REQUIRED
INTEREST IN ITEM NUMBER
ADDITIONAL INSURED
LOCATION: BL R. |BUILDING:
: T9
N LOSS PAYEE
VEHICLE: CAR BOAT:
N MORTGAGEE
...
LENHOLDER
VIKASH KUMAR, B-903 . SJR EQUINOX
OTHER
EMPLOYEE AS LESSOR
ITEM DESCRIPTION:
COMPUTER
GENERAL INFORMATION
EXPLAIN ALL "YES" RESPONSES (For all past or presem
perations
YIN
...
N
NO
DO/HAVE PAST, PRESENT OR DISCONTINUED OPERATIONS INVOLVE(D) STORING, T
TRANSPORTING OF HAZARDOUS MATERIAL? (e.g. landfills, wastes, fuel tanks, etc)
ORING, TREATING, DISCHARGING, APPLYING, DISPOSING, OR
N
NO
ANY OPERATIONS SOLD. ACQUIRED, OR DISCONTINUED IN LAST FIVE (5) YEARS?
Y
YES ,
MANUAL TESTING DISCONTINUED
...
NO
N
ANY PARKING FACILITIES OWNED/REN
YES
RENTED
...
'''
    tokens = nltk.word_tokenize(doc)
    myTextdoc = Text(tokens)
    # question_list = ["NUCLEAR MATERIALS?", "DISCONTINUED IN LAST FIVE (5) YEARS?", "RENTED TO OTHERS?", "HIRED OR LEASED?", "RECREATION FACILITIES PROVIDED?", "SWIMMING POOL ON THE PREMISES?", "ALTERATIONS CONTEMPLATED?", "FROM OTHER EMPLOYERS?", "SUBSIDIARIES?"]
    question_list = ["SUBSIDIARIES", "STRUCTURAL", "PREMISES", "LEASED", "MATERIALS"] 
    for question in question_list: 
        match = myTextdoc.concordance(question, 15)
        print(match)
