...
def get_manifold():
	...
	for layer_id, layer in enumerate(model.layers):
		...
		for epoch in range(1, epochs[layer_id] + 1):
			# make random indexs for all datas
...
def get_meta():
	meta_X = pd.DataFrame({
		'soz':(X[X==-1]+1).fillna(1).sum(axis=1),
		'nzm':X[X!=-1].mean(axis=1),
		'nzs':X[X!=-1].std(axis=1),
		'med':X[X!=-1].median(axis=1),
		'max':X[X!=-1].max(axis=1),
		'min':X[X!=-1].min(axis=1),
		'var':X[X!=-1].var(axis=1)})
	return meta_X
...
mbkm_X = pd.DataFrame(mbkm.fit_transform(X))
print('PCA')
...
X_all = pd.concat([X_drop, meta_X, manifold_X, mbkm_X, pca_X, tsvd_X, grp_X, srp_X], axis=1, sort=False)

del X_drop, mbkm, pca, tsvd, grp, srp, mbkm_X, pca_X, tsvd_X, grp_X, srp_X
