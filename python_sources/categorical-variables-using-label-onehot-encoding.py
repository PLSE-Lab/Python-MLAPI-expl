...
list1 = [ col for col in OH_X_train.columns if OH_X_train[col].isnull().any()]
list1
...
X_test[low_cardinality_cols].isnull().any()
...
OH_cols_test = pd.DataFrame(OH_encoder.transform(X_test1[low_cardinality_cols]))
# OH_cols_valid = pd.DataFrame(OH_encoder.transform(X_valid[low_cardinality_cols]))
...
# Remove categorical columns (will replace with one-hot encoding)
num_X_test = X_test1.drop(object_cols, axis=1)
# num_X_valid = X_valid.drop(object_cols, axis=1)
