...
# In this kernel we present the neural network used in the article [Fruit recognition from images using deep learning](https://www.researchgate.net/publication/321475443_Fruit_recognition_from_images_using_deep_learning).
# 
...
# 
# The code is accompanied by comments, which we hope provide better understanding of the logic behind the neural network.

# In the first section we define most of the variables we will further use in this program.
...
# * *save_interval* - the number of iterations to train for before saving the graph and checkpoint files
...
# Here we define a helper class that converts the images into tfrecords file.  The tfrecord generation logic is roughly based on TensorFlow's own build_image_data.py script.
# 
...
# 
# * *parse_single_example* method reads from a tfrecord file and extracts the data and label from one image.
...
# Following, there are a few utility methods.
...
# Here we define methods to perform data augmetation on the input images. Data augmentation is a good way to reduce overfitting on models. 
# 
...
#    
# * *build_hsv_grayscale_image* converts the input image to HSV and adds the fourth grayscale channel
...
# * *build_datasets*  create datasets on the tfrecord files; one to retrieve data for training, one to show the evolution of the accuracy during training, one to evaluate the accuracy on the test set after the training
...
# In this section we define the network structure. This is done according to the following image:
# 
...
# 
# We tested several other configurations, however this one obtanied the best accuracy on the Test set.
...
# In this section we run the training process, and after its completion, we test the resulted network on the test set.
