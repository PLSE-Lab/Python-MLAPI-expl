...
for i in range(len(df['Diagnosis'])):
    if df['Diagnosis'][i]=='Pneumonia':
        pneum_mp[df['Patient number'][i]] = 1
    else:
        pneum_mp[df['Patient number'][i]] = 0
print(pneum_mp)
...
#     print(i['Patient number'][0])
#     print((a,pneum_mp[int(i['Patient number'][0])]))
#     print(pneum_mp[i['Patient number'][0]])
    rec_annotations_dict[s] = (a,pneum_mp[int(i['Patient number'][0])])
...
lp_test = get_sound_samples(rec_annotations_dict[str_file][0], str_file, root, 22000)
...
def extract_all_training_samples(filenames, annotation_dict, root, target_rate, desired_length, train_test_ratio = 0.1):
    cycle_list = []
    for file in filenames:
        data = get_sound_samples(annotation_dict[file][0], file, root, target_rate)
        cycles_with_labels = [(d[0], d[3], d[4],annotation_dict[file][1]) for d in data[1:]]
#         print(cycles_with_labels)
        cycle_list.extend(cycles_with_labels)
    

    #Sort into respective classes
    no_labels = [c for c in cycle_list if ((c[1] == 0) & (c[2] == 0))]
    c_only = [c for c in cycle_list if ((c[1] == 1) & (c[2] == 0) & (c[3] == 1))] 
    w_only = [c for c in cycle_list if ((c[1] == 0) & (c[2] == 1) & (c[3] == 1))]
...
print(classification_report(labels, predictions, target_names = ['healthy','pneumonia','other']))
print(confusion_matrix(labels, predictions))
print('Accuracy_Score: ', accuracy_score(labels, predictions))
