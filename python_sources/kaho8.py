...
# print("-" * 10 + "BayesClassifier" + "-" * 10)
# clf_bayes = MultinomialNB().fit(X, y)  

print("-" * 10 + "XGBClassifier" + "-" * 10)
clf_xgb = XGBClassifier(max_depth=6, learning_rate=0.012, n_estimators=1000, subsample=0.65, colsample_bytree=0.62, seed=1)
clf_xgb.fit(X, y)

print("-" * 10 + "Classifier completed" + "-" * 10)
target_x = data[mask]
