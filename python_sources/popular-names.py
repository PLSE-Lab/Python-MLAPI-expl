...
#ax = (dfPassengersWithPopularNames.groupby('FirstName').Survived.sum()/dfPassengersWithPopularNames.groupby('FirstName').Survived.count()).order(ascending=False).plot(kind='bar',y='Popular Names',fontsize=8)
