...
def word_match_share(row):
    ...
    if len(shared_words_in_q1)==0 or len(shared_words_in_q2):
        return (len(shared_words_in_q2)+len(shared_words_in_q1))/(len(q1words)+len(q2words))
    R = (2/(len(q1words)/len(shared_words_in_q1)+len(q2words)/len(shared_words_in_q2)))
    return R
