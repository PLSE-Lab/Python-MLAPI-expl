...
# <h1>Visualize each toy's distribution</h1>
...
# <h1>Get initial configuration of bags</h1>
...
# <h1>Running simulated annealing</h1>
...
# <h1>Trim bags that are unlikely to be accepted</h1>
# 
# <h4>* This is useful since the SA optimization algorithm can't remove toys from the optimization problem</h4><br/>
# 
# <small>(It would be simple enough to modify the swap function to allow this, by creating a special "out-of-problem bag" that would hold all the toys not considered for the 1000 bags)</small>
...
trimmed_bag_weights = [saModel.get_samples(b, 500) for b in bags]
...
submit_df.to_csv("./SA_solution.csv", sep=",", index=False)
