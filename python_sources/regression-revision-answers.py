...
# We can see how the variables are correlated to themselves by using the correlation matrix. Correlation matrix simply shows how each feature is correlated to another one. '1' stands for the biggest positive correlation (increase in x_1 leads to increase in x_2), '-1' stands for biggest negative correlation (increase in x_1 leads to decrease in x_2) and '0' for no correlation at all.  
...
sns.barplot(x = happiness_by_country['Country'], y = happiness_by_country['Happiness Score'])
...
"""Code below will be hidden"""
x = dataset_2015[['Economy (GDP per Capita)', 'Family',
       'Health (Life Expectancy)']] # Include more features if you think that it might improve the models accuracy
