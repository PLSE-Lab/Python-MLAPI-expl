...
# # ***FAKE NEWS CLASSIFICATION - PART 1***
# In this kernel, we will explore several methods for Binary Fake News Classification using pretrained embeddings Word2Vec, which are pretrained using billions of words in an attempt to improve classification accuracy as compared to self embedding and other embedding methods. LIAR-PLUS dataset is the extended LIAR dataset for fact-checking and fake news detection. This dataset has evidence sentences extracted automatically from the full-text verdict report written by journalists in Politifact. 
# 
...
train_=pd.read_csv('../input/liarplus/liar-plus-master/LIAR-PLUS-master/dataset/train2.tsv',delimiter='\t',encoding='utf-8', header=None)
test_=pd.read_csv('../input/liarplus/liar-plus-master/LIAR-PLUS-master/dataset/test2.tsv',delimiter='\t',encoding='utf-8', header=None)
valid_=pd.read_csv('../input/liarplus/liar-plus-master/LIAR-PLUS-master/dataset/val2.tsv',delimiter='\t',encoding='utf-8', header=None)

# Create table headers    
train_.columns = ['values','id','label','statement','subject','speaker', 'job', 'state','party','barely_true_c','false_c','half_true_c','mostly_true_c','pants_on_fire_c','venue','extracted_justification']
test_.columns = ['values','id','label','statement','subject','speaker', 'job', 'state','party','barely_true_c','false_c','half_true_c','mostly_true_c','pants_on_fire_c','venue','extracted_justification']
valid_.columns = ['values','id','label','statement','subject','speaker', 'job', 'state','party','barely_true_c','false_c','half_true_c','mostly_true_c','pants_on_fire_c','venue','extracted_justification']
...
# As we are performing binary classification, select the entries having label as either "true" or "false" from the data 

# In[ ]:


train_true = df[df['label'] == 'true']
train_false = df[df['label'] == 'false']
# Copy the contents of two columns "statement" and "extracted_justification" to new column "text"
train_true['text'] = train_true['statement'] + ' ' + train_true['extracted_justification'] 
train_false['text'] = train_false['statement'] + ' ' + train_false['extracted_justification']
...
test_true = test_[test_['label'] == 'true']
test_false = test_[test_['label'] == 'false']
# Copy the contents of two columns "statement" and "extracted_justification" to new column "text"
test_true['text'] = test_true['statement'] + ' ' + test_true['extracted_justification'] 
test_false['text'] = test_false['statement'] + ' ' + test_false['extracted_justification']
...
# As we are done with preparartion of training and test data. Now, create a new column for each label and assign binary values (0 or 1) to the corresponding labels in both training and test dataset. 

# In[ ]:


truth_ = {'false':0,'true':1} # Values of label
train['num_true'] = train['label'].apply(lambda x: truth_[x])
train['num_false'] = train['label'].apply(lambda x: truth_[x])
train.head() # Check the new columns just created
...
list_classes = ['num_true', 'num_false'] # two new columns created above
...
hist = model.fit(X_t,y_t, batch_size=batch_size, epochs=epochs, validation_data=(X_te,y_te), shuffle=True)
...
mn_count_clf = MultinomialNB(alpha=0.7)
...
mn_tfidf_clf = MultinomialNB(alpha=0.7)
...
# We explored binary classification methods for Fake News Classification with and without using pretrained embeddings Word2Vec on challenging LIAS-PLUS dataset. We recorded best accuracy of 55.8% for MultinomialNB model using Tfidf Vectorizer. Hence, better results can be achieved without using pretrained embedding for Fake News Classification. 
