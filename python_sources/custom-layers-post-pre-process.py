...
# The problem in this competition is to get a prediction on defects (masks) of 256x1600 top snapshot of steel surface images.
...
def conv2d_block(input_tensor, n_filters, kernel_size = 3, batchnorm = True, dilation_rate=(1,1)):
    ...
    x = Conv2DTranspose(filters = n_filters, kernel_size = (kernel_size, kernel_size),              kernel_initializer ='he_normal', padding = 'same', dilation_rate=dilation_rate, kernel_regularizer=l2(0.00003))(x)
    if batchnorm:
    ...
    # second layer
    x = BatchNormalization()(input_tensor)
    x = Conv2D(filters = n_filters, kernel_size = ( kernel_size, kernel_size),              kernel_initializer =  'he_normal', dilation_rate=dilation_rate, kernel_regularizer=l2(0.00003), padding = 'same')(x)
    if batchnorm:
        x = BatchNormalization()(x)
    x = Activation('relu')(x)
