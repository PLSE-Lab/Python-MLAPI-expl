...
def process_grid(df_train, df_test, x_cuts, y_cuts, t_cuts,
    ...
    for i in range(x_cuts):
        ...
        for j in range(y_cuts):
            ...
            df_cell_test = df_col_test[mask]
            y_min -= y_border_aug
            y_max += y_border_aug
            mask = (df_col_train['y'] >= y_min)
            mask = mask & (df_col_train['y'] < y_max)
            df_cell_train = df_col_train[mask]
...
def prepare_data(datapath, val_start_day, fw):
    val_label = None
...
def feature_engineering(df, fw):
    ...
    df = df.drop(df[df.year < 20].index)
...
df_train, df_test, val_label = prepare_data(datapath, val_start_day, fw)
gc.collect()
