...
# So happy can share you all.
# Thanks to previous kernel
...
# 
# I just modify the next_session feature and added to next kernel.
...
def process_date_time(data_df):
    ...
    data_df['date'] = pd.to_datetime(data_df['visitStartTime'], unit='s')
    data_df = data_df.sort_values('visitStartTime')
    data_df['next_session_1'] = (data_df['visitStartTime'] - data_df[['fullVisitorId', 'visitStartTime']].groupby('fullVisitorId')['visitStartTime'].shift(1)
    ).astype(np.float32)
    data_df['next_session_1'] = data_df['next_session_1'].fillna(data_df['next_session_1'].max())
    data_df['next_session_1'] = (data_df['next_session_1'] - data_df['next_session_1'].mean()) / (data_df['next_session_1'].max() - data_df['next_session_1'].min())
    data_df['next_session_2'] = ( data_df['visitStartTime'] - data_df[['fullVisitorId', 'visitStartTime']].groupby('fullVisitorId')['visitStartTime'].shift(-1)
    ).astype(np.float32)
    data_df['next_session_2'] = data_df['next_session_2'].fillna(data_df['next_session_2'].min())
    data_df['next_session_2'] = (data_df['next_session_2'] - data_df['next_session_2'].mean()) / (data_df['next_session_2'].max() - data_df['next_session_2'].min())
#     print('next session 2 = ',data_df['next_session_2'])
    return data_df
...
train_df['predictions'] = np.expm1(oof_lgb)
test_df['predictions'] = np.expm1(predictions_lgb)
...
train_df[['fullVisitorId','predictions']].to_csv("temp_train_predictions.csv", index=False)
test_df[['fullVisitorId','predictions']].to_csv("temp_test_predictions.csv", index=False)
