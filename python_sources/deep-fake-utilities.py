...
def extract_layers(main_model, starting_layer_ix, ending_layer_ix):
    ...
    new_model(main_model.layers[starting_layer_ix].input)
    return new_model

def clip_int(x, lb, ub):
    return int(max(lb, min(x, ub)))

def clip_face_location(face_location, ub1=1920-1, ub2=1080-1, lb1=0, lb2=0, pad_ratio=1.3):
...
def get_face_given_location(frame, face_location):
    ...
    shape = frame.shape
    im = cv2.resize(frame, (540, 960))
    top, left, bottom, right = clip_face_location(face_location, ub1=shape[1]-1, ub2=shape[0]-1)
