...
# # Matplotlib
# 
# Matplotlib is a Python library used for plotting. The first step in any program is importing the library. I'll import two libraries now: Matplotlib (in which we are going to work on) and numpy (for few other helper operations here).
...
# So, the above is a simple plot by simply calling `plot` function with the x & y co-ordinates as arguments. Now, we can plot the same graph again with some more detailing - like title, xlabel, ylabel, legend, etc.
...
# # Going deeper with figure and axes
# 
...
# Now, we can try plotting one of the axes just like we did above (please note that the function names of `pyplot` (which we used earlier) and `axes` (which we are going to use now) aren't same)
...
# # Different graphs in Matplotlib
# 
