...
def build_model(save=False):
    ...
    model.save("my_model_2.h5") #Export the model to local file
...
def fit_model(model, train_generator, val_generator, save=False):
    ...
    history = model.fit_generator(train_generator, epochs=5, validation_data=val_generator,steps_per_epoch = 1000,validation_steps = 1000
                                 )
...
def evaluate_model(generator):
    start_time = time.time()
    evaluations = CNN_MODEL.evaluate_generator(generator,steps = 500)
    ...
    start_time = time.time()
    predictions = CNN_MODEL.predict_generator(generator,steps = 1000)
...
#The input sample size is not consistent, does not affect the model 
