    ...
    def __init__(self,
        ...
        with tf.variable_scope(scope, default_name="bert"):
            ...
            with tf.variable_scope("encoder"):
                ...
                self.attention_layer_norm = tf.keras.layers.LayerNormalization(name="LayerNorm", axis=-1, epsilon=1e-12, dtype=tf.float32)
                self.ffn_layer_norm = tf.keras.layers.LayerNormalization(name="LayerNorm", axis=-1, epsilon=1e-12, dtype=tf.float32)
                ...
                self.all_encoder_layers = transformer_model(
                    ...
                    ffn_layer_norm=self.ffn_layer_norm)
...
def get_assignment_map_from_checkpoint(tvars, init_checkpoint, num_of_group=0):
    ...
    for name in name_to_variable:
        if name in init_vars_name:
            init_name = name
            assignment_map[init_name] = name_to_variable[name]
        else:
            tf.logging.info("name %s does not get matched", name)
            continue
        tf.logging.info("name %s match to %s", name, init_name)
        
        # assignment_map[init_name] = name
...
def layer_norm(input_tensor, name=None):
    ...
    layer = tf.keras.layers.LayerNormalization(name=name, axis=-1, epsilon=1e-12, dtype=tf.float32)
...
def embedding_postprocessor(input_tensor,
    ...
    output = layer_norm_and_dropout(output, dropout_prob, "LayerNorm")
    return output
...
def attention_ffn_block(layer_input,
    ...
    attention_output = attention_layer_norm(attention_output + layer_input)
    ...
    ffn_output = ffn_layer_norm(ffn_output + attention_output)
    return ffn_output
                    ...
                    for inner_group_idx in range(inner_group_num):
                        with tf.variable_scope("inner_group_%d" % inner_group_idx):
                            layer_output = attention_ffn_block(
                                ...
                                attention_layer_norm, ffn_layer_norm)
