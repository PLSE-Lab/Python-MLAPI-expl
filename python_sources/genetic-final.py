...
def GPIndividual1(data):
    predictions = (np.sinh(((((np.sinh(data["team1Seed"]) - data["team2Seed"]) + ((np.tanh(data["team2Wmax"]) + ((data["team1Lstd"] + np.minimum( (data["team1losses"]),  (data["year"])))/2.0))/2.0)) + ((data["team2Seed"] == (1.0/(1.0 + np.exp(- data["team2wins"])))).astype(float)))/2.0)) +
                    ...
                    np.minimum( (np.cos(data["team1"])),  (((((((((((data["team1"] / 2.0) / 2.0) * 9.868604) <= 0.058823).astype(float)) <= data["team1Wstd"]).astype(float)) == np.ceil(((data["team1"] / 2.0) * 9.869604))).astype(float)) - 0.094340))) +
                    ...
                    ((math.tanh((-(1.619430))) > np.sin(np.maximum( (data["team2Wmin"]),  (np.minimum( (np.minimum( (data["team2Seed"]),  (((data["team1LAverage"] + data["team1Wstd"])/2.0)))),  ((((data["team2Seed"] <= data["team2Wmin"]).astype(float)) - data["team2Lstd"]))))))).astype(float)) +
