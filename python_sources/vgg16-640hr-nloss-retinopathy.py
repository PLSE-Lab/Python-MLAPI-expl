...
import numpy as np
IMG_SIZE = (640, 640) # slightly smaller than vgg16 normally expects
...
def categorical_and_healthy(in_gt, in_pred):
    """Add categorical to the correctness of the healthy stage (0 vs 123 is more important than  1 vs 2)"""
    return categorical_crossentropy(in_gt, in_pred)+binary_crossentropy(in_gt[:,0], in_pred[:,0])
def top_2_accuracy(in_gt, in_pred):
    return top_k_categorical_accuracy(in_gt, in_pred, k=2)

retina_model.compile(optimizer = 'adam', loss = categorical_and_healthy,
...
for (idx, c_ax) in enumerate(m_axs.flatten()):
    c_ax.imshow(np.clip(test_X[idx]+127,0 , 255).astype(np.uint8), cmap = 'bone')
