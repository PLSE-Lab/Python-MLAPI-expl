...
with open('../input/tactic-03-hyperparameter-optimization-lr/clf.pickle', 'rb') as fp:
    clf = pickle.load(fp)
...
table_grid_search(clf, all_columns=True)
table_grid_search(clf, all_columns=True, all_ranks=True)
...
cv_results = pd.DataFrame(clf.cv_results_).sort_values(by=['rank_test_score', 'mean_fit_time'])
cv_results[cv_results['rank_test_score'] == 2]
...
get_ipython().system('/opt/conda/bin/orca')
...
def grid_search_plot(grid_clf,
                     parameters):
    scores_df = pd.DataFrame(grid_clf.cv_results_).sort_values(by='rank_test_score')
...
grid_search_plot(clf, ['C', 'solver', 'max_iter'])
