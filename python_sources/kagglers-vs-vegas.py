...
results = [['name','logLossWin','logLossCover', 'correctWinPred', 'correctCoverPred']]
for name in sorted(data_dict.keys()):
    ...
    results.append([name, logLossWin, logLossCover, correctWinPred, correctCoverPred])


output = pd.DataFrame.from_records(results)
output.to_csv('kagglersVsVegas.csv')
