    ...
    for step in range(1, num_steps + 1):
        with tf.Graph().as_default():
            ...
            if step % display_step == 0 or step == 1:
                print("{:.4f}".format(toc - tic)+ "s,", "step " + str(step) + ", minibatch loss = " +                       "{:.4f}".format(loss) + ", training accuracy = " +                       "{:.4f}".format(acc) , ", acc_meet_target_cnt = " + "{:.4f}".format(acc_meet_target_cnt))
...
def testModel(images, labels):
    ...
    print("Testing accuracy = ",           # sess.run(accuracy,feed_dict={X:mnist.test.images/255, Y:mnist.test.labels}))
