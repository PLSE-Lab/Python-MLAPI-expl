...
def featurize(df, fcp, aggs, n_jobs=4):
    """
    ...
    agg_df['flux_diff'] = agg_df['flux_max'].values - agg_df['flux_min'].values
    agg_df['flux_dif2'] = agg_df['flux_diff'].values / agg_df['flux_mean'].values
    agg_df['flux_dif3'] = agg_df['flux_diff'].values / agg_df['flux_w_mean'].values
    
    # Add more features with
    agg_df_ts = extract_features(df, 
                                 column_id='object_id', column_sort='mjd', column_kind='passband', 
                                 column_value='flux', default_fc_parameters=fcp, n_jobs=n_jobs)

    # Add smart feature that is suggested here https://www.kaggle.com/c/PLAsTiCC-2018/discussion/69696#410538
    # dt[detected==1, mjd_diff:=max(mjd)-min(mjd), by=object_id]
    df_det = df[df['detected']==1].copy()
    agg_df_mjd = extract_features(df_det, column_id='object_id', column_value='mjd', 
                                  default_fc_parameters={'maximum':None, 'minimum':None}, n_jobs=n_jobs)
...
def predict_chunk(df_, clfs_, meta_, features, featurize_configs, train_mean):
    # Group by object id
    agg_ = featurize(df_, featurize_configs['fcp'], featurize_configs['aggs'])
    # Merge with meta data
    agg_.reset_index(inplace=True)
    full_test = agg_.merge(right=meta_, how='left', on='object_id')
...
def main(argc, argv):
    ...
    agg_train = featurize(train, fcp, aggs)
    ...
    process_test(clfs, 
                 ...
                 featurize_configs={'fcp': fcp, 'aggs': aggs}, 
                 train_mean=train_mean, 
                 filename=filename,
                 chunks=5000000)
