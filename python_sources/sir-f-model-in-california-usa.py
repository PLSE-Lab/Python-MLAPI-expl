...
# ## Trend analysis
# Here, we will check the trend of the variables. If the trend was changed drastically, we will use only the last trend to predict the future.
...
# Confirmed cases: Trend was changed on 16Mar2020, but this change is not significant. We will use all data.
...
# Fatalities: No change points were found.

# ## External data: total population
# To calculate the number of susceptible cases (explained later), it is necessary to know total population in CA, USA.
...
# # Parameter estimaition of the model
# In this section, we will estimate the non-dimensional model using training data.

# ## Create data every one minute by curve fitting
# Because we have few records, it is difficult to estimate parameters of SIR-F model with Califolnia data. We will try to increase the number of records by curve fitting of exponential function. With this method, we can create a dataset which frequency is one minute.
...
# ### 1st step: with fixed $\theta$ and $\sigma$
# We need to estimate five parameters $(\theta, \kappa, \sigma, \rho, \tau)$, but we have only two observed variables (Susceptible, Fatal) with T. To improve arcuracy of parameter optimization, we try to estimate $(\kappa, \rho, \tau)$ with the assumption that $\theta=0$ and $\sigma=0$.

# In[ ]:


get_ipython().run_cell_magic('time', '', 'first_estimator = Estimator(SIRF, many_train_df, total_population, name="California", theta=0, sigma=0)\nfirst_dict = first_estimator.run()')
...
# ## 2nd step: estimate $\theta$ and $\sigma$
# With fixed $(\kappa, \rho, \tau)$, we will estimate $\theta$ and $\sigma$ here.
...
get_ipython().run_cell_magic('time', '', 'second_estimator = Estimator(SIRF, many_train_df, total_population, name="California", **param_dict)\nsecond_dict = second_estimator.run()')
...
# ### 3rd step: Adjust paramerters
# Here we will adjust the paramers, changing them slightly.
