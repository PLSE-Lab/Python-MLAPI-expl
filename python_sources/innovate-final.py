...
def generate_user_similarity_matrix(model,interactions,num_emp):
    ...
    spsr=pd.DataFrame(cosine_similarity(sparse.csr_matrix(latusr[:num_emp,:])))
...
def generate_item_similarity_matrix(model,interactions,num_item):
    ...
    spsr=pd.DataFrame(cosine_similarity(sparse.csr_matrix(latitm[:num_item,:])))
...
ee=generate_item_similarity_matrix(modelnew,intertest,10)
create_heatmap_emp(ee,"issue similarity")
