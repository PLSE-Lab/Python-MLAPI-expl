...
def train_and_test_linear(train,test,features,target='SalePrice'): # simple xgboost
    ...
    num_boost_round = 1200 # num of booosting iterations 
    ...
    watchlist = [(dtrain, 'train'), (dvalid, 'eval')] # List of items to be evaluated during training, this allows user to watch performance on the validation set.
...
def train_and_test_tree(train,test,features,target='SalePrice'): # simple xgboost
    eta_list = [0.006,0.007] # list of parameters to try
    max_depth_list = [4,5,6] # list of parameters to try
    subsample = 0.8
    colsample_bytree = 0.8
...
def train_and_test_Kfold(train,test,features,target='SalePrice'): # add Kfold
    eta_list = [0.005] # list of parameters to try
    max_depth_list = [5]
