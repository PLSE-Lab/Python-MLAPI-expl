""" 
Fork this,and please run it without commiting, it will create this(---Optimal Feature Subset(s)---, also shown by CPU 0%) 
if you break it in this state you'll get the visualitions of the model working.
"""
import pandas as pd
...
dfData = pd.read_csv('../input/titanic-cleaned-dataset/titanic_cleaned_data.csv')

# Encode the classification labels to numbers
# Get classes and one hot encoded feature vectors
le = LabelEncoder()
le.fit(dfData['Survived'])
allClasses = le.transform(dfData['Survived'])
allFeatures = dfData.drop(['Survived'], axis=1)
...
'''
This code is approprated to the Titanic dataset from https://github.com/scoliann/GeneticAlgorithmFeatureSelection
Full Credit: Stuart C.(scoliann)
'''
