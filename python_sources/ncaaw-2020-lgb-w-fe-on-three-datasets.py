...
DATA_DIR = '/kaggle/input/google-cloud-ncaa-march-madness-2020-division-1-womens-tournament'
...
WRSCResults = pd.read_csv(DATA_DIR + '/WDataFiles_Stage1/WRegularSeasonCompactResults.csv')

A_w = WRSCResults[WRSCResults.WLoc == 'A']    .groupby(['Season','WTeamID'])['WTeamID'].count().to_frame()    .rename(columns={"WTeamID": "win_A"})
N_w = WRSCResults[WRSCResults.WLoc == 'N']    .groupby(['Season','WTeamID'])['WTeamID'].count().to_frame()    .rename(columns={"WTeamID": "win_N"})
H_w = WRSCResults[WRSCResults.WLoc == 'H']    .groupby(['Season','WTeamID'])['WTeamID'].count().to_frame()    .rename(columns={"WTeamID": "win_H"})
win = A_w.join(N_w, how='outer').join(H_w, how='outer').fillna(0)

H_l = WRSCResults[WRSCResults.WLoc == 'A']    .groupby(['Season','LTeamID'])['LTeamID'].count().to_frame()    .rename(columns={"LTeamID": "lost_H"})
N_l = WRSCResults[WRSCResults.WLoc == 'N']    .groupby(['Season','LTeamID'])['LTeamID'].count().to_frame()    .rename(columns={"LTeamID": "lost_N"})
A_l = WRSCResults[WRSCResults.WLoc == 'H']    .groupby(['Season','LTeamID'])['LTeamID'].count().to_frame()    .rename(columns={"LTeamID": "lost_A"})
...
WRSCResults['relScore'] = WRSCResults.WScore - WRSCResults.LScore
...
# ## FE on RegularSeasonDetailedResults

# In[ ]:


WRSDetailedResults = pd.read_csv(DATA_DIR + '/WDataFiles_Stage1/WRegularSeasonDetailedResults.csv')
...
# ## FE on NCAATourneySeeds

# In[ ]:


WNCAATourneySeeds = pd.read_csv(DATA_DIR + '/WDataFiles_Stage1/WNCAATourneySeeds.csv')
WNCAATourneySeeds['seed_num'] =  WNCAATourneySeeds.Seed                                 .apply(lambda x: int(x[1:3]))
seed = WNCAATourneySeeds.drop('Seed', axis=1)

del WNCAATourneySeeds
...
Seed_1 = seed.copy()
Seed_1.columns = [str(col) + '_1' if col not in ['Season','TeamID']                   else str(col) for col in Seed_1.columns ]

Seed_2 = seed.copy()
Seed_2.columns = [str(col) + '_2' if col not in ['Season','TeamID']                   else str(col) for col in Seed_2.columns ]


# ## Loading NCAATourneyCompactResults
# 
...
TCResults = pd.read_csv(DATA_DIR + '/WDataFiles_Stage1/WNCAATourneyCompactResults.csv')
