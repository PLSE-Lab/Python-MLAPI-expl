...
def run_lgb(reduce_train, reduce_test, usefull_features, params, verbose_eval=0, predict=False, show=False):
    ...
    for fold, (tr_ind, val_ind) in enumerate(kf.split(reduce_train, reduce_train[target])):
        ...
        model = lgb.train(params, train_set, num_boost_round=3000, early_stopping_rounds=100, 
...
def LGB_bayesian(max_depth,
    ...
    res = run_lgb(reduce_train, reduce_test, features, params, show=False)
...
y_pred = run_lgb(reduce_train, reduce_test, features, bayes_params, verbose_eval=100, predict=True, show=True)
