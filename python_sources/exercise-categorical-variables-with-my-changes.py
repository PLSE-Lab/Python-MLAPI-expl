...
X['GarageYrBlt'][X['GarageCars']==0 & X['GarageYrBlt'].isnull()] = 1899
X_test['GarageYrBlt'][X_test['GarageCars']==0 & X_test['GarageYrBlt'].isnull()] = 1899
...
categorial_cols = [col for col in X_train.columns if X_train[col].dtype == 'object']

drop_X_train = X_train.drop(categorial_cols, axis=1)
drop_X_valid = X_valid.drop(categorial_cols, axis=1)
...
low_cardinality_cols = [col for col in object_cols if X_train[col].nunique() < 100]
...
OH_X_train = pd.DataFrame(one_hot_encoder.fit_transform(OH_X_train))
OH_X_valid = pd.DataFrame(one_hot_encoder.transform(OH_X_valid))
OH_X_train.index = X_train.index
OH_X_valid.index = X_valid.index

numeric_cols = [col for col in X_train.columns if X_train[col].dtype != 'object']

OH_X_train = pd.concat([X_train[numeric_cols], OH_X_train], axis=1)
OH_X_valid = pd.concat([X_valid[numeric_cols], OH_X_valid], axis=1)
...
def fillnan_test(train, test):
    for col in test.columns:
        test[col] = test[col].fillna(train[col].median())

# defune and fit model
model = RandomForestRegressor(n_estimators=100, random_state=0)
model.fit(OH_X_train, y_train)

# create test
OH_X_test = X_test[low_cardinality_cols]
OH_X_test = OH_X_test.applymap(replace_nan_by_unknown)
OH_X_test = pd.DataFrame(one_hot_encoder.transform(OH_X_test))
OH_X_test.index = X_test.index
OH_X_test = pd.concat([X_test[numeric_cols], OH_X_test], axis=1)
fillnan_test(OH_X_train, OH_X_test)
preds_test = model.predict(OH_X_test)

# Save test predictions to file
output = pd.read_csv('../input/sample_submission.csv')
output['SalePrice'] = preds_test
output.to_csv('submission.csv', index=False)


# # Keep going
# 
# Continue to learn how to use **[pipelines](https://www.kaggle.com/alexisbcook/pipelines)** to preprocess datasets with a mixture of categorical variables and missing values.
