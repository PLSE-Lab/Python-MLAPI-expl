...
# Create X
features = ['LotArea', 'YearBuilt', '1stFlrSF', '2ndFlrSF', 'FullBath', 'BedroomAbvGr', 'TotRmsAbvGrd','GarageYrBlt','OverallCond','OverallQual',
            'TotalBsmtSF','Fireplaces','MasVnrArea','BsmtFinSF2','BsmtUnfSF','GrLivArea','BsmtFullBath','BsmtHalfBath',
           'FullBath','HalfBath','KitchenAbvGr','GarageCars','GarageArea','WoodDeckSF','OpenPorchSF','EnclosedPorch',
            '3SsnPorch','ScreenPorch','PoolArea'
           ]
my_imputer = SimpleImputer()


X = my_imputer.fit_transform(home_data[features])
...
# train_X,  train_y = train_test_split(X, y, random_state=1)
...
test_X = my_imputer.fit_transform(test_data[features])

# test_X = pd.get_dummies(test_data[features])
test_y = home_data.SalePrice

# val_predictions = rf_model_on_full_data.predict(test_X)
