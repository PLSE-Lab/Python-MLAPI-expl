...
train = train[train.price_doc/train.full_sq >= 100000]
...
model = xgb.train(dict(xgb_params, silent=0), dtrain, num_boost_round=344)
...
y_train = train["price_doc"] * mult + 14
x_train = train.drop(["id", "timestamp", "price_doc"], axis=1)
x_test = test.drop(["id", "timestamp"], axis=1)
...
num_boost_rounds = 397  # This was the CV output, as earlier version shows
...
df_train.drop(df_train[df_train["life_sq"] > 6500].index, inplace=True)

y_train = df_train['price_doc'].values  * mult + 12
id_test = df_test['id']
...
first_result["price_doc"] = np.exp( .73*np.log(first_result.price_doc_louis) + 
                                    .27*np.log(first_result.price_doc_bruno) )
result = first_result.merge(gunja_output, on="id", suffixes=['_follow','_gunja'])

result["price_doc"] = np.exp( .705*np.log(result.price_doc_follow) + 
                              .295*np.log(result.price_doc_gunja) )
