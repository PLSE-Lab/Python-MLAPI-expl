...
class GalData(Dataset):
    ...
    def get_mini_batch(self, batch):
        ...
        # Transfer to the accelerator device (in case this is running on a GPU).
...
class ConvNet(object):
    ...
    def __init__(self, backend, dataset):
        ...
        self.model = MLP(num_epochs=2, batch_size=128, layers=self.layers)
    ...
    def fit(self):
        Fit(model=self.model, backend=backend, dataset=self.dataset).run()
        return self

    def predict(self):
        ds = self.dataset
        outputs, targets = self.model.predict_fullset(self.dataset, 'test')
        predshape = (ds.testlen, ds.nclasses)
        ...
        # predictions to be made. We leave the missing predictions as 0.
        start = ds.winsize - 1
...
def run(subj):
    ...
    """
    ds = GalData(subj=subj, winsize=1024, subsample=16)
    network = ConvNet(backend, ds)
    labs, preds, inds = network.fit().predict()
    if validate:
        aucs = [auc(labs[:, i], preds[:, i]) for i in range(ds.nclasses)]
        print('Subject %d AUC %.4f' % (subj, np.mean(aucs)))
    return labs, preds, inds
