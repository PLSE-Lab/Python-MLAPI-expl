...
def readLangs(lang1, lang2, reverse=True):
    ...
    lines = open('/kaggle/input/englishhindi-corpus/%s-%s.txt' % (lang1, lang2),encoding='utf-8').read().strip().split('\n')  
...
input_lang, output_lang, pairs = prepareData('eng', 'hin1', False) # eng, hin1 for eng hindi and eng2, mal2 for eng mal
...
'''def evaluateRandomly(encoder, decoder, pair):
    %matplotlib inline
...
def evaluateRandomly(encoder, decoder,  n=7):
...
'''%matplotlib inline
c=[]
a=input("enter english ")
b=input("enter sudanese ")
c.append(a)
c.append(b)
pair=c
evaluateRandomly(encoder1, attn_decoder1, pair)'''
