...
cuisine_fig = pd.DataFrame(cuisine_distribution, index=[0]).transpose()[0].sort(ascending=False, inplace=False).plot(kind='pie')
...
ingredient_fig = pd.DataFrame(ingredient_distribution, index=[0]).transpose()[0].sort(ascending=False, inplace=False)[:20].plot(kind='barh',color= my_colors)
