...
absCorrs = abs(home_data.interpolate().corr()["SalePrice"]).sort_values(ascending=False).drop("SalePrice")
highCorrs = absCorrs[absCorrs > .10].index
highCorrs
...
# Create X
features = highCorrs
X = home_data.interpolate()[features]
...
test_X = test_data.interpolate()[features]
