...
def transform_data(X):
    ...
    X['created']=pd.to_datetime(X_train['created'],format='%Y-%m-%d %H:%M:%S')
    X['created']=pd.to_datetime(X_test['created'],format='%Y-%m-%d %H:%M:%S')
    X['Day']=X['created'].dt.dayofweek
    X['Day']=X['created'].dt.dayofweek
...
def normalize_high_cordiality_data():
    high_cardinality = ["building_id", "manager_id","Day","Hour"]
