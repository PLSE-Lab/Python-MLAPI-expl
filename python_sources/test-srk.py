...
def getEmpleado_N(row):
    val = row['ind_empleado'].strip()
    if val == 'N':
        return 1
    else:
        return 0

def getEmpleado_B(row):
    val = row['ind_empleado'].strip()
    if val == 'B':
        return 1
    else:
...
def processDataMK(in_file_name, cust_dict, lag_cust_dict):
    ...
    for row in csv.DictReader(in_file_name):
        ...
        x_vars.append(getEmpleado_N(row))
        x_vars.append(getEmpleado_B(row))
        x_vars.append(getEmpleado_F(row))
        x_vars.append(getEmpleado_A(row))
        x_vars.append(getEmpleado_S(row))
