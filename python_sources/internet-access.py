...
for key in state_codes.keys():
    ...
    for info, shape in zip(m.state_info, m.state):
        ...
        # get the number of anti 3 inside dataForStPuma
        noACCESS = (dataForStPuma == 3)
        accessPerc = (1-(sum(noACCESS)/(1.0*len(dataForStPuma)))) 


        colorInd = min(int(round(accessPerc*num_colors)),num_colors-1)
...
legend_str = [str(float(i))+"%" for i in bounds]
