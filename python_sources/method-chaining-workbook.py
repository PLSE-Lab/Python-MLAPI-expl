...
check_q4(chess_games.assign(n=0)
                    .groupby(["white_id", "victory_status"])["n"]
                    .count()
                    .reset_index()
                    .pipe(lambda x: x.loc[x["white_id"].isin(chess_games["white_id"].value_counts()
                                                                                    .head(20)
                                                                                    .index)]))
...
check_q6(((wine_reviews["points"].dropna() - 80) / 4).value_counts().sort_index().rename("Wine Ratings"))
...
check_q7(ramen_reviews[ramen_reviews["Stars"] != "Unrated"]["Stars"].astype("float64")
                                                                    .value_counts()
...
check_q8(ramen_reviews["Stars"].replace("Unrated", None).dropna().astype("float64")
                                                                 .map(lambda x: int(x) if x - int(x) < 0.5 
                                                                                       else int(x) + 0.5)
                                                                 .value_counts()
