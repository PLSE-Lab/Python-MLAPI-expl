...
for i in range(x_vec.shape[0]):
    ax1.plot(x_vec[i, :], y_vec[i, :], '-')


# # Change to Differential Coordinates

# In[ ]:


dx_vec = x_vec-np.tile(x_vec[:, 0:1], x_vec.shape[1])
dy_vec = y_vec-np.tile(y_vec[:, 0:1], y_vec.shape[1])
...
theta_x = np.repeat(theta.reshape((1, -1)), dy_vec.shape[0], axis=0)
...
x_coef = quad_reg.steps[-1][1].coef_
x_coef
