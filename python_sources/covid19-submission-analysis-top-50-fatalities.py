...
def calculate_lb(submissions):
    ...
    lb.loc[:, 'RMSLE'] = lb['FatalatiesRMSLE']   # NQ edit
...
def calculate_lb_and_ensemble(week):
    ...
    # NQ edit: Fatalities Only
    daily_error['Daily RMSLE'] = np.sqrt( 1* daily_error.FatalitiesSLE                                              + 0 * daily_error.ConfirmedCasesSLE)
...
submissions.SubmissionIdlbsubmissions[(submissions.SubmissionId == lb.SubmissionId.iloc[0])
              & (submissions.Province_State=='New York')].dropna(how='any').tail()
...
submissions[(submissions.SubmissionId == lb.SubmissionId.iloc[0])
              & (submissions.Province_State=='New York')].dropna(how='any').tail()
...
'Difficult Locations'
ensembles[ensembles.Week >= 3]    .groupby(['Week', 'Location']).mean().reset_index().sort_values(by='FatalitiesSLE', ascending=False).head(10)
'Easiest Locations'
ensembles[ensembles.Week >= 3]    .groupby(['Week', 'Location']).mean().reset_index().sort_values(by='FatalitiesSLE', ascending=False).dropna().tail(10)
