...
# This is the second time that I've rewritten the kernel of [Data Preparation & Exploration](https://www.kaggle.com/bertcarremans/data-preparation-exploration) made by [Bert Carremans](https://www.kaggle.com/bertcarremans).
...
# 
# Thanks to our meta file we can easily select the variables on which we want to compute the descriptive statistics. To keep things clear, well do this per data type. 
...
# 
# **Overall**, we can see that the range of the interval variables is rather small. Perhaps some transformation (eg. log) is already applied in order to anoymize the data?
...
# 
# There are many more strategies of course and MachineLearningMastery.com give a nice overview. As we have a rather large training set, we can go for **undersampling**. 
...
# Only **ps_car_11_cat** has many distinct values, although it is still reasonalbe. 
# 
...
# Scrips by https://www.kaggle.com/ogrellier
...
# ### Interval variables
# 
# Checking the correlations between interval variables. A heatmap is a good way to viualize the correlation between variables. The code below is based on an example by Michael Waskom
...
def corr_heatmap(v):
    ...
    sns.heatmap(correlations, cmap=cmap, vmax=1.0, center=0, fmt='.2f', square=True,               linewidth=.5, annot=True, cbar_kws={'shrink':.75})
    plt.show()
...
# We would lose rather many variables if we would select based on variance. But becuase we do not have so many variables, we'll let the classifier chose. For data sets with many more variables this could reduce the processing time. 
# 
...
# 
# The code in the cell below is borrowed from the GitHub repo of Sebastian Raschka. This repo contains code samples of his book Python Machine Learning, which is an absolue must to read. 
...
for f in range(X_train.shape[1]):
    print('%2d) %-*s %f' % (f + 1, 330, feat_labels[indices[f]], importances[indices[f]]))
...
# ## Feature Scaling
# 
# As mentioned before, we can apply standard scaling to the traing data. Some classifiers perform better when this is done. 

# In[ ]:


scaler = StandardScalerdardScaler()
