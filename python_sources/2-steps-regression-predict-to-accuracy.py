...
# 
# ## Based Discussion
# - https://www.kaggle.com/c/data-science-bowl-2019/discussion/122479
# 
# ## Notice
# - Since it is a trial kernel, so we use leak features(`installation_xx`).
#     - https://www.kaggle.com/hidehisaarai1213/be-cautious-on-some-features
# 
...
def read_data():
    ...
    #train_labels['sum_of_trials'] = train_labels['num_correct'] + train_labels['num_incorrect']
...
def get_data(user_sample, test_set=False):
    ...
    for i, session in user_sample.groupby('game_session', sort=False):
        ...
        if (session_type == 'Assessment') & (test_set or len(session)>1):
            ...
            features['sum_of_trials']       = true_attempts + false_attempts
...
y_1 = reduce_train['sum_of_trials']
y_2 = reduce_train['num_correct']
print(y_1, y_2)
...
plt.hist(y_1)
plt.show()
print(np.mean(y_1), (np.std(y_1)**2)) # over-dispersion.
...
cols_to_drop = ['game_session', 'installation_id', 'timestamp', 'accuracy_group', 'timestampDate','sum_of_trials', 'num_correct']
...
# ## Step 1. odds and ends
# - We predict whether the player finished game or not

# In[ ]:


params_1 = {'n_estimators':5000,
            'boosting_type': 'gbdt',
            'objective': 'binary',
            'metric': 'rmse',
            'subsample': 0.8,
...
regressor_model1.fit(X=reduce_train, y=y_2, folds=folds, params=params_1, preprocesser=mt, transformers=transformers,
                    eval_metric='auc', cols_to_drop=cols_to_drop)


# ## Step2. How try it?
# - We predict how many tries the player has done
# - We used `tweedie` objective, Maybe it is also good to use `poisson` objective.

# In[ ]:


params_2 = {'n_estimators':10000,
            'boosting_type': 'gbdt',
            'objective': 'tweedie',
            'metric': 'rmse',
            'subsample': 0.7,
...
regressor_model2.fit(X=reduce_train, y=y_1, folds=folds, params=params_2, preprocesser=mt, transformers=transformers,
                    eval_metric='tweedie', cols_to_drop=cols_to_drop)
...
# ## submission
# - We calculate `accuracy` to predict `accuracy_group`
...
y_3 = reduce_train['accuracy_group']


# In[ ]:


game_finished_train = regressor_model1.predict(reduce_train)
game_finished_test  = regressor_model1.predict(reduce_test)
...
threshold_train = np.mean(game_finished_train)
finished_train  = np.zeros(reduce_train.shape[0])

finished_train[game_finished_train.reshape(-1,)>threshold_train] = 1


# In[ ]:


get_ipython().run_cell_magic('time', '', 'count_train = regressor_model2.predict(reduce_train).reshape(-1,)\npr2 = finished_train / count_train\n\noptR = OptimizedRounder()\noptR.fit(pr2.reshape(-1,), y_3)\ncoefficients = optR.coefficients()')
...
threshold_test = np.mean(game_finished_test)
finished_test  = np.zeros(reduce_test.shape[0])
finished_test[game_finished_test.reshape(-1,)>threshold_test] = 1
count_test     = regressor_model2.predict(reduce_test).reshape(-1,)

pr1 = finished_test / count_test
