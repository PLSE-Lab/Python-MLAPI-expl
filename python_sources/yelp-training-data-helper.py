...
class YelpDataCrawler(object):
    ...
    def get_train_biz_attribs_one_hot_by_biz_id(self, biz_id):
        return np.array([(0,1)[str(i) in self.get_train_biz_attribs_by_biz_id(biz_id)]                                                             for i in range(len(self.restaurant_attributes))])
    ...
    def sample_train_images(self, sample_size, img_preprocess_fn=dflt_img_preprocess_fn, 
        ...
        X_inputs = np.array([img_preprocess_fn(self.get_train_image_by_img_id(img_id))                                                 for img_id in sample_photobiz_tbl["photo_id"] ])
        y_labels = np.array([self.get_train_biz_attribs_one_hot_by_biz_id(biz_id)                                                 for biz_id in sample_photobiz_tbl["business_id"]])
...
# ## Tile Business Images to learn Business -> Attribute classifier maybe
