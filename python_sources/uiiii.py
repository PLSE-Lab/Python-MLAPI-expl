...
#print(accuracy_score(predictions,tfidfts,normalize=False))

print (testdf[['id' , 'ingredients_clean_string' , 'cuisine' ]])
#testdf[['id' , 'ingredients_clean_string' , 'cuisine' ]].to_csv("submission.csv")
result = [(ref == res, ref, res) for (ref, res) in zip(targets_tr, classifier.predict(predictors_ts))]
