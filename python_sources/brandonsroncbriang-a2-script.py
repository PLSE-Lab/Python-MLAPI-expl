# NumPy and Pandas imports are used for data manipulation.
import numpy as np
import pandas as pd

# Custom trees from scratch using squared errors as degree of 
# fitness criterion determining the survival of that darwin generation.
def Outputs(data):
    return np.round(1.-(1./(1.+np.exp(-data))))

# The GeneticFunction is used to estimate with great accuracy which of the passengers survived and 
# which of the passengers did not survive.
# The GeneticFunction uses sin, cos, and tanh to manipulate the data numerically. 
# Sin is the ratio of the length of the side that is opposite that angle to the length of the 
# longest side of the triangle. 
# Cos is the ratio of the lengths of the side of the triangle adjacent to the angle and the hypotenuse. 
# Tanh is a ratio suitable for both symbolic and numerical manipulation. 
# Tanh threads over lists and can be evaluated to arbitrary numerical precision. 
# Sin, cos, and tanh are used to increase the accuracy and precision of predicting which passengers 
# survived and which passengers did not survive.
...
def MungeData(data):
    # Sex
    # All sex datapoints that are na are set to 0 which represents female. 
    # This is not done with age however, because higher insight is gained from the sex data than the age data. 
    # The ticket and name parameters are dropped in order to make full use of other parameters that draw more 
    # insight like: sex, class, and separation into cabins. 
    # Sex is converted to an integer.
    data.drop(['Ticket', 'Name'], inplace=True, axis=1)
    data.Sex.fillna('0', inplace=True)
    data.loc[data.Sex != 'male', 'Sex'] = 0
    data.loc[data.Sex == 'male', 'Sex'] = 1
    # Cabin
    # Replaces all cabin datapoints that are initially na with 0. 
    # Converts the different cabins to integer values.
    ...
    # Embarked
    # Replaces all embarked cabin datapoints that are initally na with 0. 
    # Converts the different embarkment locations to integer values. 
    # The data is returned as type float. Float is a numeric data type. 
    # A floating point number has decimal points even if that decimal point value is 0. 
    # That data type used will affect the decimal and in turn affect the overall score. 
    # Float values are generally out of a range.
...
#The predicted survived and did not survive results are exported to the gptest.csv file.
