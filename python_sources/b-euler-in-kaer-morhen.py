...
# Reset de keras y semillas fijas
kb.clear_session()
np.random.seed(1)
            ...
            if capture_success:
                ...
                if nu_caras > 0:
                    ...
                    if confianza >= umbral_confianza_es_cara:
                        ...
                        #if prob_manipulado >= 0.5 and prob_manipulado < 0.80:
                        #    prob_real = classifier_fake.predict(image)
                        #    prob_manipulado_fake = 1 - prob_real[0, 0]
                        #    prob_manipulado = prob_manipulado * (prob_manipulado_fake <= 0.5) + 0.80 * (prob_manipulado_fake > 0.5)
                        prob_array.append(prob_manipulado)
        if len(prob_array) > 0:
            prob_final = np.mean(prob_array)
        else:
            return 0.65
...
if __name__ == '__main__':
    try:
        ...
        models_folder = '/kaggle/input/saulbest2/'
        # modelsfake_folder = '/kaggle/input/awsauc01749/'
        ...
        try:
            classifier = models.load_model(models_folder + 'F6.1.3_Load_mejor_modelo_CNN_06-0.1721-0.94.hdf5')
            #classifier_fake = models.load_model(modelsfake_folder + '3.1._NEW_aws_tf_CNN_07-0.1749.hdf5') 
            #yaml_file_color = open('/kaggle/input/mejor-hasta-ahora-062logloss/modelo_chrom_map.yaml')
            #classifier_color_model_yaml = yaml_file_color.read()
            #yaml_file_color.close()
            # classifier = models.model_from_yaml(classifier_color_model_yaml)
            # classifier.load_weights(models_folder + '3.2aws_NEW_tf_CNN_20-0.3396-0.85.hdf5')
