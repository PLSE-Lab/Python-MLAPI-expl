...
dataset = pd.read_csv('../input/cancer.csv')
X = dataset.iloc[:, 1:31].values # independant variables
Y = dataset.iloc[:, 31].values # dependant variable
...
#
#RESULTS >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#1) LOGISTIC REGRESSION - 96% - 3rd
#2) NEAREST NEIGHBOR - 95% - 4th
#3) SUPPORT VECTOR MACHINE - 97% - 2nd
#4) KERNEL SUPPORT VECTOR MACHINE - 97% - 2nd
#5) NAIVE BAYES - 92% - 5th
#6) DECISION TREE - 96% - 3rd
#7) RANDOM FOREST - 99% - 1st
#
