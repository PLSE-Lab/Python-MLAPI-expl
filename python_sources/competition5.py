...
grid_search = GridSearchCV(forest, param_grid=p_grid, cv=cross_validation)
...
if AI == 1:
    ...
    # provides ehaustive search over specified parameter values for an estimator
    grid_search = GridSearchCV(SVC(C=1), tuned_parameters, cv=5)
    grid_search.fit(train_new, targets)
...
#    })
#print(df_output)
#df_output.to_csv('LinearSVC.csv', index=False)
print("DONE")
df_output = pd.DataFrame()
df_output['PassengerId'] = test['PassengerId']
df_output['Survived'] = output
df_output[['PassengerId','Survived']].to_csv('random_forest2.csv',index=False)