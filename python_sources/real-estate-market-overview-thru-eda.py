...
for i, txt in enumerate(Feature):
      ax.annotate(txt, (400,size[i]),fontsize=12,rotation=0,color='Red')
...
ax.annotate('2010 no of days', (50,50),fontsize=12,rotation=0,color='Green')
ax.annotate('2017 no of days', (50,51),fontsize=12,rotation=0,color='Blue');

ax.annotate('with % +/-', (50,49),fontsize=12,rotation=0,color='Brown');
...
# Median Sold Price details available for couple of states 
#          -----

# In[ ]:


distribution = sns.FacetGrid(State_house.dropna(), hue='RegionName',aspect=4)
distribution.map(sns.kdeplot,'MedianSoldPrice_AllHomes',shade=True)
distribution.add_legend();


# So we have the details of Expensive states here <br>
# **California is most expensive state and Arizona is less expensive to afford home compared to other states.**
...
v_features = State_house.iloc[:,6:15].columns
...
v_features = State_house.iloc[:,15:24].columns
plt.figure(figsize=(15,28))
gs = gridspec.GridSpec(9,1)
for i, cn in enumerate(State_house[v_features]):
    ax = plt.subplot(gs[i])
...
v_features = State_house.iloc[:,30:41].columns
...
v_features = State_house.iloc[:,41:52].columns
plt.figure(figsize=(15,32))
...
ax.annotate('2010 Home Value', (150000,35),fontsize=14,rotation=0,color='Green')
ax.annotate('2017 Home Value', (150000,36),fontsize=14,rotation=0,color='Blue');

ax.annotate('with Percent increase/Decrease', (150000,34),fontsize=14,rotation=0,color='Brown');
...
for i, txt in enumerate(Feature):
      ax.annotate(txt, (5000000,size[i]),fontsize=12,rotation=0,color='Red')
      ax.annotate('.', xy=(total[i], size[i]), xytext=(weightage[i], size[i]),
            arrowprops=dict(facecolor='LightGreen', shrink=0.06),
            )
for i, pct in enumerate(percent):
...
ax.annotate('2010 Home Value', (10000,19),fontsize=14,rotation=0,color='Green')
ax.annotate('2017 Home Value', (10000,20),fontsize=14,rotation=0,color='Blue');

ax.annotate('with % increase/Decrease', (10000,18),fontsize=14,rotation=0,color='Brown');
...
ax.annotate('2010 Home Value', (300000,183),fontsize=14,rotation=0,color='Green')
ax.annotate('2017 Home Value', (300000,184),fontsize=14,rotation=0,color='Blue');

ax.annotate('with % increase/Decrease', (300000,182),fontsize=14,rotation=0,color='Brown');
...
for i, txt in enumerate(Feature):
      ax.annotate(txt, (1200000,size[i]),fontsize=12,rotation=0,color='Red')
      ax.annotate('.', xy=(total[i], size[i]), xytext=(weightage[i], size[i]),
            arrowprops=dict(facecolor='LightGreen', shrink=0.06),
            )
for i, pct in enumerate(percent):
