...
# **It was modified from *"Titanic: Machine Learning from Disaster"* for learning.**
# 
...
d=len(train[train.Survived==0][train.Sex=='female'])
s=len(train[train.Survived==1][train.Sex=='female'])
print ("%d %d %f" % (s, d, s/(s+d)))
...
X = train.drop(['Survived',
              'SibSp', 'Room', 'Group_num', 'NorFare', 'Embarked_C',
                'Sex', 'Parch', 'NorAge', 'Embarked_Q', 'Embarked_S',
                ], axis=1)
y = train.Survived
X_train, X_test, y_train, y_test = train_test_split(X, y, random_state=42)
...
# build a neural network from the 1st layer to the last layer
model = Sequential([
    Dense(2, input_dim=X_train.shape[1]),
    Activation('softmax')
])
...
optobj = RMSprop(lr=0.1)#, rho=0.9)#, epsilon=1e-08, decay=0.0)
#optobj = opt.SGD(lr=0.1, momentum=0.0, decay=0.0, nesterov=False)
...
y_binary = to_categorical(y_train.as_matrix())
#y_binary = y_train.astype(int).as_matrix()

# training
model.fit(X_train.as_matrix(), y_binary, nb_epoch=3)

y_binary = to_categorical(y_test.as_matrix())
#y_binary = y_test.astype(int).as_matrix()
loss, accuracy = model.evaluate(X_test.as_matrix(), y_binary)
print ('%f %f' % (loss, accuracy))
...
def test(droplist):
  X = train.drop(['Survived'], axis=1).drop(droplist, axis=1)
  y = train.Survived
  X_train, X_test, y_train, y_test = train_test_split(X, y, random_state=42)
...
droplist=['SibSp', 'Deck', 'Room', 'Group_num', 'NorFare', 'Embarked_C',
                  'Title_Lady', 'Title_Master', 'Title_Miss', 'Title_Mr', 'Title_Mrs', 'Title_Officer',
                  'Title_Sir', 'Group_size_L', 'Group_size_M', 'Group_size_S',
                  'Sex', 'Parch', 'NorAge', 'NorNames', 'Embarked_Q', 'Embarked_S',
                  'Pclass_1', 'Pclass_2', 'Pclass_3']

keeps=['SibSp', 'Deck', 'Room', 'Group_num', 'NorFare', 
       ['Embarked_C', 'Embarked_Q', 'Embarked_S'],
       ['Title_Lady', 'Title_Master', 'Title_Miss', 'Title_Mr', 'Title_Mrs', 'Title_Officer', 'Title_Sir'], 
       ['Group_size_L', 'Group_size_M', 'Group_size_S'],
       'Sex', 'Parch', 'NorAge', 'NorNames',
       ['Pclass_1', 'Pclass_2', 'Pclass_3']]

records={}
for keep in keeps:
    tmpdroplist = droplist.copy()
    if type(keep) == list:
        for i in keep:
            tmpdroplist.remove(i)
    else:
        tmpdroplist.remove(keep)
    acc = test(tmpdroplist)
    if type(keep) == list:
        records[keep[0]] = acc
    else:
        records[keep] = acc
...
print('\nTesting ------------')
y_binary = to_categorical(y_test.as_matrix())
cost = model.evaluate(X_test.as_matrix(), y_binary, batch_size=len(X_test))
print('test cost:', cost)

rst = model.predict(X_test.as_matrix())
#print('rst: ' + str(rst))
r = []
for i in rst:
    if i[0] >= i[1]:
        ...
        r.append(1)
print (accuracy_score(y_test.as_matrix(), r))
