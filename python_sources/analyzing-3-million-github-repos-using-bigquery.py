...
print("Minimum git repo size is " , github_repo_sizes.min(), " bytes")
print("Maximum git repo size is " , github_repo_sizes.max(), " bytes");
...
plt.plot(github_repo_sizes.divide(1024*1024),color="black")
...
# 
# ### Which are the Popular Languages ?
...
plt.figure(figsize=(12,8))
plt.barh(y_pos,count,align='center',color=sns.color_palette("inferno",len(x_pos)))
...
github_repo_licenses = github_repos.query_to_pandas_safe(query7)
...
query8 ="""
        ...
        LIMIT 2000
        """
github_repos.estimate_query_size(query8)


# In[ ]:


github_repo_trending_repos = github_repos.query_to_pandas_safe(query8)
...
plt.figure(figsize=(12,9))
g = sns.barplot(y="repo_name", x="watch_count", data=github_repo_trending_repos[:20], palette="autumn")
plt.title('Trending Top 20 Github Repositories by their Watch Count')
plt.xlabel("");
...
file_count_by_license = github_repos.query_to_pandas_safe(query11, max_gb_scanned=6)
...
"""
github_repo_num_commits_distinct = github_repos.query_to_pandas_safe(query9, max_gb_scanned=10)
...
github_repo_num_python_files = github_repos.query_to_pandas_safe(query10, max_gb_scanned=10)
...
ORDER BY num_commits DESC
"""
github_repo_num_java_distinct = github_repos.query_to_pandas_safe(query14, max_gb_scanned=5.3)
...
# .. to be continued
# 
# Which are the most commonly used Python Packages??
