...
# # *Analyses of Regressions Models | House Prices*  
# **David Rivas, Ph.D.**

# # Summary
# 
# This kernel applies the classical Linear regression models for various regularization algorithms to a house prices data.     
# Prior to using these models subtantial classical data preprocessing is done.    
# 
# At the time of writing, it achieves a score of about 0.121 on the public LB, just using regression, no RF, no xgboost, no ensembling etc. This work is based on the references and will be further extended in the future.
...
# Notice above that only the numerical features AND only those for which skewness > 0.5 are transformed
...
# note that train at this stage was derived from the original train data only, 
# but without SalePrice, which was dropped above after defining y=train.SalePrice
# where SalePrice is the log1p of the original SalePrice, obviously from the original train data only 
# Therefore, the following X_train, X_test, y_train and y_test are all derived from the original train data only
...
# We could have shuffled the data for the cross validation, but we have not done so; maybe this has something to do with it.
...
# # Conclusion  
# 
# Putting time and effort into preprocessing the dataset and optimizing the regularization resulted in a decent score, better than some public scripts which use algorithms that historically perform better in Kaggle contests, like Random Forests.   
...
# [Regularized Linear Models](https://www.kaggle.com/apapiu/regularized-linear-models) by Alexandru Papiuerences Used  
# 
