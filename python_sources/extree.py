...
target = trainlabels
print('Training...')
extc = ExtraTreesClassifier(n_estimators=1600,max_features= 29,criterion= 'entropy',min_samples_split= 1,
                            max_depth= 40, min_samples_leaf= 1, n_jobs = -1)      

extc.fit(X_train,target) 
x_pred = extc.predict_proba(X_train)
print(log_loss(trainlabels, np.clip(x_pred[:,1]*1.035, 1e-8, 1-1e-8)))
print('Predict...')
