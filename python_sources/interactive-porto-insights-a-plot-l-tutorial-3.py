...
# This is the thrid time that I have rewritten th kernel of [Interactive Porto Insights - A Plot.ly Tutorial](https://www.kaggle.com/arthurtok/interactive-porto-insights-a-plot-ly-tutorial) made by [Anisotropic](https://www.kaggle.com/arthurtok).

# ## Introduction
# 
# This competition is hosted by the third largest insurance company in Brazil: Porto Seguro with the task of predicting the porbability that a driver will initiate an insurance claim in the next year. 
# 
# This notebook will aim to provide some interactive charts and analysis of the competition data by way of the Python visualisation library Plot.ly and hopefully bring some insights and beauriful plots that others can take and replicate. Plot.ly is one of the main products offered by the software company - Pot.ly which specialized in providing online graphical and statistical visualisations (charts and dashboards) as well as providing an API to a whole rich suite of programming languages and tools such as Python, R, Matlab, Node.js, etc.
# 
...
# 
# 1. Data Quality Checks: Visualising and evaluating all missing/Null values
# 2. Features inspection and filtering: Correlation and feature Mutual information plots against the target variable. Inspection of the Binary, categorical and other varialbes. 
...
# Let's load in the training data provided using Pandas:
...
# 
# As part of our quality checks, le us quick look at whether there are any null values in the train dataset as follows:
...
# - Our null values check returns False but however, this does not really mean that this case has been closed the data is also described as 'Values of -1 indicate that the feature was missing from the observation'. Therefore I take it that Porto Seguro has simply conducted a blaket replacement of all null values in the data with the value of -1. Let us now inspect if there where any missing values in the data. 
# 
...
# Next, we can use resident Kaggler's Aleksey Bilgur - creator of the 'Missingno' package which is a most useful and convenient tool in visualising missing values in the dataset, so check it out.
...
# 
# Another point to note is that Porto Seguro has actually provided us data with headers that come suffixed with abbreviations such as 'bin', 'cat' and 'reg', where they have given us a rough explanation that bin dindicate binary features while cat indicates categorical features whilst the rest are either continuous or ordinal features. Here I shall simplify this a bit further just by looking at float values (probably only the continuous features) and integer datatypes (binary, categorical and ordianl features). 
...
# Similarly, we can observe that there are a huge number of columns that are not linearly correlated with each other at all, evident from the fact that we observe quite a lot of 0 value cells in our correlation plot. This is quite a useful observation to us, especially if we are trying to perform dimensionality reduction transformation such as Principal Component Analysis, this would require a certain degree of correlation. We can note some features of interest are as follows:
# 
# Negatively correlated features: ps_ind_06_bin, ps_ind_07_bin, ps_ind_08_bin, ps_ind_09_bin
# 
# One interesting aspect to note is that in our earlier analysis on nullity, ps_car_03_cat and ps_car_05_cat were found to contain many missing or null values. Therefore it should come as no surprise that both these features show quite a strong positive linear correlation to each other on this basis, albeit one that may not really reflect the uderlying truth for the data.
...
# 
# The Sklearn implementation of the mutual_info_classif function tells us that it relied on nonparametric methods based on entropy estimation from k-nearest neighbors distances, where you can go into more detail on the official sklearn page in the link here. 
...
# 
# Another aspect of the data that we may want to inspect would be the columnds that only contain binary values, ie, where values take on only either of the two values 1 or 0. Proceeding, we store all columns that contain these binary values and then generagte a vertical plotly barplot of these binary values as follows:
...
# Here we observe that there are 4 features: ps_ind_10_bin, ps_ind_11_bin, ps_ind_12_bin, ps_ind_13_bin which are completely dominated by zero. This begs the question of whether these features are useful at all as they do not contain much information about the other class vis-a-vis the target.

# ### Categorical and Ordinal feature inspection
# 
# Let us first take a look at the features that are termed categorical as per their suffix 'cat.'

# ### Feature importance via Random Forest
# 
# Let us now implement a Random Forest model where we fit the training data with a Random Forest Classifier and look at the ranking of the features after the model has finished training. This is a quick way of using an ensemble model (ensemble of weak decision tree learners applied under Bootstrap aggregated) which does not require much parameter tuning in obtaining useful feature importances and is also pretty robust to target imbalances. We call the Random Froest as follows:
...
# 
# Having trained the Random Forest, we can obtain the list of feature importances by invoking the attribute 'feature importances' and plot out next Plotly plot, the Scatter plot. 
# 
...
# Furthermore, we could also diaply a sorted list of all the features ranked by order of their importances, from highest to lowest via the same plotly barplots as follows:
...
trace2 = go.Bar(
    ...
    name='Random Forest Feature Imprtance',
    orientation='h'
)
layout=dict(
...
# 
# One other interesting trick or technique of used would be to visualize the tree branches or decisions made by the model. For simplicity, I fit a decision tree (of max_depth=3) and hence you only see 3 levels in the decision brach, use the export to graph visualization attribute in sklearn 'export_graphviz' and then export and import the tree image for visualization in this notebook.
...
# ### Featue importance via Gradient Boosting model
# 
...
trace2 = go.Bar(
    ...
    name='Gradient Boosting Classifier Feature Imprtance',
    orientation='h'
)
layout=dict(
    title='Barplot of Feature Importance',
