...
# ### Note that these are some possible answers and that yours may look different but still be perfectly fine.
# 
...
PROJECT_ID = 'kaggle-bike-share-project'  # a string, like 'kaggle-bigquery-240818'

from google.cloud import bigquery
client = bigquery.Client(project=PROJECT_ID, location="US")
dataset = client.create_dataset('austin_bikeshare', exists_ok=True)
...
#plt.plot(df.start_hour, df.num_rides)
...
get_ipython().run_cell_magic('bigquery', '', '\nSELECT *\nFROM\nML.EVALUATE(MODEL `austin_bikeshare.bike_trips`, (\nSELECT COUNT(bikeid) as label, \n       start_station_name, \n       TIMESTAMP_TRUNC(start_time, HOUR) as start_hour\nFROM `bigquery-public-data.austin_bikeshare.bikeshare_trips`\nWHERE start_time >= "2018-01-01"\nGROUP BY start_station_name, start_hour\n))')
...
# outliers or missing values would cause the model to perform poorly . 
...
# The model can be improved by removing outliers. Or 2018 can be divided in to 2 and some can go to traininng and some can be left for test. 
