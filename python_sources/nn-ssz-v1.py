...
def make_dataset(useDummies = True, fillNANStrategy = "mean", useNormalization = True):
    ...
    #For CV only: (comment out if for LB predictions)
    test=train.iloc[0:20000,]
    train=train.iloc[20001:59380,]
    
    test_target=labels.iloc[0:20000]
    test_target[1:4,]
    test_target.to_csv('nn_cv_target.csv', index=False)
...
np.savetxt("NNSubmission_ssz_v1_cv.csv", pred, delimiter=",")

'''
predClipped = pred #Make the submissions within the accepted range

submission = pd.read_csv('../input/sample_submission.csv')
submission["Response"] = predClipped

submission.to_csv('NNSubmission_ssz_v1_cv.csv', index=False)
'''
