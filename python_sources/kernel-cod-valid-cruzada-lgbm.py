...
num_submissao = 29
##-----------------------------------------------------------------------------------------##
print('Carregando dataframes para a memória')
train_df = pd.read_csv('../input/train.csv') # lendo base de treino
test_df = pd.read_csv('../input/test.csv') # lendo base de teste
features = [c for c in train_df.columns if c not in ['ID_code', 'target']]
print('Separando coluna de saida')
target = train_df['target']
print('Criando grid de parâmetros')
param = {
...
##-----------------------------------------------------------------------------------------##
print('Escrevendo resultados num arquivo de texto')
sub = pd.DataFrame({"ID_code": test_df.ID_code.values})
sub["target"] = predictions
sub.to_csv('submission{}.csv'.format(num_submissao), index=False)
