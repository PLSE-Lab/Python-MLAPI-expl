...
# sliced_data=data.loc[(data['Province_State']=='')&(data['Country_Region']=='Armenia'),:]
# sliced_data=sliced_data.reset_index(drop=True)
# train=sliced_data['ConfirmedCases'].to_list()[:int(sliced_data.shape[0]*0.8)]
# test=sliced_data['ConfirmedCases'].to_list()[int(sliced_data.shape[0]*0.8):]
...
sumb=pd.read_csv('/kaggle/input/covid19-global-forecasting-week-2/submission.csv')
output=pd.DataFrame()
