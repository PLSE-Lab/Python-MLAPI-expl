...
# ## Goal
# 
# This notebook aims at understanding the factors that contributed for people to survive the sinking of the Titanic. 
# 
# ## Strategy
# 
...
# 1. Check if there are missing values and if so, impute them if it's feasible.
# 1. Engineer some variables that may bring more insigths or descriminative power for the modelling.
...
# import pandas_profiling, install it if it's not installed
try:
...
# The data schema of `train` and `test` match, with the exception that train has an extra variable, the dependent variable. Thus it's possible to concatenate `train` and `test` into just one data frame. This will facilitate the pre-processing of the data and being able to obtain better descriptive statistics.

# In[ ]:


titanic = pd.concat([train, test], sort=True).reset_index()
titanic.info()
...
# profile data
pandas_profiling.ProfileReport(titanic.copy())


# #### Data Dictionary / Some Observations
# 
...
# - `Pclass` is strongly correlated with `Fare` and it works as a proxy for economic status.
# - `Name` maybe is useful to extract the passenger's title and the family surname.
# - `Age` can be re-written as age bins and used as a feature.
...
# All 4 passengers travelled on the same ticket and had 4 cabins, looking at their surnames, number of parents/children and ages I'd say passengers 680 and 1235 were a mother and son while the other 2 passengers could be their servants.
