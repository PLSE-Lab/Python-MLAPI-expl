...
my_inputer = SimpleImputer(strategy='median') # Your code here
imputed_X = pd.DataFrame(my_inputer.fit_transform(X))
imputed_X_test = pd.DataFrame(my_inputer.transform(X_test))
...
indices = list(X_test_full.index)
indices.sort()
indices
...
# All categorical columns
object_cols_test = [col for col in X_test_full.columns                if X_test_full[col].dtype == "object"]
...
for col in object_cols_test:
    X_test[col] = X_test_full[col].fillna('None') 
for col in object_cols_test:
