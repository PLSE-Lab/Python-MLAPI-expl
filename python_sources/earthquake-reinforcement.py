...
m_1 = folium.Map(location=[35,136], tiles='cartodbpositron', zoom_start=2)
for i in range(len(plate_boundaries)):
    folium.PolyLine(locations=plate_boundaries.coordinates.iloc[i], weight=2, color='black').add_to(m_1)

# Your code here: Add a heatmap to the map
HeatMap(data=earthquakes[["Latitude", "Longitude"]], radius=19).add_to(m_1)
...
def depth_color(val):
    ...
    if val <= earthquakes["Depth"].median():
        return "forestgreen"
    else:
        return "darkred"

for _, earthquake in earthquakes.iterrows():
    Circle(location=[earthquake["Latitude"],
                     earthquake["Longitude"]],
           radius=20,
           color=depth_color(earthquake["Depth"])).add_to(m_2)
