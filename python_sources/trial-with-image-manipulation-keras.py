...
#setting train features into nparrays
x_train = df_train.values
x_train = np.reshape(x_train,(-1,28,28))

#deskew train images
x_train_new = []
for loopImage in x_train:
    (h, w) = loopImage.shape[:2]
    moments = cv2.moments(loopImage)
    skew = moments["mu11"] / moments["mu02"]
    M = np.float32([
      [1, skew, -0.5 * w * skew],
      [0, 1, 0]])
    loopImage = cv2.warpAffine(loopImage, M, (w, h),
      flags = cv2.WARP_INVERSE_MAP | cv2.INTER_LINEAR)
    x_train_new.append(loopImage)

#normalizing & reshaping train pixel values
x_train_new = np.array(x_train_new).astype("float32")
x_train_new = x_train_new / 255
x_train_new = np.reshape(x_train_new,(-1,28,28,1))
x_train = x_train_new
...
#setting test features into nparrays
x_test = df_test.values
x_test = np.reshape(x_test,(-1,28,28))

#deskew test images
x_test_new = []
for loopImage in x_test:
    (h, w) = loopImage.shape[:2]
    moments = cv2.moments(loopImage)
    skew = moments["mu11"] / moments["mu02"]
    M = np.float32([
    ...
    loopImage = cv2.warpAffine(loopImage, M, (w, h),
      flags = cv2.WARP_INVERSE_MAP | cv2.INTER_LINEAR)
    x_test_new.append(loopImage)

#normalizing & reshaping test pixel values
x_test_new = np.array(x_test_new).astype("float32")
x_test_new = x_test_new / 255
x_test_new = np.reshape(x_test_new,(-1,28,28,1))
x_test = x_test_new
