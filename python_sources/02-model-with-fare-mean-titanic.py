...
def prepare_data(df):
    ...
    # there is NaN in Fare column in test data
    mean_fares = np.zeros((2, 3))

    for i in range(0, 2):
        for j in range(0, 3):
            mean_fares[i, j] = df[(df['Gender'] == i) & (df['Pclass'] == j + 1)]['Fare'].dropna().mean()

    df['FareFill'] = df['Fare']

    for i in range(0, 2):
        for j in range(0, 3):
            df.loc[ (df.Fare.isnull()) & (df.Gender == i) & (df.Pclass == j + 1), 'FareFill'] = mean_fares[i, j]
