...
#  - Rather than splitting the data once and using the validation data for the LGBM early stopping, I create a K-Fold split and train each classifier on each fold.
#  
...
def _parallel_fit_estimator(estimator, X, y, train_index, test_index, sample_weight=None, **fit_params):
...
class VotingClassifierLGBM(VotingClassifier):
    ...
    def fit(self, X, y, sample_weight=None, **fit_params):
        ...
        self.estimators_ = Parallel(n_jobs=self.n_jobs)(
                delayed(_parallel_fit_estimator)(clone(clf), X, transformed_y, train_index, test_index,
                                                 sample_weight=sample_weight, **fit_params)
                for (clf, (train_index, test_index)) in product(clfs, folds))
...
for i in range(10):
    clf = lgb.LGBMClassifier(max_depth=-1, learning_rate=0.1, objective='multiclass',
                             random_state=314+i*2, silent=True, metric='None', 
