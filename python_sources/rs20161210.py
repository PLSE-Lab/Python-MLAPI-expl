# -*- coding: utf-8 -*-
"""
Created on Sat Dec 10 15:24:46 2016

@author: Rodrigo
"""

__author__ = 'Rodrigo Salas Vallet-Cendre rasvc@hotmail.com'
...
def get_im_cv2(path):
    img = cv2.imread(path)
    resized = cv2.resize(img, (ancho, ancho), cv2.INTER_LINEAR)
    return resized
...
def create_model():
    model = Sequential()
    model.add(ZeroPadding2D((1, 1), input_shape=(3, ancho, ancho), dim_ordering='th'))
    ...
    model.add(ZeroPadding2D((16, 1), dim_ordering='th'))
    model.add(Convolution2D(8, 3, 3, activation='relu', dim_ordering='th',init='he_uniform'))
    model.add(MaxPooling2D(pool_size=(2, 2), strides=(2, 2), dim_ordering='th'))
...
def run_cross_validation_create_models(nfolds,model):
...
if __name__ == '__main__':
    ...
    info_string, models = run_cross_validation_create_models(num_folds,model)
