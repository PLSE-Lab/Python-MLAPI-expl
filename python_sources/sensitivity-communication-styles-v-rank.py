...
# This script makes different choices to create the Passive/Assertive/Aggressive 
#   communication style vs rank analysis, to explore its sensitivity to keyword choices

sql_conn = sqlite3.connect('../input/database.sqlite')

df = pd.read_sql("SELECT score, body FROM May2015 WHERE LENGTH(body) > 5 AND LENGTH(body) < 100 LIMIT 10000", sql_conn)
    
keywords = pd.DataFrame({'Passive': pd.Series(['equal','thanks','okay','both','valid point']),
                         'Assertive': pd.Series(['better','should','could','would','opinion']),
                         'Aggressive': pd.Series(['genius','GTFU','great','hella','terrible'])})
