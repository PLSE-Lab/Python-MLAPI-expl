...
fit= model.fit_generator(generator=batch_generator(Xtrain, dummy_y, 32, True),
                         nb_epoch=15,
...
# evaluate the model
# scores_val = model.predict_generator(generator=batch_generatorp(X_val, 32), val_samples=X_val.shape[0])
# print('logloss val {}'.format(log_loss(y_val, scores_val)))

scores = model.predict_generator(generator=batch_generatorp(Xtest, 32), val_samples=Xtest.shape[0])
...
pred.to_csv('nnet_keras_full_test.csv',index=True)
