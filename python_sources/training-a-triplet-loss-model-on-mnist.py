...
# Loads the MNIST dataset from CSV files. Defines the functions used to sample batches from dataset, and in order to do so the dataset is reorganized in a more convenient way with a dictionary, where the keys access all the samples from each class
...
# Looking at the t-SNE embeddings from the raw pixel data can give us a baseline of how the data is distributed before any clustering work is done by the Triplet Loss model.
...
# The Triplet Loss function is defined based on the implementations of [FaceNet](https://github.com/davidsandberg/facenet) and [In Defense of the Triplet Loss for Person Re-Identification
# ](https://github.com/VisualComputingInstitute/triplet-reid). 
# 
# There's a lot of little modifications that can be made to the implementation, like defining a soft margin, or using different distance functions. This implementation defines a hard margin at 0.2 and uses the Euclidean Distance (not Squared Euclidean Distance such as in FaceNet) as the loss function.
