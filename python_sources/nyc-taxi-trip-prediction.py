...
kmeans_pickup_time = KMeans(n_clusters=24, random_state=2).fit(
    df.pickup_minute_of_the_day[:500000].values.reshape(-1, 1))
...
def network(num_layers, n_inputs, weights, biases, rate, is_training, activation_function):
    ...
    for layer in range(1, num_layers):
        if layer == 1:
            ...
            current_layer = eval(
                activation_function + "(tf.matmul(previous_layer,            weights['h'+str(layer)]) + biases['b'+str(layer)])")
            current_layer = tf.nn.dropout(current_layer, 1 - rate)
            previous_layer = current_layer
...
def train(model, epochs, log_string, learning_rate):
    ...
    with tf.Session() as sess:
        ...
        training_check = (len(
            x_train) // batch_size // per_epoch_training) - 1  # Check training progress after this many batches
        testing_check = (len(
            x_train) // batch_size // per_epoch_testing) - 1  # Check testing results after this many batches
...
for result in sorted_results_nn:
    ...
    learning_rate, learning_rate_decay, weights_multiplier, n_inputs, num_layers, dropout_rate, batch_size, max_nodes, activation_function = find_inputs(
        result[0])
