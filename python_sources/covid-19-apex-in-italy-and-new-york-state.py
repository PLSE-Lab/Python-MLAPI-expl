...
# **Disclaimer**
# 
# *I am a published author of suspense and young adult novels as well as political scientist, not a coder or data scientist. This small notebook is first and foremost a part of my personal learning project during which I have tried to find relevant factors concerning the apex phase of the current COVID-19 pandemic. There is no model building included in this notebook, and all analysis conclusions are subjective.*
# 
...
# 
# ***The analysis part of this notebook has now been concluded. The plot graphics etc. will however update themselves barring any modifications to the original datasets.*** 
...
# 
# Below is the plot of the *hospitalizedCurrently* column from New York state. 
...
start_date = '2020-04-15 20:00:00'
...
# 
# Next we take a look at how the daily (current) hospitalizations correlate with the number of recovered patients. At the beginning of June, the recovery rate in New York had fallen under 1:20 (about 3000 hospitalized patients and over 60K recovered patients).
...
fig.add_trace(go.Bar(
    x=df_la['NewDate'],
    y=df_la['hospitalizedCurrently'],
    name='Hospitalized (current)',
    marker_color='indianred'
))
fig.add_trace(go.Bar(
    x=df_la['NewDate'],
...
fig.update_layout(annotations=annotations)
fig.update_layout(barmode='group', xaxis_tickangle=-25)
fig.update_layout(title_text='<b>COVID-19</b>:<br>level of daily hospitalization and recovery in New York state',                  
      font=dict(family='calibri',
        size=12,
        color='rgb(64,64,64)'),
       legend=dict(
...
# mask dataframe
start_date = '2020-03-15 17:00:00'
mask = (df_states['Date'] > start_date)
df_states = df_states.loc[mask]

# plot figure
fig = px.line(df_states, x="Date", y="positive",color='state')
