...
medianAgeTrain = train_df['Age'].median()
train_df['Age'].fillna(medianAgeTrain,inplace=True)
medianAgeTest = test_df['Age'].median()
test_df['Age'].fillna(medianAgeTest,inplace=True)
#rain_df['Age'].replace('NaN', medianAge)
train_df.head(50)
test_df.head(100)
...
# 1.  Embarked = C has high survival rate
# 2.
...
train_df[['Sex','Embarked']].groupby(['Embarked'], as_index=False).count()
...
# 
# **Observations - Prasun**
# - Male + Embarked =C+ Pclass = 1&2 have high survival rate 
# - Male + Class =3 have low survival rate in all cases. For Embarked = C even male survival seems better, absolute numbe of 
# 
# 
# **Decisions.**
# 
...
# 
# We dont need Title feature as Sex + Age group combination will provide required data
...
train_df.to_csv('C:\Users\prasun.mishra\Desktop\train.csv')
test_df.to_csv('C:\Users\prasun.mishra\Desktop\test.csv')
...
sgd = SGDClassifier(alpha=0.0001, average=False, class_weight=None, epsilon=0.15,
...
models = pd.DataFrame({
    'Model': ['Lasso','Support Vector Machines', 'KNN', 'Logistic Regression', 
              'Random Forest', 'Naive Bayes', 'Perceptron', 
              'Stochastic Gradient Decent', 'Linear SVC', 
              'Decision Tree'],
    'Score': [acc_lasso,acc_svc, acc_knn, acc_log, 
