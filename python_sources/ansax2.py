...
param['colsample_bytree'] = 0.8 #0.75
param['seed'] = 0
param['gamma'] = 1
...
if cv :
        ...
        for dev_index, val_index in kf.split(range(df_train.shape[0])):
                ...
                model2 = xgb.train(plst2, xgtrain, num_rounds, watchlist, early_stopping_rounds=20)
...
print ('actual XGB train OK')

model2 = xgb.train(plst2, xgtrain, num_rounds)
print ('actual XGB train2 OK')

pred_test_xgb  = model.predict(xgtest)
pred_test_xgb2  = model2.predict(xgtest)
y=(pred_test_xgb )
#y=(pred_test_xgb +pred_test_xgb2)/2
...
sub['low']=ff2  
print (sub)
sub.to_csv("subansa1_rf.csv", index=False)
print ('finitto')
