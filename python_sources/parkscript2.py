...
# if it appear as so little occurrence, we eliminate that.
...
clf_xgb = XGBClassifier(max_depth=7, learning_rate=0.012, n_estimators=1000, subsample=0.62, colsample_bytree=0.6, seed=2016)
clf_xgb.fit(X, y)
...
submission.to_csv("ParkScript.csv",index=False)
