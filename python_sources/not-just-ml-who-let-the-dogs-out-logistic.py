...
clf = LogisticRegression(random_state=1, multi_class='multinomial', solver='saga',max_iter = 5000,verbose=1)
clf
...
model = clf.fit(preds_trn,trn_DV)


# In[ ]:


print("Log loss is: ", log_loss(val_DV, pd.DataFrame(model.predict_proba(preds_val))))
print("Accuracy is: ", accuracy_score(val_DV, pd.DataFrame(model.predict(preds_val))))
