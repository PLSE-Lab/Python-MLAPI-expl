...
# Important parameters: Kernel, gamma, C

# kernel: The functions (except 'linear') that helps to convert the 2d plot to 3d to get the decision boundary.
# 	  example:  ‘linear’, ‘rbf’, ‘poly’, ‘sigmoid’, ‘precomputed’ 
	
# gamma: higher the gamma higher will be influence of all points in drawing the DB.
# 	     example: large value of gamma gives smooth DB but classifying less training points correctly(type:float)

# C:   controls the tradeoff between smooth decision boundaey and classifying training points correctly.
# Example: small value of C gives smooth DB but classifying less training points correctly (type: float)
