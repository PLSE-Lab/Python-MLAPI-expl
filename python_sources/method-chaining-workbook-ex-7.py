...
# 1.
chess_games.groupby(['white_id','victory_status']).size().reset_index()
# or below answer is perfect with question 
# 2.
chess_games['n'] = 0
...
(chess_games
     ...
     .n.apply(len)
     .reset_index()
     .pipe(lambda df:df.loc[df.white_id
                                .isin(chess_games
                                          .white_id
                                          .value_counts()
                                          .head(20)
...
ramen_ratings = (ramen_reviews.Stars.replace("Unrated",None).dropna().astype('float64').map(lambda v : round(v * 2)/2).value_counts().rename_axis('Ramen Reviews').sort_index())
ramen_ratings
