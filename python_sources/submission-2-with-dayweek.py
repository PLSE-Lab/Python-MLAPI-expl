...
def prepare_arrays_match():
    ...
    while 1:
        ...
        ### Mycode
        weekday_created = datetime.datetime(int(arr[0][:4]),int(arr[0][5:7]),int(arr[0][8:10]),5,5,5).weekday()
        if arr[11] != '':        
            weekday_booking = datetime.datetime(int(arr[11][:4]),int(arr[11][5:7]),int(arr[11][8:10]),5,5,5).weekday()
        else:
            weekday_booking = -1
        if arr[11] != '' and arr[12] != '':
            stay_duration = (datetime.datetime(int(arr[12][:4]),int(arr[12][5:7]),int(arr[12][8:10]),5,5,5) - datetime.datetime(int(arr[11][:4]),int(arr[11][5:7]),int(arr[11][8:10]),5,5,5)).days
        else:
            stay_duration = -1
          
          
        if srch_destination_id != '' and hotel_country != '' and is_booking==1:
            w = (weekday_created, weekday_booking, stay_duration, srch_destination_id, hotel_country, hotel_market)
    ...
    f.close()
    return best_s00,best_s01, best_hotels_country, best_hotels_od_ulc, best_hotels_uid_miss, best_hotels_search_dest, popular_hotel_cluster, best_hotels_dates


def gen_submission(best_s00, best_s01,best_hotels_country, best_hotels_search_dest, best_hotels_od_ulc, best_hotels_uid_miss, popular_hotel_cluster, best_hotels_dates):
    ...
    while 1:
        ...
        #test_cols = ['index','date_time','site_name','posa_continent','user_location_country',
        ...
        ### Mycode
        try:
            weekday_created = datetime.datetime(int(arr[1][:4]),int(arr[1][5:7]),int(arr[1][8:10]),5,5,5).weekday()
            if arr[12] != '':   
                weekday_booking = datetime.datetime(int(arr[12][:4]),int(arr[12][5:7]),int(arr[12][8:10]),5,5,5).weekday()
            else:
                weekday_booking = -1
            if arr[13] != '' and arr[12] != '':
                stay_duration = (datetime.datetime(int(arr[13][:4]),int(arr[13][5:7]),int(arr[13][8:10]),5,5,5) - datetime.datetime(int(arr[12][:4]),int(arr[12][5:7]),int(arr[12][8:10]),5,5,5)).days
            else:
                stay_duration = -1        
        except:
            weekday_created = -1
        ...
        ### Mycode
        sw = (weekday_created, weekday_booking, stay_duration, srch_destination_id, hotel_country, hotel_market)
        if weekday_created != -1 and weekday_booking != -1 and stay_duration != -1 and sw in best_hotels_dates:
...
best_s00,best_s01,best_hotels_country, best_hotels_od_ulc, best_hotels_uid_miss, best_hotels_search_dest, popular_hotel_cluster, best_hotels_dates = prepare_arrays_match()
gen_submission(best_s00, best_s01,best_hotels_country, best_hotels_search_dest, best_hotels_od_ulc, best_hotels_uid_miss, popular_hotel_cluster, best_hotels_dates)
