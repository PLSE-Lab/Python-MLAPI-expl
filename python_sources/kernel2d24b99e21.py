...
project_name = 'Assignment03-cifar10-feedforward-neuralnetwork'
...
class CIFAR10Model(ImageClassificationBase):
    def __init__(self):
        super().__init__()
        self.c1 = nn.Linear(input_size, 128)
        self.c2 = nn.Linear(128, 512)
        self.c3 = nn.Linear(512, output_size)
    ...
    def forward(self, xb):
        ...
        # Outputs using hidden layer 1
        out = self.c1(out)
        # Activation function
        out = F.relu(out)
        #Outputs using hidden layer 2
        out = self.c2(out)
        # Activation function
        out = F.relu(out)
        # Outputs using hidden layer 3
        out = self.c3(out)
        return out
...
history += fit(12, 1e-4, model, train_loader, val_loader)
