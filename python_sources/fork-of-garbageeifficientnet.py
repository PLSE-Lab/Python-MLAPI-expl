...
trn_idx = list(StratifiedKFold(n_splits=10, random_state=24, shuffle=True).split(trn_csv, trn_csv.cls))[fold][0]
val_idx = list(StratifiedKFold(n_splits=10, random_state=24, shuffle=True).split(trn_csv, trn_csv.cls))[fold][1]
...
criterion =  LSR()
scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[1, 3, 5, 7, 9], gamma=0.5)
