...
# Identify the sentiments which score highly
# Separate and score the dataset into the top quartile by the sentiments:
# Positive
# Negative
# Objective
# Subjective
...
df = pd.read_sql("SELECT score, body FROM May2015 WHERE LENGTH(body) > 30 AND LENGTH(body) < 250 LIMIT 100", sql_conn)
...
# get the above average (top quartile or top 3/8) comments for each sentiment
...
print(pos_content.describe())

print(pos_content.describe().score)
...
ax.set_title('Which sentiments receives highest ranks?')
