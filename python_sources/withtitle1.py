...
##End of experiment
#Setup bg groupby object
bg=df_train[["building_id","price"]].groupby(['building_id'])['price'].mean().reset_index()
bg['average_blg_price']=np.log(bg['price'])
bg.drop('price', axis=1, inplace=True)
df_train = df_train.merge(bg,how='left', left_on='building_id', right_on='building_id')
df_train["average_blg_price"]=np.log(df_train["average_blg_price"])
...
df_test = df_test.merge(bg,how='left', left_on='building_id', right_on='building_id')
df_test["average_blg_price"]=np.log(df_test["average_blg_price"])
...
#***
df_train["average_blg_bed_price"]=df_train["average_blg_price"]/df_train["bedrooms"]
...
df_test["average_blg_bed_price"]=df_test["average_blg_price"]/df_test["bedrooms"]
...
sub.to_csv("subbest1_rf.csv", index=False)
