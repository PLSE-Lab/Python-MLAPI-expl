# This script creates and evaluates a Naive Bayes model in Python for the 'San Francisco Crime Classification' competition.
...
nb.fit(train[['day_of_week_encoded', 'pd_district_encoded', 'x', 'y', 'hour']],
...
train['pred'] = nb.predict(train[['day_of_week_encoded', 'pd_district_encoded', 'x', 'y', 'hour']])
test_pred = nb.predict_proba(test[['day_of_week_encoded', 'pd_district_encoded', 'x', 'y', 'hour']])
...
cv_scores = cross_validation.cross_val_score(nb,
                                             train[['day_of_week_encoded', 'pd_district_encoded', 'x', 'y', 'hour']],
                                             train['category_predict'],
                                             cv = 2)
