...
get_ipython().system('git clone https://github.com/pjreddie/darknet.git')

# Build gpu version darknet
get_ipython().system("cd darknet && sed '1 s/^.*$/GPU=1/; 2 s/^.*$/CUDNN=1/' -i Makefile")

# -j <The # of cpu cores to use>. Chang 999 to fit your environment. Actually i used '-j 50'.
get_ipython().system('cd darknet && make -j 50 -s')
get_ipython().system('cp darknet/darknet darknet_gpu')
...
get_ipython().system('du -sh images labels')
...
get_ipython().system('cat cfg/rsna.data')
...
get_ipython().system('echo "pneumonia" > cfg/rsna.names')
...
get_ipython().system('wget -q https://pjreddie.com/media/files/darknet53.conv.74')
...
get_ipython().system('wget --no-check-certificate -q "https://docs.google.com/uc?export=download&id=18ptTK4Vbeokqpux8Onr0OmwUP9ipmcYO" -O cfg/rsna_yolov3.cfg_train')
...
get_ipython().system('./darknet_gpu detector train cfg/rsna.data cfg/rsna_yolov3.cfg_train darknet53.conv.74 -i 0 | tee train_log.txt')
...
get_ipython().system('wget --no-check-certificate -q "https://docs.google.com/uc?export=download&id=1OhnlV3s7r6xsEme6DKkNYjcYjsl-C_Av" -O train_log.txt')
...
ex_patient_id = annots[annots.Target == 1].patientId.values[20]
ex_patient_id = test_dcm_fps[230]
ex_img_path = os.path.join(img_dir, "{}.jpg".format(ex_patient_id))
plt.imshow(cv2.imread(ex_img_path))
...
get_ipython().system('wget --load-cookies /tmp/cookies.txt -q "https://docs.google.com/uc?export=download&confirm=$(wget --quiet --save-cookies /tmp/cookies.txt --keep-session-cookies --no-check-certificate \'https://docs.google.com/uc?export=download&id=1FDzMN-kGVYCvBeDKwemAazldSVkAEFyd\' -O- | sed -rn \'s/.*confirm=([0-9A-Za-z_]+).*/\\1\\n/p\')&id=1FDzMN-kGVYCvBeDKwemAazldSVkAEFyd" -O backup/rsna_yolov3_15300.weights && rm -rf /tmp/cookies.txt')


# In[ ]:


get_ipython().system('ls -alsth backup')
...
get_ipython().system('wget --no-check-certificate -q "https://docs.google.com/uc?export=download&id=10Yk6ZMAKGz5LeBbikciALy82aK3lX-57" -O cfg/rsna_yolov3.cfg_test')


# In[ ]:


get_ipython().system('cd darknet && ./darknet detector test ../cfg/rsna.data ../cfg/rsna_yolov3.cfg_test ../backup/rsna_yolov3_15300.weights ../test.jpg -thresh 0.005')
...
get_ipython().system('wget --no-check-certificate -q "https://docs.google.com/uc?export=download&id=1-KTV7K9G1bl3SmnLnzmpkDyNt6tDmH7j" -O darknet.py')
...
# !ls -lsht
get_ipython().system('rm -rf darknet images labels metadata backup cfg')
get_ipython().system('rm -rf train_log.txt darknet53.conv.74 darknet.py darknet_gpu')
get_ipython().system('rm -rf test.jpg')
get_ipython().system('rm -rf __pycache__ .ipynb_checkpoints')


# In[ ]:


get_ipython().system('ls -alsht')
