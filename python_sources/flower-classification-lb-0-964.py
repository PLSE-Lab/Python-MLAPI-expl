...
with strategy.scope():
    ...
    model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001, beta_1=0.95, beta_2=0.999, amsgrad=False),
...
with strategy.scope():
    dn201 = tf.keras.applications.DenseNet201(weights='imagenet', include_top=False, input_shape=(512,512,3))
...
probabilities = 0.7*model.predict(test_images_ds) + 0.3* model2.predict(test_images_ds)
