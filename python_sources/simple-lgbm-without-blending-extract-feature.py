...
def historical_transactions(num_rows=None):
    ...
    hist_df_1 = hist_df[hist_df['authorized_flag'] == 1]

    hist_df_1.drop(columns=['authorized_flag'])

    hist_df_1 = reduce_mem_usage(hist_df_1)
    ...
    aggs_1 = {}
    aggs_1['purchase_amount'] = ['sum','max','min','mean']
    aggs_1['purchase_date'] = ['max','min']
    aggs_1['installments'] = ['sum','max','mean']
    aggs_1['month_lag'] = ['max','min','mean']
    aggs_1['month_diff'] = ['max','min','mean']
    aggs_1['price'] = ['sum','mean','max','min']
    aggs_1['duration']=['mean','min','max']
    aggs_1['amount_month_ratio']=['mean','min','max']
    aggs_1['category_1'] = ['mean']
    aggs_1['card_id'] = ['size','count']
    hist_df_1 = hist_df_1.reset_index().groupby('card_id').agg(aggs_1)
    hist_df_1.columns = pd.Index([e[0] + "_" + e[1] for e in hist_df_1.columns.tolist()])
    hist_df_1.columns = ['hist_1_' + c for c in hist_df_1.columns]
    hist_df_1 = reduce_mem_usage(hist_df_1)

    hist_df_1['hist_1_purchase_date_diff'] = (
                hist_df_1['hist_1_purchase_date_max'] - hist_df_1['hist_1_purchase_date_min']).dt.days
    hist_df_1['hist_1_purchase_date_average'] = hist_df_1['hist_1_purchase_date_diff'] / hist_df_1[
        'hist_1_card_id_size']
    hist_df_1['hist_1_purchase_date_uptonow'] = (
                datetime.datetime.today() - hist_df_1['hist_1_purchase_date_max']).dt.days
    hist_df_1['hist_1_purchase_date_uptomin'] = (
                datetime.datetime.today() - hist_df_1['hist_1_purchase_date_min']).dt.days

    hist_df_1 = reduce_mem_usage(hist_df_1)

    return hist_df, hist_df_1
...
def additional_features(df):
    ...
    date_features = ['hist_purchase_date_max', 'hist_purchase_date_min',
                     'hist_1_purchase_date_max', 'hist_1_purchase_date_min',
...
def main(debug=False):
    ...
    with timer("historical transactions"):
        hist_df, hist_df_1 = historical_transactions(num_rows)
        df = pd.merge(df, hist_df, on='card_id', how='outer')
        df = pd.merge(df, hist_df_1, on='card_id', how='outer')
        del hist_df, hist_df_1
    ...
    with timer("Run LightGBM with kfold"):
        extract_feature(train_df, test_df)
