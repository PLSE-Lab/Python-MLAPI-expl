...
images_metadata = pd.read_csv('/kaggle/input/ladi_images_metadata.csv')
aggregated_responses = pd.read_csv('/kaggle/input/ladi_aggregated_responses.tsv', sep='\t')
...
_path = './'
try:
    os.remove(_path + 'images1.tar.gz')
except:
    print('No file')
          
tar = tarfile.open("images1.tar.gz", "w:gz")
p = len(c)
i = 1
batch = 1
filelist = []
