...
def getCod_prov(row):
	...
	mean_value = 25
	range_value = max_value - min_value
	cod_prov = row['cod_prov'].strip()
	if cod_prov == 'NA' or cod_prov == '':
		cod_prov = mean_value
	else:
		cod_prov = float(cod_prov)
		if cod_prov < min_value:
			cod_prov = min_value
		elif cod_prov > max_value:
			cod_prov = max_value
	return round((cod_prov - min_value) / range_value , 4)    
...
def processDataMK(in_file_name, cust_dict, lag_cust_dict):
    ...
    for row in csv.DictReader(in_file_name):
        if row['fecha_dato'] not in ['2015-01-28', '2015-05-28', '2015-06-28', '2016-01-28', '2016-05-28', '2016-06-28']:
            continue
