...
def main(debug = False):
    ...
    with timer("Process bureau and bureau_balance"):
        ...
        bureau = bureau_and_balance(bureau, num_rows)
        print("Bureau df shape:", bureau.shape)
        df = df.join(bureau, how='left', on='SK_ID_CURR')
        del bureau
        gc.collect()
        
        bureau = reduce_mem_usage(pd.read_csv('../input/buro_last_year.csv', nrows = num_rows))
        bureau = bureau_and_balance(bureau, num_rows)
        print("Bureau df shape:", bureau.shape)
        df = df.join(bureau, how='left', on='SK_ID_CURR',rsuffix='_year')
        del bureau
        gc.collect()
        
        #bureau = reduce_mem_usage(pd.read_csv('../input/buro_last_halfyear.csv', nrows = num_rows))
        #bureau = bureau_and_balance(bureau, num_rows)
        #print("Bureau df shape:", bureau.shape)
        #df = df.join(bureau, how='left', on='SK_ID_CURR',rsuffix='_halfyear')
        #del bureau
        #gc.collect()
        
        bureau = reduce_mem_usage(pd.read_csv('../input/buro_last_quarter.csv', nrows = num_rows))
        bureau = bureau_and_balance(bureau, num_rows)
        print("Bureau df shape:", bureau.shape)
        df = df.join(bureau, how='left', on='SK_ID_CURR',rsuffix='_quarter')
        del bureau
        gc.collect()
    ...
    with timer("Run LightGBM with kfold"):
        ...
        b = feat_importance[["feature", "importance"]].groupby("feature").mean().sort_values(by="importance", ascending=True)[:(df.shape[1] - 600)].index
        feat_importance = kfold_lightgbm(df.drop(columns = b), num_folds=5, stratified = True, debug= debug)
