...
# 
# Normally, we would use three RAPIDS cuML function calls. Normally we call `model = KNeighborsClassifier(n_neighbors=KNN)` then `model.fit(X_train,y_train)` and finally `y_hat = model.predict(X_test)`. However RAPIDS is under development and the current version 0.12.0 of RAPIDS throws some errors when we do that. So instead we will call `model = NearestNeighbors(n_neighbors=KNN)` then `model.fit(X_train)` and finally `distances, indices = model.kneighbors(X_test)`. This returns all the neighbors successfully. With the neighbors, we then use `scipy.stats.mode` to find the prediction. This extra work takes a total of 2 minutes whereas it should be 30 seconds with the usual function calls.
