...
tox21_df= pd.read_csv('../input/tox21_descriptors/tox21_df_revised.csv')
...
tox21_descriptors_df= pd.read_csv('../input/tox21_descriptors/tox21_descriptors_df.csv',low_memory=False)
...
tox21_transformer1 = KernelPCA(n_components=nca, kernel='rbf', n_jobs=-1)
...
X_train, y_train, X_test, y_test = iterative_train_test_split(tox21_descriptors_df.values,
                                                              tox21_df.values, 
...
pred = tox21_svc.predict_proba(X_valid)
...
tox21_model.add(Activation('tanh'))
tox21_model.add(Dropout(rate=DROPRATE))
tox21_model.add(Dense(tox21_df.shape[1],kernel_initializer='he_uniform',activation='sigmoid'))


# In[ ]:


tox21_model.compile(loss='binary_crossentropy', optimizer='adam',metrics=['accuracy'])


# In[ ]:


checkpoint = ModelCheckpoint('tox21_model.h5', monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=True, mode='min')


# In[ ]:


hist = tox21_model.fit(X_train, y_train, 
...
tox21_model.load_weights('tox21_model.h5')


# In[ ]:


pred = tox21_model.predict(X_valid)
...
pred = tox21_model.predict(X_test)
...
inp = tox21_model.input
out = tox21_model.layers[-2].output
tox21_model_gb = Model(inp, out)


# In[ ]:


X_train = tox21_model_gb.predict(X_train)
X_test = tox21_model_gb.predict(X_test)
X_valid = tox21_model_gb.predict(X_valid)
...
result = tox21_svc_gb.fit(X_train, y_train)


# In[ ]:


pred = tox21_svc_gb.predict_proba(X_valid)
...
pred = tox21_svc_gb.predict(X_test)
f1_score(y_test,pred,average='macro')
...
pred = tox21_svc_gb.predict_proba(X_test)
...
tox21_xgb_gb = GridSearchCV(OneVsRestClassifier(XGBClassifier(random_state=32)), parameters, cv=3, scoring='roc_auc',n_jobs=-1)


# In[ ]:


result = tox21_xgb_gb.fit(X_train, y_train)


# In[ ]:


pred = tox21_xgb_gb.predict_proba(X_valid)
...
pred = tox21_xgb_gb.predict(X_test)
f1_score(y_test,pred,average='macro')
...
pred = tox21_xgb_gb.predict_proba(X_test)
...
with open('tox21_svc.pkl', 'wb') as fid:
