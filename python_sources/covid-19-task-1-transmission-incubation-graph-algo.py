...
# #### Submission for  What is known about transmission, incubation, and environmental stability?
...
# # Paragraph Search result snapshot. ALL results are at end of the notebook
# 
# ![image.png](attachment:image.png)
...
# # Read papers Selected for Task1 
...
Task='Task 01'
data=pd.read_excel("/kaggle/input/task3covid/task2_results_summary.xlsx")
data=data.dropna(subset=['Text'])
qadata=pd.read_excel("/kaggle/input/covid-task-qa-answer/task2_results_summary_QA.xlsx")
...
Query_Mapping['clQueries']=Query_Mapping['Queries'].str.replace(' ','').values
Query_Mapping['clSubtask']=Query_Mapping['Subtask'].str.replace(' ','').values
qadata['clQueries']=qadata['Queries'].str.replace(' ','').values
qadata['clSubtask']=qadata['Subtask'].str.replace(' ','').values
Look_up=Query_Mapping[Query_Mapping['Task'].str.contains(Task)].merge(qadata,on=['clQueries','clSubtask'],how='left').reset_index(drop=True)
...
for Subtask in Look_up.Subtask_x.unique():
    display(HTML('<font size="6" color="black"> <b> Subtask Searched : </b> </font><p> <font size="4">'+Subtask+'</font><p>'))
    Query_Mapping_Task_query=Look_up[Look_up['Subtask_x']==Subtask].reset_index(drop=True)

    for Queries in Query_Mapping_Task_query['Queries_x'].unique():
        Queries1=Query_Mapping_Task_query[Query_Mapping_Task_query['Queries_x']==Queries]['clQueries'].values[0]
        quer=qadata[(qadata['clQueries']==Queries1) & (qadata['Subtask']==Subtask)]['Question form of queries'].drop_duplicates().values.tolist()
