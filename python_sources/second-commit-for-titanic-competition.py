...
model = tf.keras.models.Sequential([
    tf.keras.layers.Dense(48, input_shape = (24,), activation = tf.nn.leaky_relu),
    tf.keras.layers.Dense(48, activation = tf.nn.leaky_relu),
    tf.keras.layers.Dense(48, activation = tf.nn.leaky_relu),
...
model.fit(X_train, y_train, batch_size = 33, epochs = 50)
...
submit.to_csv('titanic2.csv', index = True)