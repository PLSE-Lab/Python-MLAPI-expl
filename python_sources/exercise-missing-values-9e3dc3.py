...
# X_full = X_full.drop(['SalePrice'], axis=1) # similar to line above 
...
col_with_missing_val = ['LotFrontage', 'MasVnrArea', 'GarageYrBlt']
...
final_X_test = X_test.drop(col_with_missing_val, axis=1)

test_imputer = SimpleImputer()
final_X_test = pd.DataFrame(test_imputer.fit_transform(final_X_test))

final_X_test.columns = reduced_X_valid.columns
