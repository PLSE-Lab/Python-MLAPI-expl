...
def Select_Features(movies_df):
    ...
    '''
    return movies_df[['budget','release_year','no_languages','english_spoken','log_popularity']]
...
preds_train = np.exp(model.predict(train_X))
...
preds_CV = np.exp(model.predict(cv_X))
...
sub_preds = np.exp(model.predict(submission_modified))
