...
for i in range(len(final)):
    final['Text'].values[i]+=str(len(final['Text'].values[i]))
...
print(alpha_in)        
for i in alpha_in:
    # instantiate learning model (10^-4a < alpha < 100000)
    clf =MultinomialNB( fit_prior=True, class_prior=None,alpha=i)
    ...
    #finding best alpha using max value of auc score
    if roc_auc_score(y_cv,pred_cv)>max_auc_score1:
        alpha_best1=i
        max_auc_score1=roc_auc_score(y_cv,pred_cv)


# In[ ]:


print("best value of alpha is",alpha_best1)        
print("max auc score is",max_auc_score1)


# In[ ]:


# plotting curve between log alpha vs Train and Cross validation Data
alpha_out1=np.log(alpha_out)
plt.plot(alpha_out1,roc_cv ,label="AUC cv")
plt.plot(alpha_out1,roc_tr,label="AUC train")
plt.legend()
...
clf = MultinomialNB(alpha=alpha_best1, fit_prior=True, class_prior=None)
clf.fit(final_X_tr, y_tr)
#predicting probability on Test data
pred_test = clf.predict_log_proba(final_X_test)
...
predic=clf.predict(final_X_test)
...
for i in alpha_in:
    # instantiate learning model (10^-5 < alpha < 100000)
    clf =MultinomialNB( fit_prior=True, class_prior=None,alpha=i)
    ...
    if roc_auc_score(y_cv,pred_cv)>max_auc_score1:
        alpha_best2=i
        max_auc_score2=roc_auc_score(y_cv,pred_cv)


# In[ ]:


print("best value of alpha is",alpha_best2)        
print("max auc score is",max_auc_score2)


# In[ ]:


# plotting curve between alpha vs Train and Cross validation Data
alpha_out2=np.log(alpha_out)
plt.plot(alpha_out2,roc_cv ,label="AUC cv")
plt.plot(alpha_out2,roc_tr,label="AUC train")
plt.legend()
plt.title('AUC Score vs log alpha')
plt.xlabel('Log Alpha')
plt.ylabel('AUC')
...
x = PrettyTable()
x.field_names = ["Vectorizer", "Hyperameter", "AUC"]
x.add_row(["BOW",alpha_best1,max_auc_score1])
x.add_row(["TFIDF",alpha_best2,max_auc_score2])
print(x)
