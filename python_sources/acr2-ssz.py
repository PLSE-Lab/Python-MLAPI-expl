...
rf1 = RandomForestClassifier(n_estimators=500, n_jobs=-1, criterion="entropy", max_depth=10, max_features=6, min_samples_leaf=2, random_state=1)
...
test_probs = 0.39*rf.predict_proba(test[features])[:,1] + 0.41*gbm.predict(xgb.DMatrix(test[features])) + 0.2*rf.predict_proba(test[features])[:,1] 
submission = pd.DataFrame({"id": test["id"], "prediction": test_probs})
submission.to_csv("ub_rf_xgboost_submission_2.csv", index=False)
