...
# #### Week 25th week has just started.
# 
# Confirmed Cases are showing upward trend every week, recording highest number of Confirmed cases in 24th week (180k+ Confirmed Cases).
# 
...
C1=[]
while(1):
    rise_50k.append(int(india_datewise[india_datewise["Confirmed"]<=case_50k].iloc[[-1]]["Days Since"]))
    C1.append(case_50k)
    case_50k=case_50k+50000
    if(case_50k<india_datewise["Confirmed"].max()):
        continue
    else:
        break
rate_50k=pd.DataFrame(list(zip(C1,rise_50k)),columns=["No. of Cases","Days Since first Case"])
rate_50k["Days required for increase by 50K"]=rate_50k["Days Since first Case"].diff().fillna(rate_50k["Days Since first Case"].iloc[0])
...
fig=go.Figure()
fig.add_trace(go.Scatter(x=rate_50k["No. of Cases"], y=rate_50k["Days required for increase by 50K"],
                    mode='lines+markers',
                    name='Weekly Growth of Confirmed Cases'))
fig.update_layout(title="Number of Days required for increase in number of cases by 50000",
...
holt=Holt(np.asarray(model_train["Confirmed"])).fit(smoothing_level=0.2, smoothing_slope=1.9)
...
es=ExponentialSmoothing(np.asarray(model_train['Confirmed']),seasonal_periods=14, trend='mul', seasonal='add').fit()
...
model_ar= auto_arima(model_train["Confirmed"],trace=True, error_action='ignore', start_p=0,start_q=0,max_p=1,max_q=0,
