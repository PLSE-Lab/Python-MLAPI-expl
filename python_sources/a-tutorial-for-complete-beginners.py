...
print('Training accuracy...', accuracy_score(y_train, clf_train.predict(X_train)))
print('Validation accuracy', accuracy_score(y_test, clf_train.predict(X_test)))
...
with open("tree1.dot", 'w') as f:
    f = tree.export_graphviz(clf_train,
                                  out_file=f,
                                  impurity = False,
...
clf_train = DecisionTreeClassifier(max_depth = 3)
clf_train.fit(X_train, y_train)
...
test_pred = test[['Pclass', 'Sex', 'Age', 'SibSp', 'Parch', 'Fare']]
...
predictions = clf_train.predict(test_pred)
