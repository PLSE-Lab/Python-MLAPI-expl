...
# 
# The purpose of this kernel is to showcase the speedup that one gets with t-SNE algorithm between the SKLEARN version and the Rapids version. The Sklearn version can be found [here](https://www.kaggle.com/tunguz/mnist-2d-t-sne)
...
# So we get five seconds instead of three hours! That's a really substantial speedup. A speedup of over 2000X in Kaggle Kernels!
# 
# Now let's visualize to see what the output looks like. We'll have to retrain t-SNE on train features only, because we want to see how the embeddings correspond to different digits. 
