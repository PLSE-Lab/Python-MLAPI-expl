...
MAX_BLOCK_NUM = train.date_block_num.max() #33 months are included, so maxblocknum=33 by default
MAX_ITEM = len(test_items) #5100 items
MAX_CAT = len(item_cats) #84
MAX_YEAR = 3
MAX_MONTH = 4 # 7 8 9 10
MAX_SHOP = len(test_shops) #42
...
for s in test_shops:
    ...
    #go over all shop items
    for it in shop_items:        
        #go over {1,6},{2,18},{3,30}
        for i_index, i in enumerate(BLOCKS):
            sentence = []
            closest_pc = closest_price(i, it, s)           
            #check 4 months prior to nov.
            for j in range(maxlen+1):
                if j < maxlen:
                    #als shop, item, jul-nov
