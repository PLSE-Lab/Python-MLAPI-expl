...
class HumanProteinDataset(Dataset):
    ...
    def __getitem__(self, idx): # access particular data element -> converts it to tensor 
        row = self.df.loc[idx]
        img_id, img_label = row['Image'], row['Label'] # label comes from panda dataframe
...
def show_sample(img, target, invert=True): # invert blacks to whites and vice versa for visual effect
    if invert:
...
class ProteinCnnModel(MultilabelImageClassificationBase):
    def __init__(self):
        ...
        self.network = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(64, 10),
            nn.Sigmoid(0,9) # sigmoid converts every output into binary information rather than 
