...
"""
		...
		1) MinMaxScaler on the train set of all the digits "2"
		
		2) Isomap on these digits to 2 components.
		
		3) Create overlapping intervals on first dimension (x-axis) and cluster points inside this overlap.
		...
		ref: www.ayasdi.com/_downloads/Topological_Methods_for_the_Analysis_of_High_Dimensional_Data_Sets_and_3D_Object_Recognition.pdf
			 www.ams.org/images/carlsson-notes.pdf "Topology and Data"
			 wearables.cc.gatech.edu/paper_of_week/isomap.pdf "A global geometric framework for nonlinear dimensionality reduction"
...
if __name__ == "__main__":
	...
	mapper = KeplerMapper(cluster_algorithm=cluster.DBSCAN(eps=0.3, min_samples=15), 
							 reducer = manifold.Isomap(), nr_cubes=35, overlap_perc=0.5, 
							 link_local=False, verbose=1)
	...
	# Create the graph
	complex = mapper.map(data, dimension_index=[0,1], dimension_name="Isomap(2) 2D")

	# Create the visualizations (increased the graph_gravity for a tighter graph-look.)

	# Tooltips with image data for every cluster member
	mapper.visualize(complex, "results.html", "Isomap on all '2'-digits in MNIST", graph_charge=-70, graph_gravity=0.15, custom_tooltips=tooltip_s)
