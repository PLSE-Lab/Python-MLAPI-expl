...
# 
# 
# - [Population Percentage of Delhi](#Population)
# 
# - [Cases in Delhi](#Cases)
# 
...
# 
# - [Non virus deaths in Delhi](#Non_Virus_Deaths)
# 
...
#Pick the data of Delhi

covid = India_covid[India_covid['State/UnionTerritory']=='Delhi'].reset_index(drop=True).drop(['State/UnionTerritory', 'Sno', 'Time'], axis = 1)

population = India_population[India_population['State / Union Territory']=='Delhi'].reset_index(drop=True).drop(['State / Union Territory', 'Sno'], axis = 1)

ICMRTestingLabs = ICMRTestingLabs[ICMRTestingLabs['state']=='Delhi'].reset_index(drop=True).drop('state', axis = 1)

HospitalBeds = HospitalBeds[HospitalBeds['State/UT']=='Delhi'].reset_index(drop=True).drop(['State/UT', 'Sno'], axis = 1)

TestingDetails = TestingDetails[TestingDetails['State']=='Delhi'].drop('State', axis = 1)
...
for i in range(0, len(covid)):
    if((i>=0 and i<=10) or (i>=30 and i<=41) or (i>=60 and i<=71) or (i>=91 and i<=102)):
...
# # **Population Percentage of Delhi**  <a name = "Population"> </a>

# In[ ]:


Total = India_population['Population'].sum()
DL = population['Population']
labels = ["Rest of the India", 'Delhi']
values = [float(Total - DL), float(DL)]
...
fig = go.Figure(data = [go.Pie(title = dict(text= 'Population Percentage', position = 'top right'), labels= labels, values = values, textinfo='label+ percent', marker_colors = colors,
                              textfont = dict(color = 'black'), pull = [.05,0])])

fig.update_layout(title_text= 'Population of Delhi', showlegend = False)

fig.show()


# # **Cases in Delhi** <a name = 'Cases'></a>
...
DL_confirmed = covid.Confirmed.iloc[-1]
DL_deaths = covid.Deaths.iloc[-1]
DL_cured = covid.Cured.iloc[-1]


confirm = [Total_confirmed, DL_confirmed]
cured = [Total_cured, DL_cured]
deaths = [Total_deaths, DL_deaths]
...
# # **Confirmed Cases in Delhi** <a name= 'Confirmed'></a>
...
# # **Active Cases in Delhi** <a name = 'Active'></a>
...
# # **Covid Deaths in Delhi** <a name = 'Deaths'></a>
...
# # **Cured Cases in Delhi** <a name = "Cured"> </a>
...
# # **Non virus deaths in Delhi** <a name = "Non_Virus_Deaths"></a>
...
DL = non_virus_deaths.groupby('state')['deaths'].sum()['DL']
...
DL = non_virus_deaths[non_virus_deaths['state']=='DL']['reason'].sum()
...
DL_reason = pd.DataFrame(group_list(DL), columns = ['Reason', 'Count'])
DL_reason.sort_values(by = ['Count'], ascending = False, inplace = True)
DL_reason.reset_index(drop = True, inplace = True)


Total_total_count = Total_reason['Count'].sum()
DL_total_count = DL_reason['Count'].sum()
...
DL_other = DL_total_count - (DL_reason['Count'][0] + DL_reason['Count'][1]
                             + DL_reason['Count'][2] + DL_reason['Count'][3] + DL_reason['Count'][4])
...
DL_y_data = [int(DL_reason['Count'][0]/DL_total_count*100), int(DL_reason['Count'][1]/DL_total_count*100),
           int(DL_reason['Count'][2]/DL_total_count*100), int(DL_reason['Count'][3]/DL_total_count*100),
           int(DL_reason['Count'][4]/DL_total_count*100), int(DL_other/DL_total_count*100)]
...
DL_x_data = [DL_reason['Reason'][0], DL_reason['Reason'][1], DL_reason['Reason'][2], DL_reason['Reason'][3], DL_reason['Reason'][4], 'Other']
...
fig.append_trace(go.Bar(
    ...
    name='Non Virus Deaths in Delhi',
    orientation='h',
), 1, 2)
...
DL_y = np.array(DL_y_data)

# Adding labels
for ydn, yd, xdn, xd in zip(DL_y, Total_y, DL_x_data,  Total_x_data):
