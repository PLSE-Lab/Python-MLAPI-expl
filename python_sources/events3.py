...
tmp = f3['device_id'].unique()
train = train[train.device_id.isin(tmp)]
test = test[test.device_id.isin(tmp)]
...
#f1 = f1[f1['device_id'].isin(f3['device_id'])]
#f2 = f2[f2['device_id'].isin(f3['device_id'])]

tmp = f3['device_id'].unique()
f1 = f1[f1.device_id.isin(tmp)];
f2 = f2[f2.device_id.isin(tmp)];
...
X_train, X_val, y_train, y_val = train_test_split(
    train_sp, Y, train_size=0.999, random_state=10,stratify=Y)
...
for itr in range(10):
    ...
    device_id = device_id.reset_index()
    device_id.drop(["index"], axis=1, inplace=True)
    ...
    #Drop out 0.2
    #Validation 2.3017
    name = 'events_only'+str(itr)+'.csv'
    result.to_csv(name, index=True, index_label='device_id')
