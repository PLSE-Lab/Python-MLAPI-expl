...
Y_binarised_train = train_new['Rating'].map(lambda x: 0 if x < THRESHOLD else 1)
...
Y_test = mp_neuron.predict(X_binarised_test)


# In[ ]:


submission = pd.DataFrame({'PhoneId':test_new['PhoneId'], 'Class':[1]*Y_test.shape[0]})
