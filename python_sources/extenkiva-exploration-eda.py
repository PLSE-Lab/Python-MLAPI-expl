...
#df_kiva = df_kiva.sample(10000, replace=True)
#df_kiva_loc = df_kiva_loc.sample(10000, replace=True)
...
# ## Let's take a look at the distribuition of the Loan value by each country'
...
trace = go.Box(
    ...
    name="TOP 20 Countrys Loan Distribuition", showlegend=False)

data = [trace]

layout = go.Layout(
    yaxis=dict(
        title="Loan Amount Distribuition"
    ),
    xaxis=dict(
        title="TOP 20 Countrys"
    ),
    title="TOP 20 Countrys Loan Distribuition"
)
...
# - We can see an interesting behavior in some country's. For instance the Kenya, that have a high number of outlier values
...
g.set_xticklabels(g.get_xticklabels(),rotation=90)
g.set_title("The 20 most Frequency Currencies at Platform", fontsize=15)
g.set_xlabel("")
g.set_ylabel("Count", fontsize=12)
...
# We can verify a predominance of low velues in all currency's
