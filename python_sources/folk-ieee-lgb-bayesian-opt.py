...
for f in lb_enc:
    ...
    df_train_X = pd.DataFrame(X.toarray()[:train_df.shape[0]], columns=[f + str(i) for i in range(num_val)])
    df_test_X = pd.DataFrame(X.toarray()[train_df.shape[0]:], columns=[f + str(i) for i in range(num_val)])
    
    train_df = pd.concat([train_df, df_train_X], axis=1)
    test_df = pd.concat([test_df, df_test_X], axis=1)
