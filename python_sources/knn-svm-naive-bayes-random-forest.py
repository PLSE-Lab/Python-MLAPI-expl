"""
Analyses a database from Kaggle (https://www.kaggle.com/uciml/biomechanical-features-of-orthopedic-patients) with the
goal of classifying normal vs abnormal gait, and further Disk Hernia (DH) and Spondylolisthesis (Sp).

Four classifiers (KNN, SVM, Naive Bayes, and Random Forest) are fit to each data set and their normalized accuracies
are reported. A single train/test split is used to report a confusion matrix and respective accuracy. Next, k-fold 
cross validation is used to create a stronger accuracy report. 

NOTE: At this stage, no tuning of the classifier parameters has been performed aside from entropy criterion for RFC
      Also, no feature selection has been performed
"""
...
def data_preprocessing():
    ...
    :return: train and test data splits for 2-class and 3-class data in respective lists train_test_2c and train_test_3c
    """
    from sklearn.preprocessing import LabelEncoder
    from sklearn import model_selection

    # Import csv data and split into independent (X) and dependent (y) for normal / abnormal classification
    data_2c = pd.read_csv('../input/column_2C_weka.csv')
    ...
    data_3c = pd.read_csv('../input/column_3C_weka.csv')
...
def results_comparison(knn_results, svm_results, nb_results, rf_results, cm_output):
    """
...
def main():
    ...
    results_comparison(knn_results, svm_results, nb_results, rf_results, True)
