...
def mygenerator_batch(tabl, bs):
    gen = mygenerator(tabl)
    while True:
        train = []
        mask = []
        for i in range(bs):
...
for x in test_files:
    ...
    for ln in range(1, nlables+1):
        ...
        result.append([x, str(rle)])
 
out = pd.DataFrame(result, columns = ['ImageId','EncodedPixels'])
print(out)
...
class NucleiConfig(Config):
    ...
    NUM_CLASSES = 1 + 1  # background + 1 nucleus
...
def RLencToPixels(runs):
    ...
    return pixels
def maskFromPixels(pixels,h,w):
    new_mask = np.zeros((h,w), dtype=np.float32)
    #print(max(pixels))
    for i in pixels:
        if(i/w<w):
            new_mask[i%h,int(i/w)] = 1
    return new_mask
        
masks_raw = pd.read_csv('../input/train_masks.csv')
masks_raw = np.array(masks_raw)
masks_input = dict()
for row in masks_raw:
    if row[0] in masks_input:
        masks_input[row[0]].append(row[1])
    else:
        masks_input[row[0]] = [row[1]]
train_dir = "../input/train_images/train_images/"
ids = sorted((os.listdir(train_dir)))
#random.shuffle(ids)
    
VAL_IMAGE_IDS = ids[400:433]
class NucleusDataset(Dataset):
    ...
    def load_nucleus(self, dataset_dir, subset):
        ...
        for image_id in image_ids:
            self.add_image(
                "nucleus",
                image_id=image_id,
                path=dataset_dir + image_id + "/images/" + image_id + ".png")
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        mask = []
        for i in range(len(masks_input[info["id"]])):
            mask.append(maskFromPixels(RLencToPixels(masks_input[info["id"]][i]), 256, 256))
        mask = np.stack(mask, axis=-1)
        # Return mask, and array of class IDs of each instance. Since we have
        # one class ID, we return an array of ones
        if mask.shape[0] != 256:
            print("Nonconforming mask:" + mask.shape + info["id"])
...
dataset_train.load_nucleus("../input/train_images/train_images/", "train")
dataset_train.prepare()
dataset_val = NucleusDataset()
dataset_val.load_nucleus("../input/train_images/train_images/", "val")
dataset_val.prepare()
...
detect(model, "../input/test_images/test_images/", "train")
os.listdir("../working")