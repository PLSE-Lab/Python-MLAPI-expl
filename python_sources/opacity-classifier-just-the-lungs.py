...
import os
lseg_dir = os.path.join('..', 'input', 
                        'training-u-net-on-tb-images-to-segment-lungs')
get_ipython().system("unzip -q {os.path.join(lseg_dir, 'masks.zip')}")
...
def read_dicom_image(in_path):
    """a frankenstein function to mask the images from the segmentations"""
    img_arr = pydicom.read_file(in_path).pixel_array
    mask_path = in_path.        replace('../input/rsna-pneumonia-detection-challenge/','').        replace('.dcm', '.png')
    mask_arr = np.array(Image.open(mask_path)).astype(bool)
...
from keras.layers import Dense, Dropout, Flatten, Input, Conv2D, multiply
from keras.models import Model
from keras.optimizers import Adam
pt_features = Input(base_pretrained_model.get_output_shape_at(0)[1:], name = 'feature_input')
pt_depth = base_pretrained_model.get_output_shape_at(0)[-1]
from keras.layers import BatchNormalization
gap_raw = layers.GlobalMaxPooling2D()(pt_features)
gap = BatchNormalization()(gap_raw)
...
get_ipython().system('rm -rf stage_*')
