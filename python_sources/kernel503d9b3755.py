...
df_train= pd.read_csv('/kaggle/input/covid-create/train.csv')
df_test= pd.read_csv('/kaggle/input/covid-create/test.csv')
...
countries = c_train.Country_code.unique()
...
for country in countries:
    states = c_train.loc[c_train.Country_code == country, :].State_code.unique()
    #print(country, states)
    # check whether string is nan or not
    for state in states:
        X_xTrain_CS = c_train.loc[(c_train.Country_code == country) & (c_train.State_code == state), ['State_code','Country_code', 'Date', 'ConfirmedCases', 'Fatalities']]
        
        y1_xTrain_CS = X_xTrain_CS.loc[:, 'ConfirmedCases']
        y2_xTrain_CS = X_xTrain_CS.loc[:, 'Fatalities']
        
        X_xTrain_CS = X_xTrain_CS.loc[:, ['State_code','Country_code', 'Date']]
        
        X_xTest_CS = c_test.loc[(c_test.Country_code == country) & (c_test.State_code == state), ['State_code','Country_code', 'Date', 'ForecastId']]
        
        X_xTest_CS_Id = X_xTest_CS.loc[:, 'ForecastId']
        X_xTest_CS = X_xTest_CS.loc[:, ['State_code','Country_code', 'Date']]
