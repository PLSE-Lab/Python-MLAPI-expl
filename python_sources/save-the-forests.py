...
class Clean:
    ...
    def scale_features(cls, df, cols=[]):
        scaler = StandardScaler()
        df_name = df.name
        if df_name == 'train':
            feature_cols = df.drop(columns=[cls.id_col,cls.target_col]).columns
        if df_name == 'test':
            feature_cols = df.drop(columns=[cls.id_col]).columns
        feature_values = train_df.loc[:, feature_cols].values
        feature_values = scaler.fit_transform(x)
        feature_values_df = pd.DataFrame(data=feature_values, columns=feature_cols)
        feature_values_df = pd.concat((df[cls.id_col], feature_values_df), axis=1)
        if df_name == 'train':
            df = pd.concat((feature_values_df, df[cls.target_col]), axis=1)
        if df_name == 'test':
...
class Engineer:
    ...
    def simplify_feature(cls, df):
        ...
        bins = (0, 2000, 2250, 2750, 3000, 3250)
        group_names = [2000, 2250, 2750, 3000, 3250]
        categories = pd.cut(df['Elevation'], bins, labels=group_names)
        df['Elevation_Mod'] = categories
        return df
...
class Model:
    ...
    def pca_fit_transform(cls, df):
        pca = PCA(.8)
        df_name = df.name
        if df_name == 'train':
            target_data = df[cls.target_col]
            df.drop(cls.target_col, axis=1, inplace=True)
        pca.fit(df)
        pca_values = pca.transform(df)
    ...
    def tune_parameters(cls, model, parameters):
    ...
    def cross_validate(cls, model, parameters):
        ...
        for train_index, test_index in skf.split(X, y):
            ...
            X_predictions = temp_model.predict(X_test)
...
class Data(Explore, Clean, Engineer, Model):
    ...
    def __init__(self, train_csv, test_csv, target_col='', id_col='',
                 ignore=[], keep=[], col_sum=[]):
...
def run(d, model, parameters):
    mutate = d.mutate
    # mutate(d.simplify_feature)
    # mutate(d.encode_categorical, ['Elevation_Mod'])
    # mutate(d.scale_quant_features, ['Elevation'])
    # mutate(d.sum_features,[[
    #         'Horizontal_Distance_To_Roadways',
    #         'Horizontal_Distance_To_Fire_Points',]
    #         ])
    # e = Explore()
    # train = d.get_df('train')
    # skewed = e.get_skewed_features(train, ['Elevation', 'Aspect', 'Slope', 'Horizontal_Distance_To_Hydrology',
    #     'Vertical_Distance_To_Hydrology', 'Hillshade_9am', 'Hillshade_Noon',
    #     'Hillshade_3pm'])
    # mutate(d.normalize_features, skewed)
    mutate(d.sample_ros)
    # mutate(d.scale_features)
    mutate(d.drop_ignore)
    mutate(d.fill_na_simple)
    # mutate(d.pca_fit_transform)
    # mutate(d.fill_na_simple)
    # model, parameters = d.tune_parameters(model, parameters)
    # print(parameters)
