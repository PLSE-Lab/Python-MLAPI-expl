...
new_cols = ['street', 'avenue', 'east', 'west', 'north',
'south','ave','blvd','square','park','place']
...
param['colsample_bytree'] = 0.7 #0.5
...
plst2 = list(param.items())

param = {}
param['objective'] = 'multi:softprob'
param['eta'] = 0.005  #0.016
param['max_depth'] = 6#6
param['silent'] = 1
param['num_class'] = 3
param['eval_metric'] = "mlogloss"
param['min_child_weight'] = 1
param['subsample'] = 0.85#0.80
param['colsample_bytree'] = 0.55 #0.5
param['colsample_bylevel'] = 0.7 #
#param['num_parallel_tree'] = 3 #
param['seed'] = 1
param['lambda'] = 1
...
if cv :
        ...
        for dev_index, val_index in kf.split(range(df_train.shape[0])):
                ...
                model2 = xgb.train(plst2, xgtrain, num_rounds2, watchlist, early_stopping_rounds=15)
                pred_test_xgb2  = model2.predict(xgtest)
                score2=log_loss(val_y, pred_test_xgb2)
                
                #model3 = xgb.train(plst3, xgtrain, num_rounds, watchlist, early_stopping_rounds=15)
                #pred_test_xgb3  = model3.predict(xgtest)
                #score3=log_loss(val_y, pred_test_xgb3) 
                
                print (noa,' *** XGB1  ',score1)
                print (noa,' *** XGB2  ',score2)
                #print (noa,' *** XGB3  ',score3)
                
                #Ansamble
                
                y_val_pred7=(pred_test_xgb  +pred_test_xgb2 )/2
                score=log_loss(val_y, y_val_pred7)
                print (noa,' 5/5/ XGB_ANS  ',score)
                
                y_val_pred7=(0.3*pred_test_xgb  +0.7*pred_test_xgb2  )
                score=log_loss(val_y, y_val_pred7)
                print (noa,' 3/7 XGB_ANS  ',score)
                
                y_val_pred7=(0.7*pred_test_xgb  +0.3*pred_test_xgb2  )
                score=log_loss(val_y, y_val_pred7)
                print (noa,' 7/3 XGB_ANS  ',score)
                
                y_val_pred7=(0.25*pred_test_xgb  +0.75*pred_test_xgb2  )
                score=log_loss(val_y, y_val_pred7)
                print (noa,' 25/75 XGB_ANS  ',score)
                
                y_val_pred7=(0.75*pred_test_xgb  +0.25*pred_test_xgb2  )
                score=log_loss(val_y, y_val_pred7)
                print (noa,' 75/25 XGB_ANS  ',score)
                break
...
model2 = xgb.train(plst2, xgtrain, num_rounds2)
pred_test_xgb2  = model2.predict(xgtest)
print ('XGB2 train OK')

#model3 = xgb.train(plst3, xgtrain, num_rounds)
#pred_test_xgb3  = model3.predict(xgtest)
#print ('XGB3 train OK')
y=(0.75*pred_test_xgb +0.25*pred_test_xgb2 )
#================================================
#Output setup
sub = pd.DataFrame()
...
sub.to_csv("subxansa3.csv", index=False)
