...
def getAll(#obj='airplane',
    train=True, is_local=False, cube_len=64, obj_ratio=1.0):
    objPath = '../input/train/train/'
...
volumes = getAll(#obj=obj, 
    train=True#, is_local=is_local, obj_ratio=obj_ratio
)
...
def discriminator(inputs, phase_train=True, reuse=True):
...
def trainGAN(is_dummy=False,output=[]):
    ...
    with tf.Session() as sess:  
        ...
        if is_dummy:
            ...
            volumes = getAll(#obj=obj, 
                train=True, is_local=is_local, obj_ratio=obj_ratio)
            print ('1st Trial')#'Using ' + obj + ' Data'
...
output=[np.where(i>0.5,1,0) for i in output]
output=[output[i].reshape((64,64,64)) for i in range(0,len(output))]

data=np.reshape(output,(len(output),262144))
