...
#NOTE: Code modified to get the same result 
...
#NOTE: Code added from keras github repository
f = np.load('../input/imdb.npz')
...
# On CPU, this will take less than two seconds per epoch -- training is over in 20 seconds. NOTE: 2s per epoch in kaggle kernel. At the end of every epoch, there is a slight pause as the model computes its loss and accuracy on the 10,000 samples of the validation data.
# 
